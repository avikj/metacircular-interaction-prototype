...
# `machine/karana sima` is the list of what this whole assembly cannot do, and
...
ghc -O0 -i"$root/machine" -outputdir "$out" -o "$root/machine/karana" \
    "$root/machine/AnyatKaranaRun.hs" >"$out/build.log" 2>&1 \
  || { echo "the build failed:"; grep -E "error" -A6 "$out/build.log" | head -30; exit 1; }
echo "built."
echo

"$root/machine/karana" pariksa
echo
...
if command -v jq >/dev/null 2>&1; then
  n=$("$root/machine/karana" sadhana --kevala | jq 'length')
  echo "schema: valid JSON, $n tools"
  "$root/machine/karana" sadhana | jq -e '.["uttara-rupa"].margau | length == 2' >/dev/null \
...
rm -f "$lekha"
SABHA_LEKHA=$lekha "$root/machine/check-sabha.sh" >/dev/null 2>&1 || true
if [ -s "$lekha" ]; then
  "$root/machine/karana" abhyasa "$lekha" > "$out/check-karana-abhyasa.jsonl"
