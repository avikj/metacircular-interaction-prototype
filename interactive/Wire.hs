-- Json â” the transport layer of the sabh, and the
...
-- Bhya, c. 400 CE).  A wire is exactly that â” everything the machine
...
--
--   1. THERE IS NO BOOLEAN.  âˆAâˆâ has no retraction â” à¨à¾àààà¿-ààà°ààà¯à¾à¨à¯à¨à®à,
--      AHIMSA_SUTRA_VISTARA Â§5 â” so a `true` on the wire is not a compact
...
-- `fibre/src/Loss/KuttakaValli_â¦`:
--
--     àµà²àà²à, the step, is a total function ààà°à¿à•à â’ ààà°à¿à•à written with NO
--     comparison, NO `Dec`, NO `Bool` â” because àà•ààà has already recorded
...
-- to stand where the slot was.  That is ryabhaa.s àààà à°à•àà read as
...
-- àµà²àà²à needs no comparison because àà•ààà kept the slot, which is checkable
...
parseLine s = do
  ...
  case skip rest of
    ...
    r  -> Left ("trailing text after the utterance: " ++ show (take 40 r)
                ++ " â” one utterance per line on this wire")
       ...
       then Left ("a number was started and not written: " ++ show (take 12 s))
         ...
         (c:_) | c == '.' || c == 'e' || c == 'E' ->
           Left ("a float arrived on the wire: " ++ show (takeWhile (`notElem` ",}] ") s)
                 ...
                 ++ "exact object â” a ratio as two integers, or the integer "
                 ++ "itself â” and if the quantity is genuinely approximate, "
...
-- machine, reached `hPutStrLn`, and the encoder threw THERE â” half an answer
-- already on the wire, exit 1, the transcript never created, the session's
-- written remainders destroyed unwritten.  That is neither road (Â§6:
-- ààààà¯à‹ à®à¾à°àà—à‹ à¨ àµà¿à¦àà¯àà) and it was the default behaviour of this branch.
--
-- Surrogates exist only to encode a supplementary character as a PAIR in
-- UTF-16.  So the pair is accepted and combined â” nothing is lost, that is
-- the transport â” and a lone one is turned back with the repair named, which is
...
uEscape n rest
  | n >= 0xD800 && n <= 0xDBFF =
      case rest of
        ...
        _ -> Left (loneMsg n "high" "a low surrogate \\udc00â“\\udfff")
  | n >= 0xDC00 && n <= 0xDFFF =
      Left (loneMsg n "low" "a high surrogate \\ud800â“\\udbff before it")
  ...
  where
    ...
    loneMsg v which want =
      ...
      ++ "here would hand the writer a value it cannot write â” and the "
...
--   arrived from somewhere other than this parser â” a handler that built it,
--   a file read under a different decoder â” and `render` would then produce
...
--   the answer is Just.  Â§6, held at the boundary where it was being lost.
...
-- with no error and no residue â” so the parser preserved the multiplicity and
...
-- about: `look` has the same two roads as everything else (Â§6), and a key
...
look k (JObj kvs) = case [ v | (k', v) <- kvs, k' == k ] of
  ...
  vs  -> Left ("key `" ++ k ++ "` is bound " ++ show (length vs)
               ...
               ++ "and no way back (âˆAâˆâ has no section: AHIMSA_SUTRA_VISTARA "
               ++ "Â§5), so it is turned back rather than resolved.  Send the key "
               ++ "once; if you meant several things, send them as one list "
               ++ "under it: \"" ++ k ++ "\":[â¦] â” a list says `these are "
...
--
-- â” because `Either String a` has two positions and the reader needs three.
...
-- cannot do.  A default may attach to àà¨àà•ààà®à and to nothing else.
--
-- The correct pattern was already in the kernel twice â” kuaka's `c` and
-- vargaprakti's `n` are read with `case look k j of Left _ -> â¦; Right
-- (JInt n) -> â¦; Right other -> turned back` â” and was written out longhand each
-- time, which is why the other four sites did not get it.  Â§35: à¯àà ààààà¨à¾
-- àà¨àà¯àà ààà à¨ àààà¾ààà¯àà â” what a rule generates is not stored in six copies.
--

-- | What a key on the wire is, from the reader's side.  Three, not two.
data Vacana a
  = Anuktam            -- ^ àà¨àà•ààà®à â” not uttered.  The key is absent.
  | Uktam a            -- ^ à‰à•ààà®à â” uttered, and readable in the shape asked for.
  | Durvacam String    -- ^ à¦àà°ààµàà®à â” uttered and NOT readable.  Carries the
...
-- | àààµà¾ â” "or else".  The ONLY sanctioned way to give a key a default, and
--   the whole of its discipline is in which constructor it may touch: the
--   default applies to àà¨àà•ààà®à and never to à¦àà°ààµàà®à.  A `Left` out of this is
...
-- | àà¨à§à¿à•ààà®à â” given the names an operation declares, the keys in an
...
-- nothing â” the reader took the first and the rest went with no trace â” and
...
-- smuggling a fifth constructor in, and the hole it names was real â” the
...
-- drops from `the type has no constructor for it` â” mechanical, at the
-- moment of the act â” to `nobody has done it yet`.  Â§5: à¨àààŸà¿à°ààµà¿à¨à¾àà à à¯àà
-- à¨àààŸà ààà àà—àà°à à¨ à²ààà¯à®à â” that is not a trade.
--
