-- Obstruction â” the kernel's negative answers, read as obligations: material
...
-- statement of WHY it did not close â” and that statement is truncated to 160
-- characters, written to a log nothing reads, and collapsed to `False`.
--
-- Read three of them from interactive/machine.log:
--
--   x    != max x x      of type â•   when checking that refl has type x â‰¡ max x x
--   zero != x âˆ x        of type â•   when checking that refl has type zero â‰¡ x âˆ x
...
-- terms at the point where computation stalled â” the residual.
--
-- And the third one is the interesting kind.  The goal was `x â‰¡ 1 Â x`; the
-- residual is `x â‰¡ x + 0 Â x`.  Those are not the same statement.  Agda
-- unfolded `1 Â x` one step and got stuck, so the residual is a NEW, more
-- primitive subgoal â” and it is exactly the missing lemma.  Prove
-- `x + 0 Â x â‰¡ x` and the parent closes.
--
...
-- is the other thing â” new material forced by where the work actually stalled.
--
-- This module turns that text back into terms.  It parses nothing it cannot
-- parse honestly: sections (`cong (x Â_)`), implicit arguments and anything
...
-- codebase â” there is no shared core â” but introducing a shared module is a
...
-- `KernelRefl` standpoint â” Agda's definitional equality with `refl` as the
...
--
--   _Â_  infixl 7      _+_  infixl 6      _âˆ_  infixl 6
...
-- left-associative + and âˆ at the same level
pAdd :: P Term
pAdd s0 = do
  (t0, r0) <- pMul s0
  go t0 r0
  where
    go acc r = case skip r of
      ('+':r') -> step "+" acc r'
      cs | "âˆ" `isPrefixOf` cs -> step "-" acc (drop (length "âˆ") cs)
      _ -> Just (acc, r)
    step op acc r' = do
      (u, r'') <- pMul r'
      go (F op [acc, u]) r''
...
pMul s0 = do
  ...
  where
    go acc r = case skip r of
      cs | "Â" `isPrefixOf` cs -> do
             (u, r'') <- pApp (drop (length "Â") cs)
             go (F "*" [acc, u]) r''
      _ -> Just (acc, r)
...
-- WHY FEEDING RESIDUALS BACK TERMINATES â” and why the obvious argument for
...
-- DECREASE â” Euclidean descent â” and the residuals here DO NOT.  Agda unfolds
-- as it goes, so a residual can be larger than the goal that produced it:
--
--     goal  x â‰¡ 1 Â x        residual  x â‰¡ x + 0 Â x     (larger, not smaller)
--
...
-- something it did not know when it failed â” the feedback cannot run away.
--
...
-- the stream and both are false: `xÂx = s(x)` (30 occurrences),
-- `xÂmax(x,1) = s(x)` (100 occurrences).  Verify the `mFailed` interaction
...
-- STANDPOINT.  This is the report of the `Rewriter` naya â” the machine's own
...
-- à•ààµà¾àà¿ à¨ ààà¨àà¯àà‹à§à â” nowhere a bare truth-value.  Renamed and re-typed
-- 2026-08-18 after reading `formal/cubical/Anekanta.agda`, which does this
-- properly: à¨à•à¾à°à à–ààà¡à¨à à¦à¦à¾àà¿, àààµàà•à¾à°à àà¾à•ààà¿àà®à à  Previously three of the
-- four constructors were bare labels, which is the durnaya this whole file
-- exists to remove â” a verdict carrying no evidence of why it is the verdict.
...
data Verdict
  = Aviruddha [[Integer]]   -- ààµà¿à°àà¦àà§ â” unrefuted, CARRYING THE DOMAIN
  ...
  | Khandita [Integer]  -- à–ààà¡à¿à â” refuted, with the assignment that kills it.
                   -- A refutation is exact: one disagreement is a proof.
  | Nirdharmin (Int, Int)  -- à¨à¿à°àà§à°àà®à¿à¨à â” no subject to predicate of, carrying
                   -- WHICH two distinct variables made it subjectless.
  | Tusnim String  -- ààààààà®à â” this naya declines to speak, carrying the
                   -- symbol outside its semantics that silenced it.  SPLIT OUT
                   -- 2026-08-18.  Previously this returned `Plausible` with
                   -- the comment "no opinion offered" â” i.e. silence and
...
-- every log line unreadable and â” worse â” makes the next author delete the
...
-- in the engine and as in Agda's â•.
...
-- re-tested against an EXHAUSTIVE sweep of 0..12 in three variables â”
-- 13Â³ = 2197 assignments, 28— more work â” and **all 78 survived that too.
...
-- à¨à•à¾à°à à–ààà¡à¨à à¦à¦à¾àà¿, àààµàà•à¾à°à àà¾à•ààà¿àà®à â” à•ààµà¾àà¿ à¨ ààà¨àà¯àà‹à§à à
...
triage (l, r)
  ...
  | otherwise = case [ e | e <- searched, evalT e l /= evalT e r ] of
      (e:_) -> Khandita e
      -- ààµà¿à°àà¦àà§ carries the domain actually searched.  That is the yogya
...
-- ààà¿ààà¾.  àà•à¾à¨ààà àà¿ààà¾ â” one-sidedness is violence, and this was the last
...
-- without its evidence â” and then this function took the witnessed verdict and
...
-- à¨à•à¾à°à à–ààà¡à¨à à¦à¦à¾àà¿, àààµàà•à¾à°à àà¾à•ààà¿àà®à applies to admission exactly as it
-- applies to judgement.
data Pravesha
  = Pravishati Verdict  -- ààà°àµà¿ààà¿ â” it enters, carried in by THIS verdict
  | Nivartate  Verdict  -- à¨à¿àµà°àààà â” it turns back, turned by THIS verdict
  deriving (Eq)
...
-- the residual (in Agda's).  So the log is a bipartite graph goal â’ residual,
...
--
--     unblocks 14   0 = yÂ0
--     unblocks 11   xÂle(x,0) = 0
--     unblocks  8   xÂ0 = 0
--     unblocks  5   x = x+0
--
-- The machine ALREADY HAS `xÂ0 = 0`.  It is a defining equation of `*` in
-- its own vocabulary (`symDefs`).  It is demanding something it knows.
--
-- The reason is that Agda's `_Â_` on â• recurses on the other argument, so
-- `x Â 0` is not definitionally `0` there â” it needs induction â” while the
...
-- kernel â” which is the thing that has been capping accepted theorems all
-- along.
...
--
-- SOURCES.  Umsvti, Tattvrthastra (c. 2ndâ“5th c. CE) 1.6
...
-- Akalaka, Laghyastraya (c. 720â“780 CE) for the argument that the number
...
--     KERNEL-REJECT  x = (xmaxx)  ... refl has type x â‰¡ max x x
...
-- paragraph already found one level of that collapse (Bool â’ three
...
--      43  refl
--
--   asti       â” some naya in the log AFFIRMS the claim (an ACCEPT line).
--   nsti      â” some naya in the log DENIES it (a REJECT line; kernel-refl
--                denied), or the rewriter refutes it by evaluation.
--   avaktavya  â” the kernel's answer is not expressible as a standpointed
--                predication at all: Agda's message yields no term in the
--                language, so no `Vacana` is even formable.  This is the
--                Â§5 notion of Saptabhangi.agda in its degenerate case, and
...
-- Saptabhag presupposes a dharma predicated of a dharmin â” a property of
...
data Evidence = Evidence
  ...
  , evSakshin    :: Maybe String       -- àà¾à•ààà¿à¨à: the ACCEPT line that affirms
...
--
--   * over REJECTION LINES        â” every position is reachable
--   * over DISTINCT RESIDUALS     â” `avaktavya` is 0 BY CONSTRUCTION, since
...
-- row â” a table with one line per line.  So the tally groups by KIND.  The
-- first version of this did it by rebuilding the constructors with empty
-- payloads, which printed `Aviruddha over NOTHING` and `Khandita at []` â”
...
selfTest = do
  ...
  where
    ...
    -- the sevenfold's ONE new axis â” asti, "some naya affirmed this" â” is
    ...
    sap =
      ...
      , ( "nasti only: no accept line anywhere for this claim"
        ...
        -- The kernel answer quoted below mentions a section `cong (x Â_)`,
...
-- àà¾à•ààà¿à¨à â” the affirming witness, or nothing.  Not a Bool: if a naya
