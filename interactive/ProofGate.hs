-- ProofGate.hs â” the Agda emitter for the kernel gate.
--
-- LEGACY POINTER (2026-08-23).  MathMachine.hs â” "the CLOSE-lane
-- â•-equation spinner", 239 rounds / ZERO theorems installed â” was RETIRED
...
--
--     candidate : (x y z u v w : â•) â’ LHS â‰¡ RHS
...
--       library â” it is the NaturalMachine lane, and it carries
...
--
--       i.e. the gate rejects `0 + x â‰¡ x`.  Zero KERNEL-ACCEPTs was never
...
--       `agdaTerm` knew only 0, s, + and Â â” four of the eight symbols in
--       `vocabulary` â” and the only proof shape was `refl`, so nothing
...
--   *        Agda.Builtin.Nat._*_  via Cubical.Data.Nat     (see note A)
--   -        Agda.Builtin.Nat._-_ = _âˆ_ via Cubical.Data.Nat
--            machine: -x0=x, -0x=0, -(sx)(sy)=-xy
--            builtin:  n-zero=n, zero-suc m=zero, suc n-suc m=n-m
--            â” same clauses, same order.  Exact match.
...
--            â• (both are the â‰¡-unique greatest common divisor, gcd 0 0 = 0),
...
--
-- Note A.  Agda's `+` and `Â` recurse on the FIRST argument; MathMachine's
-- symDefs recurse on the second (x+0=x, x+s y = s(x+y)).  These are the
-- same function on â• â” the machine's `symSem` is Haskell's (+) and (*),
-- and the cubical/builtin operations are ordinary â• addition and
-- multiplication â” so the certificate is about the right object.  The
...
-- (s x + y) = s (x + y) and (s x Â y) = y + x Â y refl, which the
...
-- always cheapest-first â” a plain `refl` module (one call), then the
...
-- call and be rejected, always â” the emitter emitted no induction module for
...
-- every note stripped â” which is exactly the shape a residual arrives in â”
...
-- module source is the complete input to agda â” it already encodes the
-- conclusion, the concept definitions, the vocabulary transcriptions
-- (`localMax`, `localLe`), the imports, and the proof skeleton â” so hashing
...
--     permanent verdict â” the failure mode this whole file was written to
...
-- label is not marked cached â” the run did do kernel work.
--
...
-- needs a lemma environment â” certificates emitted in dependency order,
-- with earlier theorems in scope for later ones â” not more step shapes.
...
-- with the single defining equation (p, cN x0 â¦ x_{ar-1}); so the Agda
-- transcription is `cN a0 â¦ a_{ar-1} = p[V i := a_i]`, which is what
...
-- anything â” including newlines.  Two consequences were reproduced against
-- this very file:
--
--   * a name containing "\n{-# OPTIONS --no-safe #-}\npostulate evil : â¦"
...
--     postulate is refused â” verdict `Rejected`); and
--
...
--     line, so the module agda checks is `candidate : (x : â•) â’ (suc x) â‰¡
...
--     left-hand side (GateAudit.hs:100â“109).  That is a soundness break, not
...
-- the enforcement FAILS CLOSED â” an offending name yields no module at all
-- (`Nothing` â’ `Untranslatable`), never an emitted module that agda is asked
...
-- next line â” the three positions become splice-proof rather than
-- splice-audited.
...
-- regex alone admits `where`, `module`, `postulate`, `data` â¦, which lex as
-- KEYWORDS rather than identifiers and so would still restructure the module
-- (they cannot make a false equation check â” `--safe` is untouched at line 1
-- and postulates are refused â” but a name that changes the module's grammar
...
-- whatever invented concepts the caller supplies.  Anything else â” the
-- eigenconstant #, an unsupplied concept â” returns Nothing, which the
...
-- x y z u v w, a local definition's parameters are a0 a1 â¦.
...
render defs nameOf = go
  where
    ...
    go (F "*" [a, b]) = infixAgda go "Â" a b
    go (F "-" [a, b]) = infixAgda go "âˆ" a b
...
-- rendered module goes through here â” `render`'s fallback clause, which then
...
preambleWith defs syms =
  ...
  where
    ...
    -- `lookupDef` above â” an ill-formed name cannot enter `closure`, hence
    -- cannot enter `used` â” and it is kept anyway, because this is where the
    ...
    emit d =
      case render defs defVar (defBody d) of
        ...
        Just body ->
          [ defName d ++ " : " ++ concat (replicate (defArity d) "â• â’ ") ++ "â•"
...
--
-- It was written twice â” here and in `canaryModule` â” and the two copies
...
preambleCore syms =
  ...
  -- _+_, _Â_ and _âˆ_ come in unconditionally even when the equation does
  -- not mention them: the step shapes in `stepShapes` build sections like
  -- (y +_) and (_Â k), and an operator that is not in scope makes the
  -- module fail to PARSE rather than to typecheck â” which burns an
  ...
  , "open import Cubical.Data.Nat using (â• ; zero ; suc ; _+_ ; _Â_ ; _âˆ_)"
  ]
  ...
  where
    ...
    -- and `zero âˆ n` are STUCK terms â” the first clause of `max` (max a zero)
    -- and of `_âˆ_` (n âˆ zero) blocks on a variable in the second argument, so
    ...
    -- in `citingStepShapes` discharge the goal with `sym (â¦)`.  This is the
    ...
    localMinusZeroL
      | need "-" =
          [ "minusZeroL : (n : â•) â’ zero âˆ n â‰¡ zero"
...
agdaCertificateWith defs eq@(l, r) = do
  ...
  pure $ unlines $
    ...
    ++ [ "candidate : " ++ telescope names ++ lhs ++ " â‰¡ " ++ rhs
       , "candidate " ++ unwords names ++ " = refl"
       ]

telescope :: [String] -> String
telescope [] = ""
telescope ns = "(" ++ unwords ns ++ " : â•) â’ "
...
agdaInductionCertificate defs eq@(l, r) v step = do
  ...
  if v `notElem` vs then Nothing else do
    ...
    let pre = preambleWith defs (equationSymbols eq)
        sig = "candidate : " ++ telescope names ++ lhs ++ " â‰¡ " ++ rhs
...
-- â•-semiring identities whose base case is itself a lemma (y â‰ y + zero
...
-- kernel checks under the candidate's own pragmas â” probed 2026-08-24: it
-- discharges (x + y) â‰¡ (y + x) (rc 0) and DENIES (x + y) â‰¡ (x Â y) (rc 1) â”
  ...
  pure $ unlines $
    ...
    ++ (if null names
          then [ "candidate : " ++ lhs ++ " â‰¡ " ++ rhs
               , "candidate = " ++ body ]
          else [ "module _ " ++ concatMap (\v -> "(" ++ v ++ " : â•) ") names ++ "where"
               , "  candidate : " ++ lhs ++ " â‰¡ " ++ rhs
               , "  candidate = " ++ body ])
...
-- it `solveâ•!` and re-exported it from the aggregator `Cubical.Tactics
-- .NatSolver`.  Only one import resolves per toolchain; the other fails with
-- "Failed to find source of module" â” an ordinary rejection, not an
-- environment fault â” so the search simply moves to the next shape.
...
-- common core.  A goal `sucµ Eâ â‰¡ sucµ Eâ` is closed by `(cong suc)µ` applied
-- to a proof of `Eâ â‰¡ Eâ`, so peeling lets the semiring solver reach a core
...
-- function â” ClauseOrder, certifyCert â” are untouched.
...
-- le theorem in the library (suc x âˆ suc x reduces to x âˆ x, so the goal
-- IS the hypothesis).
...
stepShapes ih ks =
  ...
  ++ concat
     [ [ ("cong (_+ " ++ k ++ ")", "cong (_+ " ++ k ++ ") (" ++ ih ++ ")")
       ...
       , ("cong (_Â " ++ k ++ ")", "cong (_Â " ++ k ++ ") (" ++ ih ++ ")")
       , ("cong (" ++ k ++ " Â_)", "cong (" ++ k ++ " Â_) (" ++ ih ++ ")")
       ]
     | k <- ks ]
...
-- budget for an ANNOTATED candidate â” one whose proof note names the
-- induction variable â” and it is unchanged by the fallback below.
kMaxAgdaCalls :: Int
kMaxAgdaCalls = 1
...
-- variable tried.  A false equation costs far less than this in practice â”
-- the base clause fails and `blamedLine` stops each variable's search at 2
-- calls â” but the bound is what is guaranteed and it is what is stated.
...
-- own .agda-lib names further roots (kernel, theorems/â¦).  Those roots are
-- read from that file at the moment of the run â” see `corpusIncludeRoots` â”
...
-- [], which is exactly today's behaviour â” the base root alone â” so nothing
...
-- root, as the engine uses).  LC_ALL is forced because agda prints Î» and
-- dies under the C locale.
--
-- The reader's own locale is forced too.  Agda's diagnostics contain â• and
...
-- child's exit status untouched, and `vetSuccess` â” the falsifier watch of
-- 2026-08-16 â” was reached from exactly ONE caller, `runAgdaCached`.  So the
...
-- are the controls themselves â” `kernelStatus`, which would otherwise
-- recurse â” and `GateAudit`'s PROBE-RAW, whose whole purpose is to print
...
-- number a process returned and is not evidence about mathematics â” see
...
-- what it computes it to and what it is â” every bit it derives, not a green.
--
...
-- returning each BODY (unescaped) whose TITLE is a real answer â” dropping the
...
--   * `canaryFalse` MUST NOT.  It is `suc x â‰¡ x` closed by `refl`.  A
...
canaryTrue :: String
canaryTrue = canaryModule "(zero + x) â‰¡ x"
...
canaryFalse :: String
canaryFalse = canaryModule "(suc x) â‰¡ x"
...
--
--     the kernel accepted `suc x â‰¡ x` by refl.  It is not checking proofs
--
...
-- POSITIVE control was the thing that failed â” for an `[InfectiveImport]`
...
data KernelStatus
  ...
  | KernelPositiveControlFailed String
    ...
    --   whether the kernel is honest â” it says this container cannot compile
  ...
  | KernelNegativeControlInconclusive String
    ...
    --   module â” a missing include root, a timeout, a killed process.  The
      ...
      status <-
        if not positiveHolds
          then pure (KernelPositiveControlFailed posOut)
            ...
            -- `1 != 0` â” the falsifier, not merely the absence of a success.
...
-- run says "Checking Candidate (â¦)" and nothing else.
...
-- carries no mathematical content â” a missing library, a locale fault, a
...
-- unfit, is a verdict asserting itself while concealing its standpoint â”
-- à¦àà°àà¨à¯à‹ ààµàà¿").  The older Mms name for the requirement is
...
-- c. 7th c.; stra Â§19, "à¯ààà° à¦àààà¯àà àààà° à¨ à¦ààààŸà®à ààà¿ ààà°à®à¾àà®à").  A
  ...
  | otherwise = do
      ...
      pure $ case status of
        ...
        KernelPositiveControlFailed posOut ->
          ( ExitFailure 126
          , kEnvironmentFault
              ++ ": the POSITIVE control did not check.  `(zero + x) â‰¡ x` by\n"
        ...
        KernelAcceptedAFalsehood ->
          ...
          , kEnvironmentFault
              ...
              ++ "`suc x â‰¡ x` by refl, so it is not checking proofs and\n"
              ++ "nothing it accepts is one.\n" )
...
-- have in hand â” the code and the captured output â” rather than asking them
...
--      could not recognise `suc x != x` as a located type error â” so the
...
-- actually launched â” 0 when every module in the search came from the
...
certifyWith defs root (eq, proofNote) =
  ...
  where
    ...
    -- â•-semiring class (directly, or under k leading `suc`s via the peel
    ...
    -- RESIDUAL â” a subgoal harvested from the kernel's own stall and asked
    -- back â” has no such note, and the `Nothing` branch that used to stand
    ...
    -- reaches (interactive/ResiduePariksa_...hs, and Â§9 of
    -- variable in turn moves THREE of the six from open to certified with the
    -- shape menu completely unchanged â”
    --
    ...
    --
    -- THIS IS NOT THE PROOF SEARCH Â§3a OF CERTIFICATE_REACH.md RULES OUT.  That
...
-- not the same as re-checking a fixed proof â” a change to `stepShapes` could
...
-- term), so:
--
--   (a) it RECONSTRUCTS THE PROOF TERM deterministically â” `reconstructModule`
--       emits the one Agda module that carries the accepted proof, with no
--       search; and
--   (b) it ROUND-TRIPS â” `serializeCert` / `parseSerialCert` are inverses on
...
-- denote pointwise-equal functions â• â’ â•.
...
-- ledger re-checkable â” it trusts the witness, never re-searches.
...
-- (MathMachine.hs ~465): variables are x y z u v w / nâ, `0` and other
-- nullary symbols are bare, `+ * ^ gcd max` are infix `(a op b)`, and every
-- other symbol (`s`, `-`, `le`, invented concepts `c0`â¦) is prefix
-- `f(a,b,â¦)`.  This parser is the exact inverse of that `show`, so a line the
...
-- failed on EVERY `max` line the machine has ever written â” four of the 28
-- lines of `interactive/library.snapshot.txt` â” and failed by returning "this
...
-- machine's out-of-range spelling nâ; otherwise a nullary symbol like `0`.
atom :: String -> Term
atom nm
