...
-- reactor whose law is welded in: `composition` hard-codes N(x,y) = xÂ² âˆ’ D yÂ²,
-- `chooseM` hard-codes the interpolator (m, 1, mÂ² âˆ’ D), and every path
...
-- Brahmagupta's; it is xÂ² âˆ’ D yÂ² = 1 read back onto him.
--
...
-- past N = âˆ’1 by squaring, so what it reports is the fundamental solution
-- of xÂ² âˆ’ D yÂ² = 1 and never the fundamental UNIT.  Those differ by a
-- square exactly when the norm âˆ’1 equation is solvable, which is most of
...
-- Îµ: the first element the wheel reaches with |N| = 1.
fundamentalUnit :: Eq v => Law v -> Either Defect v
fundamentalUnit law = last <$> reactor law

-- and the norm-+1 element, which is Îµ when N(Îµ) = 1 and ÎµÂ² when N(Îµ) = âˆ’1.
-- This is BRAHMAGUPTA's own bridge (Brhmasphuasiddhnta 18): a norm âˆ’1
-- solution composed with itself has norm (âˆ’1)(âˆ’1) = 1.  It is checked, not
...
-- Brahmagupta's arbitrary-kepa claim rather than the N = 1 special case.
...
-- Î” = 436 the answer has sixteen digits and a Double would be silently
...
-- silent collapse the stra names, so it is made impossible here.
...
vargaPrakrti t c
  | disc <= 0 = Left (Defect "leg 3 (descent)"
      "Î” = TÂ² + 4C is not positive: the order is imaginary and its unit \
  ...
  where
    disc = t * t + 4 * c
    inst = "T = " ++ show t ++ ", C = " ++ show c ++ ", Î” = " ++ show disc
    ...
    law = Law
      { lawName = "varga-prakti  Ï‰Â² = " ++ show t ++ "Ï‰ + " ++ show c
                  ++ "   (Î” = " ++ show disc ++ ")"
    ...
    descendWith w q@(Quad a b) = do
      ...
      mt <- case inverseMod b n of
        Nothing -> Left (Defect
          ("leg 3 (descent), Î” = " ++ show disc)
          ("kuaka: gcd(b, |k|) = " ++ show (gcd b n)
            ++ " â‰  1, so the congruence k | (a + b(m+T)) has no solution class")
          (show q ++ "   k = " ++ show k))
      ...
      m <- if null cands
             then Left (Defect ("leg 3 (descent), Î” = " ++ show disc)
      ...
      if nrm q' /= k'
        then Left (Defect ("leg 3 (descent), Î” = " ++ show disc)
               ("the step did not preserve the form: N(step) = "
                 ++ show (nrm q') ++ " but (mÂ² + Tm âˆ’ C)/k = " ++ show k')
...
maximalOrderLaw disc = case disc `mod` 4 of
  ...
  r -> Left (Defect "leg 3 (descent)"
         ("Î” â‰¡ " ++ show r ++ " (mod 4) is not the discriminant of any \
...
--                analogue of Bhskara's congruence would give is (âˆ’1, âˆ’1),
...
selfTest = do
  ...
  putStrLn "     D       Îµ = fundamental unit                      N(Îµ)   \
           \norm-one solution"
  ...
  where
    ...
    -- xÂ² âˆ’ D yÂ² = 1.  D = 61 is Bhskara's own worked example.
    classical =
...
widen l = l { lawDescend = wideDescend }
  where
    wideDescend q =
      -- re-derive T and C from the norm form: N(1,0)=1, N(0,1)=âˆ’C, N(1,1)=1+Tâˆ’C
...
vargaPrakrtiWide t c w
  | disc <= 0 || isSquare disc =
      Left (Defect "leg 3" "Î” not a positive non-square"
              ("T = " ++ show t ++ ", C = " ++ show c))
  | otherwise = Right go
  where
    disc = t * t + 4 * c
    ...
    go q@(Quad a b) = do
      let k = nrm q
          n = abs k
      bi <- maybe (Left (Defect "leg 3" "kuaka: no solution class" (show q)))
