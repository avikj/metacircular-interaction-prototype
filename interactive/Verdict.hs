...
--   Akalanka, Laghiyastraya / Astasati, c. 720â“780 CE â” order (sequential)
...
--   Mallisena, Syadvadamanjari, 1292 CE â” wholedesa (total statement,
...
-- collapse to make (AHIMSA_SUTRA_VISTARA Â§7: it is not forbidden, it does
...
--
-- defect, and no third.  `order` is the transport â” nothing is lost, and
...
data Bhanga
  ...
  | SyadAstiNasti            -- ààà¯à¾à¦à ààààà¿ à à¨à¾àààà¿ à        (order)
...
-- still says.  So order IS the join of profiles â” transport, not collapse â”
...
order x y = forceVerdict (unprofile (samyoga (profile x) (profile y)))
  where
    ...
    forceVerdict Apratipatti  =
      error "order: unreachable (see ààà¯à‹à—-àà°à¿à•ààà®à, checked in Agda)"
...
-- The order order.  PARTIAL â” `nyuna SyadAsti SyadNasti` and
...
selfTest =
  [ ("order associative  (à•àà°à®-àà™àà—àà¿à)",
      and [ order (order x y) z == order x (order y z) | x <- bs, y <- bs, z <- bs ])
  , ("order commutative  (à•àà°à®-àµà¿à¨à¿à®à¯à)",
      and [ order x y == order y x | x <- bs, y <- bs ])
  , ("order idempotent   (à•àà°à®-àààµà¯à®à)",
      and [ order x x == x | x <- bs ])
  , ("order has a top    (àà¿à–à°à®à)",
      and [ nyuna x SyadAstiNastiAvaktavya | x <- bs ])
  , ("order of asti,nasti is the THIRD  (à•àà°à®àà-à‰àà¯à®à)",
...
-- than silently reconciled (AHIMSA_SUTRA_VISTARA Â§6: transport, or a WRITTEN
