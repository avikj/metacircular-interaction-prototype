...
--
--   1. POSITION IS PART OF MEANING.  1.4.2 àµà¿ààà°àà¿ààà§à àà°à à•à¾à°àà¯à®à
...
--      ààà°ààµààà°à¾àà¿à¦àà§à®à `prvatrsiddham`: from 8.2.1 on, each rule is
...
--      Â§à© of the same note continues: ààµà•àààµàà¯à àààà‹ àµààà¿ à àààà‹ à—à°ààà, à¨
--      àµà¿àà²àà¾ à à—à°ààà¾à¦à àà—àà°à¿à®à‹ à¨à¯à‹ àà¾à¯àà -- in the avaktavya the residue
...
--      `Avaktavya` now carries a `Residue` as well: the object and the
...
--
--     ààà°ààµàà°à¨à¿ààà¯à¾à¨ààà°à™àà—à¾ààµà¾à¦à¾à¨à¾à®ààààà°à‹àààà°à àà²àà¯à
...
-- defines it à•ààà¾à•ààààà°àà™àà—à¿ à¨à¿ààà¯à®à `ktktaprasagi nityam` -- nitya is
...
-- and is allowed to say it does not know.  àà¨ààà°à™àà—à ààà¿à°à™àà—à¾àà -- the rule
...
showDosa d =
  ...
  , "  collapse (AHIMSA_SUTRA_VISTARA Â§à: transport or a written defect, no"
...
-- THE RESIDUE.  ààà -- what the fourth position holds.  `Dosa` above is the
...
-- AHIMSA_SUTRA_VISTARA Â§à©: ààµà•àààµàà¯à àààà‹ àµààà¿ à àààà‹ à—à°ààà, à¨ àµà¿àà²àà¾ à
-- Everything born from an avaktavya in
-- `interactive/AvaktavyaPrasava_TheFourthPositionBearsTheRuleThatDecidesIt.hs`
-- is computed from this record, and none of it is computable from `Dosa`.
data Residue o = Residue
  { residueVastu  :: o          -- the object both standpoints are speaking about
  , residueNyasah :: [Nyasa o]  -- the contending offers, entire
  }

data Nirnaya o
  = Nirnita (Nyasa o) [Sthana] Balya   -- decided: winner, beaten, by which metarule
  | Avaktavya Dosa (Residue o)            -- undecided: the fourth position, written
                                       -- AND retained (see Residue)
  ...
  where
    ...
    tryPara acc =
      ...
      in case (tParatva t, winners) of
           ...
           _           -> flip Avaktavya (Residue o xs) Dosa
