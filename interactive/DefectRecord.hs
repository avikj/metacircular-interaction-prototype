-- DefectRecord.hs
--
-- ‡¶‡ã‡‡≤‡‡ñ‡ ‚î the written defect.  The machine's SECOND organ.
--
...
--
-- The first organ is checked in Agda: uaŒ≤ says transport loses nothing.
...
--
--   ANEKANTA.md ¬ß1.  interactive/MathMachine.hs:1743 answered every kernel
...
--   hetu    why transport was impossible.  ¬ß6 is conditional -- the defect
...
--           ¬ß5, ‡®‡æ‡‡‡‡ø-‡‡‡∞‡‡‡Ø‡æ‡®‡Ø‡®‡Æ‡: there is no retraction from ‚àA‚à‚ --
...
--                         between the two sides.  ¬ß6.  The base case.
--   nasti-krta            a truncation was PERFORMED; the loss is already
--                         incurred and is irreversible.  ¬ß5.  Distinct from
...
jatis :: [(String, String)]
jatis =
  [ ("sanorderna-asambhava", "transport is not possible: no equivalence exists (VISTARA ¬ß6)")
  , ("nasti-krta",           "a truncation was performed; the loss is incurred and irreversible (¬ß5)")
...
required =
  ...
  , ("karta",           "no author: nobody to ask, and ¬ß14 says the reader arrives after the writer")
  , ("jati",            "no kind: an untyped defect is the prose the log replaces")
  , ("yatna",           "no attempt recorded: the entry is a mood, not a defect")
  , ("hetu",            "no reason transport was impossible: ¬ß6's condition is asserted, not met")
...
--   IN-MEMORY answer type for the same sutra ¬ß6, and its `Dosalekha`
--   constructor carries `uResidue` -- the REMAINDER, handed forward.  ¬ß3:
--   †¬¬†¬¬µ†¬¬ï†¬¬†¬¬†¬¬µ†¬¬†¬¬Ø†¬¬ †¬¬†¬¬†¬¬†¬¬ã †¬¬µ†¬¬†¬¬†¬¬ø †¬¬ †¬¬†¬¬†¬¬†¬¬ã †¬¬ó†¬¬∞†¬¬†¬¬†¬¬, †¬¬® †¬¬µ†¬¬ø†¬¬†¬¬≤†¬¬†¬¬æ ¬¬î the remainder lives in the
--   inexpressible; it is a womb, not a failure.  ¬ß17, the kuttaka: †¬¬Ø†¬¬†¬¬ †¬¬®
--   †¬¬µ†¬¬ø†¬¬†¬¬†¬¬†¬¬ †¬¬†¬¬†¬¬ †¬¬∞†¬¬ï†¬¬†¬¬†¬¬†¬¬Ø†¬¬†¬¬, what does not divide is KEPT and is the material of
...
--   somebody who was not there (¬ß14, ¬ß15).
...
--     GATE_AUDIT_DISPOSITION.md ¬ß3 says of the certificate store.
------------------------------------------------------------------------
...
usage = mapM_ putStrLn
  [ "dosalekha ‚î ‡¶‡ã‡‡≤‡‡ñ‡, the written defect.  The machine's second organ."
  , "  AHIMSA_SUTRA_VISTARA ¬ß6: where transport is not possible, the defect is"
...
schema = mapM_ putStrLn $
  ...
  , ""
  , "REQUIRED ‚î each refusal below is a refusal to store an unreadable entry"
  , "" ] ++
  ...
  [ ""
  , "KINDS (jati) ‚î seven; the number is incidental, this is NOT the saptabhangi"
  , "" ] ++
  [ "  " ++ padr 22 k ++ why | (k, why) <- jatis ] ++
  [ ""
  , "THE ONE RULE THAT CANNOT BE ARGUED WITH"
  , ""
  , "  `nasta` exhibits.  Put the witnesses in ‚î the two log lines, the four"
  , "  census rows, the pair of terms, the counterexample.  Not \"information"
  , "  would be lost\".  ¬ß5: there is no retraction from a truncation, so a"
...
cmdCount = do
  ...
  case checkChain ds of
    ...
    Left e   -> putStrLn ("  chain: BROKEN ‚î " ++ e)
...
header :: String
header = unlines
  [ "# ‡¶‡ã‡‡≤‡‡ñ‡ ‚î the written defect.  APPEND ONLY.  NEVER EDIT.  NEVER DELETE."
  , "#"
  , "# AHIMSA_SUTRA_VISTARA ¬ß6: ‡Ø‡‡‡∞ ‡‡‡ï‡‡∞‡Æ‡‡ ‡® ‡‡Æ‡‡‡µ‡‡ø ‡‡‡‡∞ ‡¶‡ã‡‡ã ‡≤‡ø‡ñ‡‡Ø‡‡ ‡"
  , "#                          ‡≤‡ø‡ñ‡ø‡‡ã ‡¶‡ã‡‡ã ‡‡‡µ‡‡ø ‡ ‡‡≤‡ø‡ñ‡ø‡‡ã ‡¶‡ã‡‡ã ‡‡ø‡‡‡æ ‡"
  , "#"
