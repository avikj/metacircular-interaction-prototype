#!/usr/bin/env python3
"""Drive the machine over its wire.  usage: machine_client.py <requests.jsonl>
Each request line is sent verbatim; each answer is summarised (kind, position,
route, first defect line) and the full answer appended to machine_answers.jsonl."""
import json, subprocess, sys, os
ROOT = '/home/user/metacircular-interaction-prototype'
S = os.path.dirname(os.path.abspath(__file__))
reqs = [l for l in open(sys.argv[1]).read().split('\n') if l.strip()]
env = dict(os.environ, MATH_AGDA_TIMEOUT='3000', PATH=S + '/bin:' + os.environ['PATH'], YANTRA_OUT=S + '/machine_out', DOSA_LEKHA=S + '/machine_session.lekha')
p = subprocess.Popen(['sh', 'interactive/run-machine.sh', '--wire'], cwd=ROOT, env=env,
                     stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
out = open(S + '/machine_answers.jsonl', 'a')
for r in reqs:
    p.stdin.write(r + '\n'); p.stdin.flush()
    ...
    if kind == 'dosalekha':
        ...
        for s in u.get('residue', [])[:2]:
            print('   residue:', str(s)[:200])
    else:
        print('   saksin:', str(pr.get('saksin'))[:300])
        vs = u.get('vahita', {})
        if isinstance(vs, dict):
            for k, v in vs.items():
                if k in ('padani', 'vislesana', 'phala', 'residueh'):
