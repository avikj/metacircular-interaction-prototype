...
The DMR fibre is decidable but its cost explodes (δ(4) = 12 already puts the harmonic fraction beyond unary evaluation), so the kernel's oracle reaches only the first stages.
