...
```sh
OUT="$(mktemp -d "${TMPDIR:-/tmp}/mip-machine-smoke.XXXXXX")"
...
sh interactive/run-machine.sh --wire < /path/to/handoff/infra/smoke_requests.jsonl \
...
1. `machine.kriyah` to read the live capabilities, not infer them from documentation;
...
4. `machine.sthiti` to preserve session output.
...
No command in this handoff has been executed against Machine/Agda here. The requested receiving agents have the infrastructure and must record their own commands and evidence.
