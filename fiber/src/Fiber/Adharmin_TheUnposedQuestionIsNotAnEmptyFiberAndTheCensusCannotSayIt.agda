...
-- Punargamana � ��������
--
-- ������� � the property-bearer, the SUBJECT of which something is
-- predicated.  Nyya's term, and the thing an inference must have before
-- it can have a ����� at all.  �������� / �����������, as used here, is not a
...
-- back-attributed: the Nyya debt is �������; the negated compound and
...
-- `isContr` was merging ������ with ������.  That was right.
--
...
--
--     "Absence of an ACCEPT line is NOT recorded as nsti.  The log
...
-- it: grepping `formal/cubical/` for ADharmin / ����������� returns three
-- hits, all of them prose in headers.  `Nirnaya_TheVerdictCannotDropIts
-- Witness` names ����������� in its own vocabulary list and then defines
-- `���������� c = ������� c � ������ c` � two outcomes.  The verdict that
-- cannot drop its witness drops this one.
--
-- §2 is the theorem that makes the fourth outcome load-bearing rather
-- than decorative, and it is `Saptabhangi.�������` one notch wider:
...
--
-- WHAT IS NOT DONE.  `Saptabhangi.�������` is NOT imported and NOT
...
-- over four names, and the relation to ������� is a claim about two
...
--
-- Nirnaya's header carries a FIFTH name, �������� � "the symbol outside
...
module Fiber.Adharmin_TheUnposedQuestionIsNotAnEmptyFiberAndTheCensusCannotSayIt where
...
------------------------------------------------------------------------
-- 1.  ������ and ����� � the fourth outcome, one level up.
--
-- A ������ is a question that may not have been posed.  `�����` is what
...
स्थान         nothing  = Unit*       -- अधर्मिन् : no subject; no fiber to take
...
------------------------------------------------------------------------
-- 2.  �����-������� � four names into two values must merge two.
--
-- `Saptabhangi.�������` proves this for the three seeds asti / nsti /
...
data चतुष्कम् : Type₀ where
  ...
  सकलादेश-पदम्  : चतुष्कम्   -- contractible fiber: nothing lost
  विकलादेश-पदम् : चतुष्कम्   -- crowded fiber: the loss
  नास्ति-पदम् : चतुष्कम्   -- empty fiber: not sayable, and NOT a loss
...
-- �������� is not ������ � the unposed question is not the empty fiber,
