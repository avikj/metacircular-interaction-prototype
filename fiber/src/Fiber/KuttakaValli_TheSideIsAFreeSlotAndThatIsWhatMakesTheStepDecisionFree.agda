...
--
-- SOURCE.  ryabhaa, *ryabhaya*, �������� 32�33, 499 CE � the ����������,
-- whose instruction is "���� �����", keep the remainder, and whose result
-- is read back by climbing the ����� (the creeper, the column of
-- quotients).  The three slots named here � ������ (side), ��������
-- (magnitude), ���� (remainder) � are the quantities that survive one
...
-- the ����� to the next, and dropping any one of them is what breaks it.
--
------------------------------------------------------------------------
...
-- three theorems, `������-�-����������`, `��������-�-�����������`,
-- `����-�-����������`, each exhibiting two ������� that agree on two slots,
...
--
-- What IS determined is the PAIR.  ������� : ������� � � � � reads the pair
-- (a , b) back out of the record, and the pair is a function of the
-- record.  So the instance is
--
--     ���������� = Carrier ������� ,     base = ������� ,     carried = � � �.
--
-- and the fiber �[ p ∈ � � � ] (������� t ≡ p) = singl (������� t) is
-- contractible, so ������� � ���������� and, by univalence, ������� ≡ ����������.
--
-- WHY THIS IS THE INTERESTING DIRECTION.  `�����`, the step, is a total
-- function ������� � ������� written with NO comparison, NO `Dec`, NO `Bool`
-- � because ������ has already recorded which side the remainder fell on.
-- The subtractive Euclidean step needs a decision only for a base that
-- has thrown that slot away.  `�����-���` / `�����-�������` below prove that
...
-- ������� (��� a b) ≡ (a , b)) is the same statement as `�����������` in
...
--
-- SECOND DEFECT.  `�����` is the SUBTRACTIVE step (anthyphairesis), not
...
-- below that says "�����" means the subtractive column.
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- ������ � the side, as a type of its own, so that "which side" is a
...
------------------------------------------------------------------------
-- ������� � the three slots, one record.  ������ is the constructor,
-- �������� is d, ���� is the excess.  �� carries no excess because there
...
-- ������� is not `Carrier g` for any g out of a two-slot base, because the
-- fiber of the forgetful map is not contractible � it has two points that
-- are not joined.
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- ������� � the pair IS a function of the record.  This is the f of the
-- law.  a = d + ���� on the ��� side, b = d + ���� on the ������� side,
-- a = b = d at ��.
------------------------------------------------------------------------
...
-- the four coordinates of a ����������: three from the base, one carried
...
------------------------------------------------------------------------
-- ��� � the descent that manufactures the record from a pair.  Decisionless:
...
-- the round trip.  See DEFECT in the header: this is `�����������` of
...
--
-- Written with no comparison, because ������ already says which way to
-- subtract.  At �� the algorithm has terminated and the step is the
...
-- The magnitude at �� is the common value of the pair, and it is fixed
...
-- because � has eta and `descend` does not pattern match.  Neither the
