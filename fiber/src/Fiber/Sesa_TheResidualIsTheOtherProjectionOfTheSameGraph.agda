...
-- Punargamana � ���
--
-- ��� (ea) � the remainder that is KEPT and made the material of the
-- next step, rather than discarded.  ryabhaa, *ryabhaya*,
-- Gaitapda 32�33 (499 CE), where it is the kuaka's governing move
...
--     Γ f  =  �[ a ∈ A ] �[ b ∈ B ] (f a ≡ b)
--
-- which is `Carrier f` (§2, `������`, by nothing more than reassociating the
-- �).  Γ f has two projections and they behave completely differently:
--
--   ���-��������� : Γ f � A     fiber over a is  singl (f a)  � ALWAYS
--                              contractible, so this is ALWAYS an
--                              equivalence.  (§3)
--
--   �������-��������� : Γ f � B    fiber over b is  ��� b  � contractible
--                              exactly when f is an equivalence.  (§4)
--
-- So the two halves of the design law are the two projections of one �:
--
...
--
-- The sentence is a ������ and `Saptabhangi.�������` is the proof of why:
-- a two-valued verdict on a threefold situation must identify two of the
-- three.  `isContr (��� f b)` fails in two OPPOSITE ways �
--
--   the fiber is EMPTY   � over `b` there is no source: syd ������.
--                          Nothing was destroyed (that is the crowded arm),
--                          so this end is ����������, POSITIVE.  (NOT
--                          �����������: that is the fourth bhaga, yugapat
--                          inexpressibility, earned only where a krama-pair
--                          recovers it � SaptabhangiNaya � not asserted of
--                          every empty fiber.)
--   the fiber is CROWDED � two points not identified.  ������, �����,
--                          �������������.
--
...
-- diagnosis is a CENSUS � a function `B � ��� f b` whose constructors
...
--
-- What survives untouched: §2's `������`, and the observation that `Carrier`
...
module _ {A B : Type ℓ} (f : A → B) where
  ...
  ------------------------------------------------------------------------
  -- 1.  ��� � the residual over a target point.
  --
  ...
  ------------------------------------------------------------------------
  -- 2.  ������ � one graph, two orders of the same �.
  --
  -- `Carrier-as-�` reads a Carrier as a source point paired with a point
  ...
  -- are refl: this is �-eta and nothing else, which is precisely why the
  ...
  ------------------------------------------------------------------------
  -- 3.  ���-��������� � the source projection.  ALWAYS an equivalence.
  --
  ...
  ------------------------------------------------------------------------
  -- 4.  �������-��������� � the target projection.  An equivalence exactly when
  ...
  -- fiber is contractible", so "f loses nothing" and "f is an
...
-- to its source (�����) and provably not to its target (�-��������).  The
