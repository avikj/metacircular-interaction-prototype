...
--
-- Two of those three fields look like new data and are not.  The fiber
--
--   �[ b ∈ B ] (f a ≡ b)   =   singl (f a)
--
...
--
-- THE CONVERSE, which is the part that does work.  A NON-contractible fiber
-- cannot be declared equivalent to its base.  Contractibility of the fiber is
-- the hypothesis, not a formality � `fiber-isContr` is what `�-law` consumes,
...
module _ {A B : Type ℓ} (f : A → B) where
  ...
  -- ���������� over a is singl (f a) � and it is contractible.  Everything
  ...
  --
  --   Cubical.Foundations.Equiv.Base   fiber f y = �[ x ∈ A ] (f x ≡ y)
  --   here                             ����������   a = �[ b ∈ B ] (f a ≡ b)
  --
  -- Cubical's is a � over the DOMAIN � what f collapses onto y, i.e. the
  -- loss.  This one is a � over the CODOMAIN � where f a is free to sit,
