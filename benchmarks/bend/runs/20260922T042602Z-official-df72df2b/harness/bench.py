...
def tool_inventory(tools):
    ...
    for name, path in tools.items():
        if not path.is_file():
            ...
            continue
        entry = {'path': str(path), 'resolved_path': str(path.resolve()), 'sha256': digest(path)}
        try:
            proc = subprocess.run([str(path), 'version' if name == 'isabelle' else '--version'],
                                  capture_output=True, text=True, timeout=30)
            entry.update(version=(proc.stdout + proc.stderr).strip(), returncode=proc.returncode)
...
def main():
    ...
    for _ in range(args.repetitions):
        ...
        manifest['tool_inventory'] = tool_inventory(tools)
