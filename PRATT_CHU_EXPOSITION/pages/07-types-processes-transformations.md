# Types Are Processes — Transformations Are Executable
...
Univalence is decisive. An equivalence `A ≃ B` becomes a path `A =_U B`, and coercion along that path computes the equivalence. A theorem that two structured presentations are equivalent therefore does not end with an external correspondence: every dependent construction over one presentation can be transported to the other.

Dynamic reflection lifts this through continuing behavior. If `Φ : A → A` and `e : A ≃ A'`, conjugate dynamics `Φ' = e Φ e⁻¹` commute with the whole coinductive unfolding. The future does not need to be re-derived after a change of presentation; it transports.

Metacircularity closes the remaining meta/object split. An interaction can establish a typed transformation; that transformation remains an inhabitant of the same executable universe and can be installed as a subsequent operation. `IntrinsicRewrite`, self-presentation, and productive propagation make “the mathematics changes the mathematics” a checked lifecycle rather than a metaprogramming slogan.
