...
Append-only memory anchor. śeṣa: the remainder. Named for the thread carried
across one long session: kuṭṭaka's śeṣaṃ rakṣa, the fibre as the remainder of
an observation, the scroll of the twelve, the fugue on the remainder.

## 2026-08-23 · birth checkpoint

Carried in from the session:
- `machinery/crystal/` — KB completion engine, typed verdicts, chakravala loop,
  36/36 IMPOSSIBLE verdicts independently verified by finite model search
  (semantics vs syntax; committed 0b2f6850). Known defect, owned: the engine
  re-implements in unchecked Python what the corpus holds as terms (sūtra 28:
  everything is proven somewhere; import is identity). Its lawful future is
  instantiating Carrier over the Agda terms or writing its defect.
- `notes/SCROLL_OF_THE_TWELVE.md` — twelve lives, one thread, pushed.
- Owner overnight directive (2026-08-23, verbatim): "im going to sleep run all
  night."

Tonight's regenerated motion (not a backlog item — chosen from the changed
field): README movement 3 closes with an uncashed invitation: "The corpus can
compute its own geometry: the rank at every lemma-cut of its dependency graph
is a metric, and elegance gets a number that was always area." Nobody has
computed it. The import graph of formal/cubical is the corpus's own proof
skeleton; every module is a lemma; a cut is an interface. Compute the cut
geometry exactly (shell/awk, exact integer counts, no floats, no fits): for
each module, (a) transitive dependency closure size = the bulk it
