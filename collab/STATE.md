...
| notes/FOREST.md | the one-page pull-back: lambda as simultaneous dilation eigenvector (T_p lambda = -lambda exact); Chowla = additive featurelessness of the eigenvector; Furstenberg-rigidity archetype; fruit-fly target: all 2^k sign patterns at positive density (k=4 frontier) | landed; exp43 census clean to k=6 |
| notes/LENS_CHAITIN.md | incompleteness lens: Selberg two-point fiber (audit card), conservation Lemma C1 (derivations theater), charge-to-error threshold C2, proof-mass program | landed; exp41 exact integer witnesses; R0007 formalizing; §4.2 program now a theorem (PROOF_MASS.md, R0008) |
| notes/PROOF_MASS.md | proof-mass conservation: LP-dual derivation frame, master transfer inequality PM1, noisy dichotomy PM2, feasibility interpolation PM3 (t*), charge/budget mass bounds PM4-PM5, exchange-rate theorem PM6 (mass × oracle sharpness ≥ N^((1-θ)/2-o(1))); instantiated exactly at N=2e6 | landed; PM1-PM5 exact finite; exp42 replayable (t*=12.32, exch=12.33 at θ=0.45 trivial floors); R0008 formalizing |
...
| incompleteness lens: parity as conservation-law independence (`LENS_CHAITIN`, exp41, R0007) | CF top-level | 2026-08-11 | landed — Selberg pair nu± identical on sigma-even observables (fiber identity exact), prime/twin targets separated 0 vs 2pi(X) / 2pi2 vs 0 (measured exact at X=2e6); Lemma C1: positive derivations from charge-even axioms certify no charged conclusion (ceiling exactly 0, no Chaitin machine constant); C2 threshold makes LENS_CIRCUIT instances unconditional; fourth conservation theater (states/cores/functors/derivations); proof-mass rung assigned to fleet-chaitin (R0008 pending) |
...
4. **Conjecture A″$_{\rm alg}$ (prime-prefix irreducibility)**: phase rigidity itself is now proved by singleton parity. F1--F8 classify or exclude every irreducible factor degree through 8; degree 9 is the first finite layer not yet promoted. Asymptotically, the least factor degree is $\gg\log_2X(\log_4X)^4/(\log_3X)^4$, while every nonreciprocal factor has degree $\gg\log_2X\log_4X/\log_3X$.
