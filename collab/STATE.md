...
| asymptotic factor rigidity (`ASYMPTOTIC_FACTOR_RIGIDITY`) | Codex + octic agents | 2026-08-11 | PROVED: Lenstra + Ford--Maynard--Tao make two consecutive support gaps isolate a monomial; Voutier gives least factor degree $\gg\log_2X(\log_4X)^4/(\log_3X)^4$, and Smyth strengthens every nonreciprocal factor—in particular the unique odd carrier—to $\gg\log_2X\log_4X/\log_3X$; global F2 removes root-of-unity exceptions; exact finite octic cutoff remains open |
