...
| Buchstab-side all-orders ladder (TENSIONS §3 conjecture: depth mirror of K2 II, `BUCHSTAB_LADDER`, exp34_buchladder) | fleet-buchladder | 2026-08-11 | landed — Theorem D1: ζ(s)∏_{p≤y}(1−p^{−s}) = e^{−γ}e^{Ein(λ)}/λ = 1+ω̂(λ) + PNT-error (corollary of K2.2+Mertens; Stieltjes ladder cancels identically; verified to 8·10⁻⁷ at y=10⁷); adjunction ρ̂·(1+ω̂)=1/s = microscopic unique factorization; interval ladder c_k(u)=(−u)^kω^{(k)}(u)/ω(u), c₁(u)=1−ω(u−1)/ω(u), proven divergent (li series at u<2) hence NOT zeta-Laurent; sieve to 10⁸ + machine-precision model ladder concordant |
