# Typed path-endpoints for Ref-headed spines — the enabler for isPropIsEquiv
...
(uaβ, already *definitional* here) and (ii) a prop-filler on the isEquiv
...
`isPropIsContr(_, u0 y, u1 y) @ i0  ≡  u0 y`. This is a *typed* path-endpoint
...
**global definition** (a `Ref`) of Path type — so `isPropIsContr(...) @ i0`
stayed neutral and the endpoint check failed. This is the same wall that
Σ-η would clear, but the Ref-endpoint law is the more principled fix: it is
...
The existing `App`/`PAp` cases then peel the Π/Path spine as before, so
...
- `isPropIsEquiv`  — green, **definitional**  (equiv.bend)
- `pathToEquiv`    — typechecks               (equiv.bend)
- `uaβ`            — already definitional      (transport along `ua` = f)
- `isPropIsContr`  — green via general hcomp   (unchanged)

No regression: cubical suites, stock examples, and `isPropIsContr` unchanged.
The remaining reverse-round-trip assembly (`equivEq` / `isProp→PathP`) is
