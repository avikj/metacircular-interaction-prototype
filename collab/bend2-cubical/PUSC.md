...
That is the level at which the name should be understood. Its subject is the execution of a mathematical language: dependent types, functions, equivalences, paths, higher paths, inductive constructions, and continuing processesâ”not a machine that merely stores descriptions of these things or reports "relationships" between them.
...
Write sequential composition as gâˆ˜f, and write independent composition as fâŠ—g. Then the elementary interchange equation is:
...
A sequential schedule is therefore a traversal of the dependency structureâ”not necessarily the identity of the computation.
...
In one direction, an equivalence e : A â‰ B gives ua(e) : A =_ð’° B. In the other, a universe path gives an equivalence by transport. The cubical library constructs both directions and their round trips.

The decisive computer-science consequence is substitutability through dependent programs.

Given any definable family F : ð’° â’ ð’°, the equivalence induces:

    act_F(e) = transport(i â¦ F(ua(e)(i))) : F(A) â’ F(B).
...
For example, let F(A) = A â’ A. Then transporting an operation f : A â’ A has the action f â¦ e âˆ˜ f âˆ˜ eâ»Â. For a binary operation m : A â’ A â’ A, the transported operation has the action m'(x, y) = e(m(eâ»Âx, eâ»Ây)).
...
where Î©(ð’°, A) is the type of loops at A, and Aut(A) is the type of self-equivalences of A.
...
This is why a univalent computer is more than a system of conversion functions. It computes with the structure of those conversionsâ”including their compositions and higher agreements.

#### "Cubical" supplies the execution rules

A path is represented through a symbolic dimension and boundary conditions:

    p = âŸ¨iâŸ© t(i),    p(0) = a,  p(1) = b.
...
Ordinary lambda notation hides the operational significance of using something more than once: Î»x. h(x, x). The repeated occurrence invokes a structural operation: duplication. An affine presentation instead makes the split explicit.

IC introduces two dual forms: DUP_â“ and SUP_â“.
...
The core interactions form a systematic pattern:
    ...
    APP â‹ˆ LAM,    DUP â‹ˆ SUP,

together with the crossed interactions:

    APP â‹ˆ SUP,    DUP â‹ˆ LAM.
...
With the same label, SUP_â“(aâ, aâ) and SUP_â“(bâ, bâ) can be consumed as the correlated pairs (aâ, bâ), (aâ, bâ). Different labels retain independent branching dimensions.
...
A total one-step simulator can have the type step : Config â’ Result + Config. Its execution is then defined productively:
...
Thus computer includes the universal evaluator, the represented program, and its continuing executionâ”not just the terminal value extracted from one run.
...
Suppose a function body contains a common computation c, while two uses will supply different arguments: f(x) = H(c, x). A na¯ve split can create two independent computations of c. Sharing the completed value of f(x) does not help when the arguments differ. IC's duplication-through-lambda mechanism allows the common body structure to remain shared while the input-dependent portions separate.
...
The work is factored before, during, and across applicationsâ”not only batched afterward.

### Parallel + Univalent: computational independence has higher-dimensional content

Take e : A â‰ B and f : C â‰ D. Changing the left coordinate and changing the right coordinate are independently composable representation changes.
...
Parallel computer therefore means an engine for evolving dependency graphs, including graphs that represent further programsâ”not merely a large collection of arithmetic units.
...
Consider a branch-indexed dependent family A : Î© â’ ð’°. A branch may determine both a value and its type: x : âˆ_{Ï‰:Î©} A(Ï‰). Now suppose e : âˆ_{Ï‰:Î©} A(Ï‰) â‰ B(Ï‰). Then transport acts coherently branch by branch:

    y(Ï‰) = transport(ua(e(Ï‰)), x(Ï‰)).

The branch determines the type, the value, and the equivalence together.

That matters for a dependent package such as Î_{p:Program} Correct(p). The proof cannot be separated from the program whose type it inhabits. Likewise, a continuation cannot be separated from its state index.
...
For explicitly represented labelled families, the compatibility equation is:
    ...
    select_Ï‰(transportFamily(e, x)) = transport(e(Ï‰), select_Ï‰(x)).
...
Start with any map f : A â’ B. Define fib_f(b) = Î_{a:A} (f(a) = b). Then:

    A â‰ Î_{b:B} fib_f(b).

The forward map is a â¦ (f(a), (a, refl)). The visible result is still exactly f(a). The completed presentation retains what that visible result alone omits.
...
Your source states the stronger organization, LawfulStep(A) â‰ (A â’ A), and explicitly realizes an ordinary universal-machine step as the visible projection of its lossless completion.
...
The same idea applies to program synthesis. The mathematical result sought is not a high-scoring string; it can be Î_{p:Code} Correct(p). The evaluator can compute through candidate constructions, share common reductions, and retain the program/evidence dependency.
...
The underlying operational reason is again the IC interaction between application, abstraction, duplication, and superpositionâ”not an extra interpretation attached to a list of answers.
...
A useful schematic object is A : Î© — Iâ¿ â’ ð’°. Here Î© indexes correlated branch choices; Iâ¿ supplies cubical dimensions; A gives the dependent type at each context.
...
For example, A(Ï‰, i, j) = ua(e_Ï‰)(i) — ua(f_Ï‰)(j) is a family of independent representation-change squares, indexed by a correlated computational family.

Now all three words matter simultaneously. Superposition preserves the branch-dependent e_Ï‰, f_Ï‰, and inputs together. Univalence makes those equivalences act through the dependent family. Parallelism permits the independent coordinate computations to advance separately while shared branch structure remains shared.
...
Suppose P_A : A â’ A and e : A â‰ B. Define the transported program P_B = e âˆ˜ P_A âˆ˜ eâ»Â. Then P_B âˆ˜ e = e âˆ˜ P_A as a path-valued equality of functions.
...
The distinction from a conventional "run N programs on N workers" design is representational: a shared family of computations â‰  a collection of independently copied computations.
...
Now consider the total typed state space Point_â“ = Î_{A:ð’°_â“} A. At (A, a), an interaction can supply a target type B and an operation f : A â’ B, producing (B, f(a)).
...
Coinduction supplies the persistent execution form:
    ...
    ISC(w) â‰ âˆ_{q:Q(w)} Î_{w'} Î_{o:O(w,q,w')} E(w,q,w',o) — â–ISC(w').
...
Univalent Superposition Computer therefore means a computer whose operational material includes dependent terms, their correlated alternatives, and their higher identificationsâ”and which can continue computing with the results.
...
A useful schematic judgment is Î”; Î¦; Î; Î“ âŠ t : A, where Î” is the cubical dimension context, Î¦ the face assumptions, Î the correlated-choice context, and Î“ the ordinary dependent-variable context.
...
Ordinary substitution must agree with dimension substitution: (t[Ï])[Î] = (t[Î])[Ï[Î]] for compatible substitutions.

Branch restriction must agree with dependent transport: tr_F(e, t)|_Ï‰ = tr_{F|Ï‰}(e|_Ï‰, t|_Ï‰).
...
Take a branch-indexed family of machines s_Ï‰ : A_Ï‰, Î´_Ï‰ : A_Ï‰ â’ A_Ï‰, with representation equivalences e_Ï‰ : A_Ï‰ â‰ B_Ï‰. Define s'_Ï‰ = e_Ï‰(s_Ï‰) and Î´'_Ï‰ = e_Ï‰ âˆ˜ Î´_Ï‰ âˆ˜ e_Ï‰â»Â. Then, for every finite n:

    (Î´'_Ï‰)â¿(s'_Ï‰) = e_Ï‰(Î´_Ï‰â¿(s_Ï‰)).
...
For cubical homotopy type theory, the full scope includes Î , Î, ð’°, Path, transport, composition, Glue, together with the admitted inductive and productive coinductive constructions.

Their interaction is the computational theoryâ”not just a collection of labels attached to values. Constructive cubical foundations and their higher-inductive extensions supply precisely this kind of computational interpretation.
...
It is that the universe, its terms, its dependent operations, its equivalences, and its higher equalities participate in executionâ”and that execution itself admits sharing, superposition, parallel composition, and productive self-application.
...
|---|---|---|
| the interval, paths, types, `coe`, `hcomp`, `Glue` as runtime data | `--to-hvm4-full` erases nothing; `@coe`/`@hcomp` dispatch on type data at runtime; a composite with an undecided face is the value `#HCm{â¦}` | RUNTIME_FULL.md, SYNTHESIS.md Â§1â“2 |
| univalence computes, both directions, coherently | `ua`/`Glue`, `uaÎ²` definitional, `pathToEquiv(uaE e) = e` | `uaequiv.bend`, `uaglue.bend`, GLUE.md, UNIVALENCE.md |
| the lossless presentation `A â‰ Î B (fiber f)` as an executable universe path | `fibrelaw.bend`, present/retrieve run on the net | FIBRE_LAW.md |
| superposition as the DUP/SUP structural rules, routing transport branch by branch | `coe` along a superposed line needs no rule (a match commutes over `&L{}`); measured regimes | `supline.bend`, SYNTHESIS.md Â§4 |
