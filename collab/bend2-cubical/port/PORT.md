...
| **The front-ends** (`scripts/emit-*.py`): each reads the field's own format and emits a file in the shape of the hand-written presentation, with the emitter's own evaluations stated as `refl` theorems so the checker certifies emitter and generator agree | `ReactionEmitted.bend` (SBML L3 + PEtab-shaped JSON → species record, `fire` peeling stoichiometries, modifiers gated on presence, conditions/experiments/observables) | 87 | 2 (291 itrs) |
| | `PangenomeEmitted.bend` (GFA1 two-bubble graph → oriented segments, reverse complements, links as an indexed family with overlaps, paths as walks, sequence fold trimming overlaps, `takes(seg)` calls, the fibre law at the reference call and the two-point fibre found by the emitter) | 94 | 6 (1466 itrs) |
| | `PangenomeSpecEmitted.bend` (the GFA specification's own example: `11+,12-,13+` with 4M/5M overlaps) | 81 | 9 (1001 itrs; the GFA path sequence) |
| | `LineageEmitted.bend` (Newick tree → one record per leaf with the child taken at each division, terminal bodies by refl, and for every terminal reading shared by two leaves the non-contractible fibre, found by the emitter) | 105 | 11 (447 itrs) |
| | `SpatialEmitted.bend` (points CSV + segmentation rules → field, segmentations as threshold functions, the fibre law at each, every cell-by-gene entry by refl, the gene count descending through the coordinate-forgetting quotient, the cell assignment refuted as non-descending where two same-gene molecules split, segmentation dependence/invariance per gene) | 111 | 2 (2011 itrs) |
| **GRN inference** (the CausalBench shape), on the perturbation presentation: a graph is a reading of the minimal machine. A second generator family with one more edge (tf → hk) is behaviourally identical under the tgt assay — `edge_invisible` for every cell and word, the Nerode relations coincide both ways — while a family with the tf → tgt edge removed is separated by one experiment (`edge_visible`), and no pointwise identification of its Nerode relation with the first exists (`machines_differ`, via `tf_is_necessary`). Edge lists are receivers of the family, not of the machine | `GRN.bend` | 193 | 2 (480 itrs) |
| **Whole cell as composition**: the product of the expression state and a reaction-network state, the sum of their generators, one derivation type over the composite; uncoupled, a gene step and a reaction step commute on every state by refl; coupled by one cross-scale edge (passage translates hk into the enzyme) they do not, and hk — never read by the metabolite assay, in the other subsystem — becomes necessary state (`hk_necessary_coupled`, one composite experiment) while uncoupled it is erasable (`hk_erasable_uncoupled`, every word); the expression-side count and the reaction-side conservation certificate are folds of the same composite history | `WholeCell.bend` | 215 | 1 (525 itrs) |
