...
whnfHCm lv book a r u0 u1 x =
  case whnf lv book r of
    ...
    r' -> if constLine u0 && constLine u1
            -- Degenerate composition: both side-walls are constant in the
            -- composition dimension, so the square has degenerate sides and
            -- the lid equals the base.  Sound for ANY type A (no dispatch on
  ...
  where
    ...
    constLine u = case whnf lv book u of
      PLm _ f -> not (occursMarker (normal 0 book (f coeMarker)))
      _       -> False
