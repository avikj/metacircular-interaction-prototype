...
|---|---|---|---|
| Path/PathP, refl/sym/cong/âˆ™, funExt, J, subst, transport | ubiquitous | native (`Path`, `<i>`, `@`, `hcomp`/`coe`) | `port/Prelude.bend` 47â“; every suite file |
| univalence (`ua`, `uaÎ²`, `isoToEquiv`, `equivFun`, `compEquiv`) | 758 / 577 / 552 / 431 | `ua` with uaÎ² definitional, Glue, `lemIso` transcribed | `uaequiv.bend`, `uaglue.bend`, `roundtrip.bend`, `port/Carrier.bend` |
| h-levels (`isContr`/`isProp`/`isSet`, `isContrSingl`, `isPropâ’isSet`, `isPropIsContr`, `isSetâ•`) | ~2800 | defined; `isSetNat` by encodeâ“decode, `isPropToIsSet` by a square | `port/Prelude.bend`, `hset.bend`, `effective.bend` |
| `Cubical.HITs.PropositionalTruncation` (117 files), `SetQuotients` (66), `S1` (44), `Truncation`/`SetTruncation` (32), `Sn`, `KleinBottle`, `Torus`, `Susp`, `Pushout`, `Bouquet`, `FreeGroup`, `MappingCones` | | ONE declaration schema (`type â¦ path @c(â¦): Path(â¦)`), dependent eliminator, transport and hcomp through HITs, full runtime | `HITS.md`, `hit_*.bend` (16 files), effectivity 23â“ |
| inductive families with indices (`Step x y`, `Vec A n`, â¦) | everywhere | constructors carry index equations `T{x == â¦}`, matched by `{==}` | `port/RewriteCertificate.bend` 57â“ |
| records with dependent and type-valued fields (`NativeOperation`, `Machine`) | everywhere | `type` with dependent fields, one-case matches as projections | `port/ControlledGrammar.bend` 73â“ |
...
| `Cubical.Data.Nat` (+ Order, Divisibility, Mod, GCD), `Int`, `List`, `Sigma`, `Sum`, `Maybe`, `Fin`, `Bool`, `Unit`, `Empty` | 1613 / 280 / 553 / â¦ | Nat/List/Bool/Unit/Empty/Î native; Int, Sum, Maybe, Fin as declared `type`s; laws proved as needed | `port/Prelude.bend` (Nat laws), library to grow with the port |
...
| universe levels (`Type â“`) | | `Set : Set` (no levels) â” a consistency caveat of Bend2, not of the port | â” |
| `abstract` (226), `postulate` (only in the erased/mukha lane) | | not needed; nothing is postulated in the port | â” |
...
- Conversion is untyped one-step unfolding: some definitional equalities
  Agda sees need an explicit path here (e.g. Î for a user record is Î-Î
