# Coinduction in Bend2 — what "no coinduction" meant, what is true, what was fixed

## The corpus's need

The calculus is coinductive at its core: `IExec`, `Answers`, `ISC`, `Exec`,
`Dhārā` are coinductive records; histories are infinite; determinism is
...
1. **A corecursive path proof looped the checker.** `repOnes : Path(Stream,
   ...
   shape — hung. Cause: `epNormCtx` (my Ref-unfolding for typed endpoints)
