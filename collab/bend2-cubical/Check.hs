...
infer d span book@(Book defs) ctx term =
  ...
  case term of
    ...
    Glu a fs -> do
      -- Glue A [(φ,T,e)] : Set.  A : Set; each face gives an interval extent
      -- φ, a partial type T : Set, and its forward transport e : T -> A.
      -- (The full equivalence obligation on e is a partial-element condition;
      -- e's forward map is what unglue/coe consume, and it is typed here.)
      check d span book ctx a Set
      mapM_ (\(p,t,e) -> do
        check d span book ctx p Itv
        check d span book ctx t Set
        check d span book ctx e (All t (Lam "_" (\_ -> a)))) fs
      Done Set
    GlB a fs x -> do
      -- glue A [(φ,t)] x : Glue A [(φ,T,e)] ; x : A is the base, t : T the
      -- partial section, coherent (e t = x on φ).  We infer the Glue type
      -- from the faces' inferred T's and the base type.
      check d span book ctx x a
      tfs <- mapM (\(p,t) -> do
        check d span book ctx p Itv
        tT <- infer d span book ctx t
        Done (p, tT, Lam "_" (\_ -> x))) fs
      Done (Glu a [ (p, tT, Lam "z" (\z -> z)) | (p,tT,_) <- tfs ])
    UnG g -> do
      gT <- infer d span book ctx g
      case force book gT of
        Glu a _ -> Done a
        _ -> Fail $ TypeMismatch span (formatCtx d book ctx) (format d book (Glu (Var "_" 0) [])) (format d book gT)
    HCm a fs x -> do
      check d span book ctx a Set
      check d span book ctx x a
