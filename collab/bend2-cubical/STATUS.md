...
| **Biology as finite presentations of the machine** (2026-09-14; `bio/`) | `Descent.bend` (erasure = descent as a type, `refuse`, the fibre law with ua β); `Perturbation.bend` (AdiBija fold + uniqueness, Vivarana, the Nerode quotient, a two-rung descent-classified compression curve, the context-naturality square's three outcomes, a VCC-shaped prediction); `Segmentation.bend` and `Pangenome.bend` (the same `Descent`, unchanged); `bench/` (N-cell superposition vs list vs hoist: the emitter's output shows a ~1.45× per-cell premium that is the nested-pair record encoding, not the calculus — hand-written flat HVM4 gives 0.91; and the I64-literal-as-0 emitter bug found and fixed) | `bio/suite.sh` bad=0: 75/194/114/112 ✓, mains 0/12/1/2 on HVM4 (0/1256/1216/813 itrs); `bio/bench/README.md` |
