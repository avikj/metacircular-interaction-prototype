# Cubical Bend / HVM: Universal Executable Identity and Optimal Interaction

## Thesis

This is the HOC/Taelin-facing account of the completed Bend/HVM convergence.

The construction is not a theorem prover attached to Bend and not cubical syntax beside an unrelated evaluator. It identifies one executable mathematical object whose different readings are proof, transport, inference, computation, factorization, optimization, and continued interaction.

\`\`\`text
proof = transport = inference = computation = factorization
\`\`\`

Cubical paths make identity computational. Univalence makes equivalence executable as identity. Higher cubes retain coherence among alternative transports. The fibre retains exactly the distinction hidden by an observation. Coinduction returns the whole process as its continuation. HVM realizes the object through local interaction with explicit sharing and correlated superposition.
...
The lawful completion space of a fixed map is contractible. The residual is therefore forced, up to identity, by the computation itself. At process level, lawful lossless steps are equivalent to the maps themselves.

The later kernel and cost work closes the operational side. Cost is a receiver of the same represented interaction; the lower-bound/attainment theorem identifies geodesic realizations in the declared primitive interaction geometry. With unit-cost interactions, the attained geodesic has minimum interaction count. With vector costs, the corresponding statement is Pareto/componentwise.

This is the quantity for which the launch Bend2 implementation moved away from interaction nets.
...
- which identity witnesses this transformation?
- where does a backend first destroy a source distinction?
- are these two implementations equivalent at the declared observation?
- what is the exact cost of the transport?
- what factorization is forced by the dependency geometry?
- which admitted realization attains the geodesic?

This removes the architectural need for correctness to live in a separate kind of metatheory. It does **not** mean that writing code in Bend magically proves it: it means the implementation and its preservation obligations inhabit the same executable mathematical universe as everything else.

### Cubical structure is not an intrinsic runtime tax

Cubicalization is not “ordinary computation plus proof bookkeeping.” An existing realization remains available when additional certified identities and factorizations are installed. If \(\mathcal R_K(F)\) is the realization space before and \(\mathcal R_{K'}(F)\) after adding knowledge,

\`\`\`math
\mathcal R_K(F)\subseteq\mathcal R_{K'}(F).
\`\`\`

For the same cost interpretation,

\`\`\`math
\min_{r\in\mathcal R_{K'}(F)}C(r)
\le
\min_{r\in\mathcal R_K(F)}C(r).
\`\`\`

Additional identity can leave the geodesic unchanged or expose a shorter factorization; it does not force a longer one. Proof-only structure may erase at the selected runtime interface, but before erasure its identities can eliminate mediation, duplicated work, conversions, and representation detours. If an observable genuinely depends on a transport, that transport is the requested computation, not a second “cubical operation” charged on top of an otherwise identical cheaper computation.

### HVM is a realization, not the foundation

This directory targets the pre-launch Bend2 interaction-net lineage. The September 2026 \`bendlang/bend\` implementation uses BendRT rather than HVM as its primary runtime.

That does not make the cubical construction specific to the old runtime. HVM is the existing realization in which the cubical object reduces directly on the interaction substrate. The same universal cubical core can in principle be installed over a lower-order C/Metal/CUDA/JS backend; that backend and its lowering then become additional objects whose preservation and cost are queried by the same machinery.

This is why the current launch Bend2 architecture is a particularly direct application: its paper theory, Lean model, production checker, compiler, and runtime are separate artifacts intended to correspond. Cubical identity can make those correspondence maps first-class computational objects rather than informal semantic firewalls.

### Exact cost certificate
...
A remaining-work potential \(\Phi\), zero at the demanded terminal observation, with

\`\`\`math
\Phi(u)\le c(u,v)+\Phi(v)
\`\`\`

on every primitive edge gives

\`\`\`math
\Phi(s)\le C(\gamma)
\`\`\`

for every admitted realization from \(s\) to that observation. If a native path attains equality edge-by-edge, it meets the lower bound exactly and no admitted realization is cheaper.

The repository contains a closed native example: the all-word prefix theorem forces at least \(n\) crossings to bring cell \(n\) to the head, while \`Bring(n)\` attains exactly \(n\). Thus the displayed interaction is a proved geodesic, not merely one short implementation.
