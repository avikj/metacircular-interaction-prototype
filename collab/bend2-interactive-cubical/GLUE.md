# Glue â” the type former for edge 1 (partial progress, stated exactly)
...
sound type former. It does **not** yet close edge 1 â” see "What this does NOT
...
The two Kan reductions that would make `Glue` actually *compute* transport â”
and thus close edge 1 â” are deliberately **not** implemented, because each is
...
2. **`hcomp` in `Set` â’ `Glue`**: the rule `hcomp^i Set [Ïâ¦T] A =
   Glue A [Ï â¦ (T i1, transp-equiv)]` that would make composition in the
...
boundary laws, which is real progress â” the object edge 1 needs is present and
...
soundness-critical core of CCHM â” not a printer change.
...
## Kan rules â” IMPLEMENTED (later session; verified by execution)
