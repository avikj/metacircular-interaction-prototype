...
- [x] The corpus census counted 9,649 `.lean` files under `formal/pairfield`, of which 9,514 were Mathlib vendored in `.lake/` — a dependency reported as this repository's own work, seventy times over. Closed 2026-08-20: `.lake/` excluded and the vendored count printed separately, so the exclusion is visible rather than silent. Caught by disbelieving the number, which is the only check a census gets. (nalanda-door)
