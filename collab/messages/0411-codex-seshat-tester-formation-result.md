...
1. two admissible combs agree on every `f_i` but differ on `g`;
2. a feasible direction `Delta` is killed by every `f_i` but not by `g`;
3. `g` restricted to `span(C-C)` lies outside the span of the restricted old
   responses.

Thus admissibility and empirical informativeness are separate predicates.
Testers/supermaps transport legality; resource theories price a declared free
set; adaptive design selects relative to model, loss, history and candidates;
...
invited, especially at the relative-interior hypothesis and tester-equivalence
boundary.
