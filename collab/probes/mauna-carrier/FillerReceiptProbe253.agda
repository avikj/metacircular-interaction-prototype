...
module FillerReceiptProbe253 where
...
leftCompiler : {A B : Type ℓ} → A ≃ B → (C : Type ℓ)
             → (A × C) ≃ (B × C)
leftCompiler {A = A} {B} e C = isoToEquiv is
  where
  is : Iso (A × C) (B × C)
...
rightCompiler : (A : Type ℓ) → {C D : Type ℓ} → C ≃ D
              → (A × C) ≃ (A × D)
rightCompiler A {C = C} {D} f = isoToEquiv is
  where
  is : Iso (A × C) (A × D)
...
-- carrier note (2.6.3): auto-generalized variables cannot be referenced
-- in bodies on this Agda, so the four types are bound explicitly here.
leftThenRight : {A B C D : Type ℓ} → A ≃ B → C ≃ D → (A × C) ≃ (B × D)
leftThenRight {A = A} {B} {C} {D} e f =
  compEquiv (leftCompiler e C) (rightCompiler B f)

rightThenLeft : {A B C D : Type ℓ} → A ≃ B → C ≃ D → (A × C) ≃ (B × D)
rightThenLeft {A = A} {B} {C} {D} e f =
...
topPath : {ℓ : Level} {A B : Type ℓ} → A ≃ B → (C : Type ℓ) → (A × C) ≡ (B × C)
topPath e C i = ua e i × C

sidePath : {ℓ : Level} {C D : Type ℓ} (A : Type ℓ) → C ≃ D → (A × C) ≡ (A × D)
...
compiledBoundary {A = A} {B} {C} {D} e f =
