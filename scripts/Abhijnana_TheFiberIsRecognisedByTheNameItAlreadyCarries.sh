...
# question: **does this fiber already have a name in this corpus?**
...
# prose that Metre, Vak and Chosen are its fibers.  No term said it, so the
...
# to `Σ[ a ] (f a ≡ b)`.  A definition of that exact shape IS a fiber.  The
...
# ── २ · the shelf: every fiber already written out, anywhere ───────────
#
# A fiber written out is a Σ whose body ends in an equation into the index.
...
# (the fiber of a SEPARATION, counted apart), data-eq, subtype-eq,
# sigma-bare-eq -- across all three lanes, and finds 475.  It also found
# `NaturalMachine/PairsSummingTo.agda`, which holds ADDITION'S OWN FIBER and
...
runghc interactive/Upalabdhi_TheFiberWasAlreadyWrittenAndTheCensusCalledItUndecided.hs \
  2>/dev/null \
...
printf '\n  अभिज्ञान — does this fiber already have a name here?\n'
printf '  ──────────────────────────────────────────────────────────\n'
printf '  undecided edges on road two   : %s\n' "$NQ"
printf '  fiber rows already written out: %s\n' "$NS"
...
if [ "$CHECK" -eq 0 ]; then
  printf '  Each is a LEAD.  Only the kernel says whether the map whose fiber\n'
  printf '  was written is the map in the queue.  Run with --check.\n\n'
  exit 0
fi

# ── ५ · the probe.  `fiber f b` unfolds to `Σ[ a ] (f a ≡ b)`; if the
#        written fiber IS that Σ for the queued map, `refl` typechecks.
...
while IFS="$TAB" read -r kind sb mb sraw traw site ssb smb ssraw smraw def np idx file line shape; do
  N=$((N+1))
  PN=$(printf 'P%03d' "$N")
  # the fiber's host module, read from the file itself — never guessed
  ...
  case "$file" in
    fiber/*)
      printf '%s\t%s\t%s\t%s\t%s\n' "$kind" "$site" "$def" "SKIP" \
        "fiber lives in the fiber library, probe lives in natural-machine; cross-library probe not attempted" \
...
_ = refl"
    ...
    printf '%s\t%s\t%s\t%s\t%s\n' "$kind" "$site" "$def" "SKIP" \
      "the written fiber takes $np indices; a joint fiber is not `fiber f b` without Σ-reassociation (see छन्दोमुद्रा §३)" \
      >> "$W/verdicts.tsv"
    continue
  fi
...
-- the index of a 0-parameter fiber is a LITERAL lifted out of the source
