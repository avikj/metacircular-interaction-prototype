...
==============================================================================
  ...
  Historical sources --- Pini's grammar, Indian algebra, and the classical
...
the pin it stops and says so â” so a red is never ambiguous between "the
...
knows the three obstacles that actually bite â” cabal writes its config
...
------------------------------------------------------------------------------------------
  ...
  formal/cubical/kernel/ â” 296 lines in three files, and the entry is
...
not a reduction relation but a generated equivalence carried as data â”
...
endpoints are two distinct objects.  Then one line closes the circle::
      ...
      install : Derivation lhs rhs â’ NativeOperation
...
`control-sound : Control t â’ t â‰¡ source` pins an OPEN field: the caller
...
one composite â” however permissive the control, an operation has at most
ONE locus â” and computes the installed locus type: contractible, centre
...
**3 Â Multiplicity is conserved.**
`advance-preserves-branch-count` â” no dedupe, no sort, no quotient.  Read
...
**4 Â Reversibility is structural, not inferred.**
`reverse` is a constructor.  It has to be, because `Samyoge_` proves
losslessness composes and lossiness does not â” `Unit â’ Bool â’ Unit` is
the identity with neither factor an equivalence â” so you may certify a
pipeline lossless step by step and may NOT diagnose it lossy that way.

**5 Â New content comes from changing the reading, and its yield is
...
invisible to `eval` is a nontrivial loop in the universe, through `uaÎ²`.
`VyatyasaVarga_` then measures that loop: order exactly two.  **â/2** â”
not a braid generator, because âŠ is symmetric monoidal and its symmetry
is an involution by construction.
  ...
  own number, Bjagaita, 1150 â” with every norm the cakravla visited.
...
------------------------------------------------------------------------------------------
  ...
  own collapse identifies.  The blindness is FORCED by the collapse â” not
...
------------------------------------------------------------------------------------------
  ...
  8.2.1   ààà°ààµààà°à¾àà¿à¦àà§à®à   from this stra to the end of the text, a rule is
          asiddha â” "as if not having taken effect" â” with respect to
          everything that precedes it.  The blindness is one-way, ordered,
          and stated as a rule of the grammar itself.

  the collapse    8.4.56 àµà¾à½àµàà¾à¨à sends two forms that DISAGREE about
...
agreeing with it.  Not a different answer there â” NO VALUE THERE.
...
set.
  ...
  constant.  Hence no descent â” and the blindness is forced, not
  stipulated.
...
------------------------------------------------------------------------------------------
  ...
  what goes blind the holonomy â” exactly, and in both directions.

invisibleExactlyWhenInvariant  â”  a semantics is unmoved by transport
...
content of LQG kinematics, and it is four things:
  ...
  Â edges are group actions;
  Â vertices are intertwiners, and gauge invariance IS the equivariance
    square â” not a constraint added on top of the label;
  Â flux is a derivation, and subdivision-compatibility is FORCED by
    multiplicativity and Leibniz alone.  Orientation and intersection
    sign are chosen afterward and are not needed for it;
  Â an observable is blind to holonomy exactly when it is gauge-invariant.
  ...
  the entire physical residue â” what no gauge-invariant observable can see
  â” is holonomy, exactly and measurably.
...
congruences, installed primes â” is idempotent, because combining a datum
with itself adds nothing.  lcm is idempotent for that reason, and not for
a reason about divisibility.

sign-is-not-accumulable  â”  there is no accumulative law and no
multiplicative f into â taking the value âˆ’1.  Ever.  Not "hard to
accumulate."  Over â the units are Â1, so this rules out Î» everywhere and
Î¼ off the squares, FOR EVERY ACCUMULATIVE LAW AT ONCE, with no domain
hypothesis.
...
àà™àà•àà¾à, akapa, "the net of digits", is Bhskara II's Llvat section
on permutations (~1150): the ARRANGEMENT, as against the count of
arrangements.  No verse number â” editions differ in their numbering there.

  the collapse    eval : Tm â’ Env â’ â•, a readout into a set.
  what goes blind a transposition.  One â/2 of holonomy.

no-counting-criterion-separates  â”  for ANY type C at ANY level and ANY
function Ï of the counting meaning, Ï cannot distinguish performing the
commutation from doing nothing.  Universally quantified over every
possible readout, not over the ones anyone has tried.

comm-loop-is-a-nontrivial-loop-in-the-universe  â”  and the categorified
semantics DOES see it.  The path is not refl, proved through uaÎ², which is
univalence's Î²-rule COMPUTING.  Calculated, not asserted.

Commutativity of addition is not free information.  It is a â/2 of
holonomy, and every readout valued in a set annihilates it.
  ...
  holonomy.  Inductive bias is not free capacity â” it is paid for at a
  measurable rate, one â/2 per independent loop.
...
------------------------------------------------------------------------------------------
  ...
  the collapse    derivation-sound lands in an identity type of â•, and â•
                  is a set, so that type is a PROPOSITION.
  what goes blind which route was taken.

two-nodes-cannot-disagree  â”  any two derivations between the same terms
...
merge-is-idempotent  â”  the library join is grow-only, commutative and
