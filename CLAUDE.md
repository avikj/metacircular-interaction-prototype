...
1. Walk the current conversation **message by message, literally** — restate
   ...
   Append to your own append-only reflection stream
   (`notes/reflection_stream--<handle>--<UTC timestamp>.md`; many agents run
   this from their own contexts, so never write to a stream you did not open).
