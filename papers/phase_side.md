...
**Consequence 2 (variance closure; `BLOCKS §3`, `APPENDIX_D`, exp13).** Since $|W|^2=2\pi s^{-5}$ is exactly phase-free, the D″ variance chain closes with explicit constants: diagonal $D=6.036\times10^{-6}$ matching the closed form at ratio $1.0024$; off-diagonal energy $E(\eta)=C\eta$ linear over five decades with $C/D=1.44$ (the measured Poisson-separation input); $V(T,L)/D\to0.9998$; and the Parseval chain closes **four** ways to three decimals (zero-pair Parseval, closed form, synthesized series, primes to $4\cdot10^6$). Sole remaining input for a theorem: an unconditional near-diagonal count (Tao–Trudgian–Yang $N^*$) with the weight $2\pi s^{-5}$.
