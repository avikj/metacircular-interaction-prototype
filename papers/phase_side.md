...
5. **Screw positivity, quantitatively:** ~~mixed-block statement~~ — corrected by the audit and finalized by exp27 (`BLOCKS §5.1`): the block constants *run with the profinite scheme* ($0.362\log^2Q+\dots$ for $[\sharp\sharp]$; sum pinned at the invariant $c_2=-2.2803$ to $10^{-4}$ over $Q\in[5,120]$), so MS positivity is a statement about the scheme-invariant combination; the screw kernel's *fluctuation* lives in the mixed block. Remaining: an intrinsic (scheme-free) characterization of the fluctuation projector.
