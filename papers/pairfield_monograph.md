...
**P2 — Conjecture A″, non-cyclotomic part.** F2 classifies every cyclotomic factor globally, F3--F5 classify every factor degree through five, and exact certificates leave no such factor for $X\ge13$.  The first open degree is therefore sextic.  What remains is to exclude non-cyclotomic factorization beginning in degree $6$, or prove the weaker exact condition that no nontrivial factor reversal is again a $0$--$1$ polynomial.  The latter already suffices for prime phase rigidity.  Computational next checkpoint: $X=10^5$ (~2 h at current FLINT scaling).
