...
> **Theorems F1 and F3** (`RIGIDITY_FRONTIER §2`; `CUBIC_OBSTRUCTION`). *For $X\ge5$, $F_X$ has no linear factor; every irreducible quadratic is cyclotomic; and an irreducible cubic factor occurs iff $5\le X<7$, when $F_X=x^3+x+1$.*  The cubic proof actually applies to every finite $0$--$1$ polynomial $1+x+x^3+\sum_{j\ge5,\ j\text{ odd}}\epsilon_jx^j$: root geometry and Vieta reduce a putative cubic to six integer pairs, all but $x^3+x+1$ impossible.  Thus for $X\ge13$ the unconditional lower bound on factor degree is $4$.
...
Consequently **any counterexample to A″ must consist entirely of non-cyclotomic factors of degree at least $4$.**
...
**P2 — Conjecture A″, non-cyclotomic part.** F2 now classifies every cyclotomic factor globally, and F3 excludes every cubic factor for $X\ge7$.  The first open degree is therefore quartic.  What remains is to exclude non-cyclotomic factorization beginning in degree $4$, or prove the weaker exact condition that no nontrivial factor reversal is again a $0$--$1$ polynomial.  The latter already suffices for prime phase rigidity.  Computational next checkpoint: $X=10^5$ (~2 h at current FLINT scaling).
