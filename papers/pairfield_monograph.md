...
> **Theorems F1, F3, and F4** (`RIGIDITY_FRONTIER §2`; `CUBIC_OBSTRUCTION`; `PARITY_RESULTANT`). *For $X\ge5$, $F_X$ has no linear factor; every irreducible quadratic is cyclotomic; an irreducible cubic factor occurs iff $5\le X<7$, when $F_X=x^3+x+1$; and no irreducible quartic factor occurs for any $X$.*  The cubic proof actually applies to every finite $0$--$1$ polynomial $1+x+x^3+\sum_{j\ge5,\ j\text{ odd}}\epsilon_jx^j$: root geometry and Vieta reduce a putative cubic to six integer pairs, all but $x^3+x+1$ impossible.  The quartic theorem is an exact finite certificate: parity forces $a^2-abc+c^2=\pm1$, root geometry leaves 62 triples, and exact Sturm/resultant/tail inequalities eliminate them all.  Thus for $X\ge13$ the unconditional lower bound on factor degree is $5$.
...
Consequently **any counterexample to A″ must consist entirely of non-cyclotomic factors of degree at least $5$.**
...
**P2 — Conjecture A″, non-cyclotomic part.** F2 classifies every cyclotomic factor globally, F3 excludes every cubic factor for $X\ge7$, and the exact computer-assisted F4 theorem excludes every irreducible quartic.  The first open degree is therefore quintic.  What remains is to exclude non-cyclotomic factorization beginning in degree $5$, or prove the weaker exact condition that no nontrivial factor reversal is again a $0$--$1$ polynomial.  The latter already suffices for prime phase rigidity.  Computational next checkpoint: $X=10^5$ (~2 h at current FLINT scaling).
