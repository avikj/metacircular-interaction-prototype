...
> **Theorems F1 and F3--F5** (`RIGIDITY_FRONTIER §2`; `CUBIC_OBSTRUCTION`; `PARITY_RESULTANT`; `QUINTIC_OBSTRUCTION`). *For $X\ge5$, $F_X$ has no linear factor; every irreducible quadratic is cyclotomic; an irreducible cubic factor occurs iff $5\le X<7$; no irreducible quartic occurs; and an irreducible quintic occurs iff $7\le X<11$, when it is $F_7=x^5+x^3+x+1$ itself.*  The cubic proof reduces a putative factor to six integer cases.  The quartic and quintic theorems use the degree-independent identity $\operatorname{Res}(g,g(-x))=2^{\deg g}\operatorname{Res}(E,O)^2$ for $g=E(x^2)+xO(x^2)$, followed by exact finite certificates.  Thus for $X\ge13$ the unconditional lower bound on factor degree is $6$.
...
Consequently **any counterexample to A″ must consist entirely of non-cyclotomic factors of degree at least $6$.**
...
**P2 — Conjecture A″, non-cyclotomic part.** F2 classifies every cyclotomic factor globally, F3--F5 classify every factor degree through five, and exact certificates leave no such factor for $X\ge13$.  The first open degree is sextic, but its reciprocal subfamily is now excluded exactly (`RECIPROCAL_SEXTIC`); only nonreciprocal sextics remain.  What remains is to exclude non-cyclotomic factorization beginning there, or prove the weaker exact condition that no nontrivial factor reversal is again a $0$--$1$ polynomial.  The latter already suffices for prime phase rigidity.  Computational next checkpoint: $X=10^5$ (~2 h at current FLINT scaling).
