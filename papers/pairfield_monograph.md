...
> **Theorem F1** (`RIGIDITY_FRONTIER §2`, proof there). *For $X\ge5$, $F_X$ has no linear factor; any irreducible quadratic factor is one of $\Phi_3,\Phi_4,\Phi_6$; hence (with Theorem F2) for every $X\ge13$ every irreducible factor of $F_X$ has degree $\ge3$.* Ingredients: the root annulus $\tfrac12<|z|<2$ (Lemma F0) and positivity $F_X(t)\ge1$ on $t\ge0$. Corollary F1.1 (Kronecker dichotomy): every non-cyclotomic factor has Mahler measure $>1$; non-reciprocal ones have $M\ge1.3247\ldots$ (Smyth). Degree 3 is where the provable zone currently ends.

> **Theorem F2** (classification of cyclotomic ties; `RIGIDITY_FRONTIER §3`, PROVED + MACHINE-VERIFIED). *For every $m\le1000$ and every $X\ge3$: $\Phi_m\mid F_X\iff(X,m)\in\{(3,2),(11,6)\}$.* Structure: for squarefree $m$ the primitive $m$-th roots are $\mathbb Q$-independent (Lemma F2.1), so the tie equation pins the class counts $n_c(X)$ to explicit values, giving finite, effectively computable tie intervals (Theorem F2-sf) — for $m=6$ the interval is $[11,13)$, so **the $(11,6)$ tie is the unique $\Phi_6$ tie ever**; for $4\mid m$ a Galois parity averaging kills ties for all $X$ (Theorem F2-4 — the prime 2's contribution is frozen into the rational coordinate); for the remaining non-squarefree $m$, exact lattice computations (HNF over FLINT) certify non-membership of the target for all 392 values $m\le1000$. Scan (`exp7b`): all $m\le200$, all $X\le10^7$ — exactly one tie, $(11,6)$.

Consequently **any counterexample to A″ must consist of non-cyclotomic factors of degree $\ge3$, or a cyclotomic $\Phi_m$ with $m>1000$, which forces $\varphi(m)\ge240$.**

The recurrence question — do $\varphi(m)=2$ ties recur, Chebyshev-bias style? — is resolved **negatively** (`RIGIDITY_FRONTIER §4`, PROVED): the class-count walk is not mean-zero but monotone with drift; for $m=3,6$ the tie pins a monotone coordinate to a small integer passed by $X=13$, so ties are impossible forever; for $m=4$ the walk is recurrent but in the wrong affine subspace — the tie point is off-lattice, and the recurrent structure survives only as near-ties at distance exactly 1, which occur infinitely often *unconditionally* (Littlewood 1914; 111 cutoffs with $|v|^2=1$ up to $10^7$, clusters matching the Leech / Hudson–Bays sign-change regions). The effective walk dimension $D_m$ has the closed form $\prod_{p^e\|m}d_{p^e}$ (machine-verified to $m\le200$), and every non-squarefree $m$ not divisible by 4 has $D_m\ge4$: transient. **Conjecture F2∞:** $(3,2)$ and $(11,6)$ are the only cyclotomic ties, over all $m$ and all $X$.
...
`WEIL` fixes the normalization of the explicit formula (Proposition W1, proved there) and verifies it to worst-case relative deviation $1.8\cdot10^{-10}$ over 16 test functions (Gaussian family and complex mixtures) with up to 100k zeros and $\Lambda$ to $10^7$ (`exp14`) — a stringent joint test of every constant. Weil's criterion (Proposition W2): RH $\iff W(g)\ge0$ for all $g\in C_c^\infty$, $W(g)=\sum_\rho\Phi_{g\star\tilde g}(\rho)$.
...
**P2 — Conjecture A″, non-cyclotomic part.** After F1/F2, what remains is: (i) exclude irreducible non-cyclotomic factors of degree $\ge3$ — the first open degree is cubic (a cubic factor would be a non-reciprocal unit with one real root in $(-2,-\tfrac12)$ and Mahler measure $\ge1.3247$; root location alone does not exclude it); (ii) the §3.3 lemma: for odd $p$ with $p^2\mid m$, show $-t_m\notin\mathbb Q\text{-span}\{v_c\}$ in general (machine-true for all $m\le200$); (iii) any individual $m$ is decidable by the finite F2-sf computation. The weaker corrected rigidity target (§2.3) suffices for the arithmetic conclusion. Computational next checkpoint: $X=10^5$ (~2 h at current FLINT scaling).
...
graph TD
  ...
  subgraph partII["Part II — Spectrum"]
    ...
    EN["exp16: energy measured, c ≈ 2.8δ"]
...
Every measured number quoted in this monograph is printed by the named script: `exp1/1b/1c` (rigidity, factorizations), `exp2` (aperture), `exp4` (one singular series, two marginals), `exp5` (zero field), `exp6b` (sum spectrum), `exp7/7b` (ties), `exp8` (BC correlator, E0), `exp9` (crossover, L-identity), `exp10` (parity atoms), `exp12` (screw), `exp13` (blocks), `exp14` (Weil), `exp15` (divisor), `exp16` (energy), `exp17` (D-side), `exp19` (ternary), and the four `redteam_*.py` replications. Zeros: `data/odlyzko_zeros_100k.txt`.

### Verdict, in one paragraph

The pair field is not a new structure — it is the rank-one square of the prime indicator, and every physics reading tested was inert. Interrogating it adversarially produced: a genuinely new rigidity theory for the primes' gap data (A′, F1, F2, with an unconditional frontier at $X=5\cdot10^4$); a two-line smoothed form of "average Goldbach ⟺ RH" (C) that relocates a celebrated difficulty into the cutoff; the direct spectral display of the zeta sum-spectrum inside Goldbach data (D/D′), verified to $10^{-3}$–$10^{-4}$ and adversarially replicated, with its variance theory reduced to one finite-checkable energy bound (D″/P4); a forced, measured, three-block anatomy of the smoothed Goldbach count; a critical scaling theory for the Hardy–Littlewood singular series with Dickman crossover and Mertens finite-size correction ([CROSS]); and an exact symmetry-protection account of the parity barrier, closed at the level of the core (F, CORE_KMS), with the Weil obstruction stated as a proposition rather than a mood (W3). The wall is now four named problems with exact reductions, each calibrated against a solved case (ternary) and a solvable model (divisor). What the program did not do is advance RH, binary Goldbach, or Chowla by one epsilon — and it says so.
