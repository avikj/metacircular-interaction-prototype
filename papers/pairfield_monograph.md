...
**P2 — Conjecture A″, non-cyclotomic part.** F2 classifies every cyclotomic factor globally and F3--F7 classify every factor degree through seven.  The only septic is the non-cyclotomic cofactor $F_{11}/\Phi_6$; exact certificates leave no factor of degree at most seven for $X\ge13$, and the carrier decomposition then proves $F_{13}$ and $F_{17}$ irreducible.  The first open degree is octic; at $F_{19}$ the only surviving proper degree pattern is $8+9$.  What remains is to exclude higher non-cyclotomic factorization, or prove the weaker exact condition that no nontrivial factor reversal is again a $0$--$1$ polynomial.  The latter already suffices for prime phase rigidity.  Computational next checkpoint: $X=10^5$ (~2 h at current FLINT scaling).
