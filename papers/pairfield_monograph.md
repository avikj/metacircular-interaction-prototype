...
**P2 — Conjecture A″, non-cyclotomic part.** F2 classifies every cyclotomic factor globally and F3--F6 classify every factor degree through six, with exact certificates leaving no such factor for $X\ge13$.  The first open degree is septic.  What remains is to exclude non-cyclotomic factorization beginning in degree $7$, or prove the weaker exact condition that no nontrivial factor reversal is again a $0$--$1$ polynomial.  The latter already suffices for prime phase rigidity.  Computational next checkpoint: $X=10^5$ (~2 h at current FLINT scaling).
