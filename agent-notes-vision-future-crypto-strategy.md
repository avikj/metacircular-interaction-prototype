...
And the coinductive ISC is the **interactive face of the same generative grammar**: `respond` = fire an applicable rule/function; the guarded continuation `ISC … w'` = the rest of the generation. Generation and interaction are one object seen from two sides. Everything — expectancy, substitution (sthānivadbhāva, `AdeshaSthanivat_…`: "the substitute behaves like the original so every proven rule speaks at every instance"), the engine-never-touches-meaning invariant (`Alopa_TheEngineNeverTouchesTheMeaning`) — is the grammar. **Including the coinductive calculus**, because the coinductive calculus is the grammar generating unboundedly.
