...
# elaborated by Bend2 without a verdict (`--to-pusc-unchecked`) and checked by `pusc check`; the
...
TMP=${TMPDIR:-/tmp}/pusc-check.$$; mkdir -p "$TMP"
...
for f in "${files[@]}"; do
  ...
  if ! ( cd "$d" && timeout 120 "$BEND" "$b.bend" --to-pusc-unchecked > "$TMP/$b.pusc" 2>/dev/null ); then skipped=$((skipped+1)); echo "SKIP $b (no elaboration)"; continue; fi
  timeout 120 "$PUSC" check "$TMP/$b.pusc" > "$TMP/$b.out" 2>"$TMP/$b.err"
  ...
  else files_bad=$((files_bad+1)); echo "DIFF $b  ($a/$n agree; pusc reported $ngot)"; echo "$dis" | sed 's/^/  /' | head -6; [ -s "$TMP/$b.err" ] && head -c 200 "$TMP/$b.err" | sed 's/^/  ! /'; fi
done
