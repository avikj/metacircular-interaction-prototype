...
document.addEventListener('highlight', (({ detail: { link, on } }: CustomEvent) => {
  ...
  if (link instanceof Element && link.localName === 'a') {
    const anchor = link as HTMLAnchorElement;
    match = that => that.getAttribute('href') === anchor.getAttribute('href');

    requestAnimationFrame(async () => {
      try {
        const hover = await getHover(anchor);
        if (hover) {
...
export function refreshLinks(parent?: HTMLElement): HTMLAnchorElement[] {
  ...
  here.forEach(link => {
    if (link.namespaceURI === 'http://www.w3.org/1998/Math/MathML' && !('href' in link)) {
