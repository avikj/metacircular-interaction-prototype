...
module Kernel.WhatThisIsAndHowToDescendIntoTheMetacircularKernel where
...
open import Kernel.TheInstalledOperationHasNoneSoTheKernelMemorisesAndTheSchemaIsWhatMakesItGeneralise
  using (enabled-set-is-subsingleton ; no-native-operation-does-this
        ; SchematicOperation ; kernel-cannot-reach-a-tower)
open import Kernel.TheDerivationCarriesNoMeaningAtAllSoAllOfItIsRemainderAndNoSemanticCriterionSelectsTheShortOne
  using (every-semantic-criterion-is-blind ; cost-does-not-factor
        ; derivations-are-not-a-proposition)
open import Kernel.TheCountingSemanticsIsADecategorificationAndTheBitItDropsIsASymmetry
  using (comm-loop-is-a-nontrivial-loop-in-the-universe
        ; no-counting-criterion-separates ; derivation-equiv)
open import Kernel.TheWholeDerivationTypeIsOneFibreSoSoundnessIsNeverAnEquivalence
  using (the-whole-derivation-type-is-one-fibre
        ; soundness-is-not-an-equivalence-at-the-kernels-own-seed)
open import Kernel.TheKernelIsAnInteractiveSystemAndTheSessionRetiresIntoOneOperation
