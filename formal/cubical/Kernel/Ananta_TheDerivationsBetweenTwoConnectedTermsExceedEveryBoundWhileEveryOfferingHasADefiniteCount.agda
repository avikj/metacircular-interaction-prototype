...
-- exceeds every such bound.  ¬ß1-¬ß2 exhibit, between any two connected
...
--       downstairs, through `Sesa_‚¶`, forced by ‚ï being a set.
--
...
open import Cubical.Data.Nat
  using (‚Ñï ; zero ; suc ; _+_ ; +-suc ; injSuc ; znots ; snotz ; inj-+m)
...
------------------------------------------------------------------------
-- ¬ß1.  THE PADDING, AND IT IS UNIFORM.
--
...
inflate : (k : ‚Ñï) {a b : Tm} ‚Üí Derivation a b ‚Üí Derivation a b
inflate zero        d = d
inflate (suc k) {a} d =
  then-step (reverse (add-zero a)) (then-step (add-zero a) (inflate k d))

inflate-len : (k : ‚Ñï) {a b : Tm} (d : Derivation a b)
            ‚Üí len (inflate k d) ‚â° (k + k) + len d
inflate-len zero    d = refl
inflate-len (suc k) d =
    cong (Œª n ‚Üí suc (suc n)) (inflate-len k d)
  ‚àô cong (Œª n ‚Üí suc (n + len d)) (sym (+-suc k k))

------------------------------------------------------------------------
-- ¬ß2.  AND THE INDEX IS RECOVERABLE, so the family is not a repetition.
------------------------------------------------------------------------

double-inj : (k k' : ‚Ñï) ‚Üí k + k ‚â° k' + k' ‚Üí k ‚â° k'
...
double-inj (suc k) (suc k') p =
  cong suc (double-inj k k'
    (injSuc (sym (+-suc k k) ‚àô injSuc p ‚àô +-suc k' k')))

inflate-inj : {a b : Tm} (d : Derivation a b) (k k' : ‚Ñï)
            ‚Üí inflate k d ‚â° inflate k' d ‚Üí k ‚â° k'
inflate-inj d k k' p =
  double-inj k k'
    (inj-+m (sym (inflate-len k d) ‚àô cong len p ‚àô inflate-len k' d))

------------------------------------------------------------------------
-- ¬ß3.  THE STATEMENT.  One derivation between two terms produces, for
...
ananta : {a b : Tm} (d : Derivation a b) (k : ‚Ñï)
       ‚Üí Œ£[ e ‚àà Derivation a b ] (len e ‚â° (k + k) + len d)
ananta d k = inflate k d , inflate-len k d

------------------------------------------------------------------------
-- ¬ß4.  AND ALL OF IT IS ONE BIT DOWNSTAIRS.  Not by choice of `eval`:
--      `Sesa_‚¶` forces it from ‚ï being a set.  The order of the object and
...
the-whole-family-means-one-thing :
  {a b : Tm} (d : Derivation a b) (k : ‚Ñï) (œÅ : Env)
  ‚Üí derivation-sound (inflate k d) œÅ ‚â° derivation-sound d œÅ
the-whole-family-means-one-thing d k œÅ = soundness-is-constant (inflate k d) d œÅ
