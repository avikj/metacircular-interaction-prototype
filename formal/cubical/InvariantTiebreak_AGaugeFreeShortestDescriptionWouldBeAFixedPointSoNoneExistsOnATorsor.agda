...
-- proves, for the transporter `T x y = �[ g ] (g � x ≡ y)`:
...
--      is literally the same measure, so every R�nyi entropy H_�,
--      � ∈ [0,∞], agrees across the pair while the verdicts differ.
--
--      The size of a certificate ensemble does not decide whether a
--      symmetry-natural point exists: it does so on a torsor
--      (§2 + StabilizerTorsor) and only
...
module Invol {ℓx : Level} (X : Type ℓx) (f : X → X)
  ...
  -- generator.  This is the � � −∞ end of the R�nyi family made
