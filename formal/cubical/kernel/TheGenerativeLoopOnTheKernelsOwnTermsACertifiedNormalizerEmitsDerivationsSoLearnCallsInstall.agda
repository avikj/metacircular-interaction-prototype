...
------------------------------------------------------------------------
-- TheGenerativeLoopOnTheKernelsOwnTerms ‚î the corpus's largest structural
-- gap (descent doc ¬ß7: the proved-terminating generative learner runs on
...
-- (Siddhasadhana: self-installation is a plateau) ‚î it closes the
-- DISCOVERY‚íINSTALLATION loop on one language, which the corpus did not
-- have.  Item ¬ß8(a) of the descent doc, discharged in the buildable form.
------------------------------------------------------------------------
...
_‚ñ∏_ : {a b c : Tm} ‚Üí Derivation a b ‚Üí Derivation b c ‚Üí Derivation a c
done a        ‚ñ∏ e = e
then-step s d ‚ñ∏ e = then-step s (d ‚ñ∏ e)

infixr 5 _‚ñ∏_

sucD : {a b : Tm} ‚Üí Derivation a b ‚Üí Derivation (suc a) (suc b)
sucD (done a)        = done (suc a)
sucD (then-step s d) = then-step (suc-step s) (sucD d)

addLD : {a b : Tm} ‚Üí Derivation a b ‚Üí (z : Tm) ‚Üí Derivation (add a z) (add b z)
addLD (done a)        z = done (add a z)
addLD (then-step s d) z = then-step (add-left s z) (addLD d z)

addRD : (z : Tm) ‚Üí {a b : Tm} ‚Üí Derivation a b ‚Üí Derivation (add z a) (add z b)
...
addNorm : (l r : Tm) ‚Üí Œ£ Tm (Œª t ‚Üí Derivation (add l r) t)
...
norm (add l r)  =
  ...
  in t , (addLD dl r ‚ñ∏ addRD l' dr) ‚ñ∏ da
...
normalize-sound : (t : Tm) (œÅ : Env) ‚Üí eval t œÅ ‚â° eval (normalForm t) œÅ
