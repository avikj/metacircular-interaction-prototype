...
--
-- TERM.  ��������� � akapa -- "the net of digits", the traditional name for
...
-- SET (`Tm` for control, `�` for meaning), hence in a proposition, hence
-- carries zero bits.  So the diagnosis is not "the proofs are weak" but:
--
--     THE KERNEL'S SEMANTICS IS DECATEGORIFIED.
--     `eval : Tm � Env � �` keeps a cardinality and drops the bijection.
--
...
--   §4  THE SEPARATION.  Let � interpret `var` by `Unit`, and let
--       `comm-loop : Derivation� (add var var) (add var var)` be the single
--       commutation.  Then:
--
--         counting-semantics-cannot-see-it
--           its �-meaning is equal to `refl`, forced, and by `TheDerivationCarriesNoMeaning�` §4 NO
...
--       valued in a set annihilates it (��-�������).
--
...
-- proves no function of their �-MEANING tells them apart.  The natural
...
--
--     length     invisible to �, invisible to Type   (§5)
--     symmetry   invisible to �, VISIBLE to Type     (§4)
--
...
-- �-semantics cannot separate is not separated by the type semantics either.
