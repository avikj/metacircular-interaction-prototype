...
------------------------------------------------------------------------
-- TheWholeDerivationTypeIsOneFiberSoSoundnessIsNeverAnEquivalence
--
...
--   TheDerivationCarriesNoMeaning      RewriteCertificate.derivation-sound : D a b � eval a � ≡ eval b �
...
--
--   §1  fiber-is-everything.  If M is a proposition, then for EVERY m : M,
--       `fiber f m � A`.  The fiber is not a part of the domain; it is the
--       domain.  Nothing at all sits underneath it.
--
--   §2  Instantiated: `Derivation a b � fiber derivation-sound m`, for any
--       meaning m whatsoever.  The kernel's derivation type IS one fiber of
--       its own soundness.
--
--   §3  THE CONTRAST WITH THE ONE PRIMITIVE, which is the point of doing
--       this in this repository.  `fiber/src/Fiber/Carrier.agda`
--       is the fiber law: for f : A � B, BIND THE OUTPUT and the fiber is
...
--       fiber law: everything it computes is priced as loss, because its
...
--       every fiber contractible, nothing lost either way.  Here:
...
--       exact defect is the fiber of §2: the machine cannot recover its route
...
-- MANY, or HOW LONG.  Every quantity a policy would need lives in a fiber
...
module TheWholeDerivationTypeIsOneFiberSoSoundnessIsNeverAnEquivalence where
...
--
-- A map into a proposition has no fibers in the plural.  Every fiber is the
-- whole domain, so "which fiber am I in" is not a question the codomain can
...
fiber-is-everything {A = A} isPropM f m =
...
-- THE DERIVATION TYPE IS ONE FIBER OF ITS OWN SOUNDNESS.
the-whole-derivation-type-is-one-fiber :
  (a b : Tm) (m : Meaning a b) → fiber (sound a b) m ≃ Derivation a b
the-whole-derivation-type-is-one-fiber a b m =
  fiber-is-everything (isPropMeaning a b) (sound a b) m

-- Which fiber you name makes no difference, because there is only the one.
-- (Stated because "the fiber over m" reads as a choice, and it is not.)
the-choice-of-meaning-is-no-choice :
  (a b : Tm) (m n : Meaning a b) → m ≡ n
the-choice-of-meaning-is-no-choice a b = isPropMeaning a b

------------------------------------------------------------------------
-- §3.  BOTH BINDINGS OF THE FIBER LAW COINCIDE HERE.
--
...
-- where the fiber law puts the price of a computation -- is empty for this
...
both-bindings-agree a b m n =
  compEquiv (the-whole-derivation-type-is-one-fiber a b m)
            (invEquiv (the-whole-derivation-type-is-one-fiber a b n))
