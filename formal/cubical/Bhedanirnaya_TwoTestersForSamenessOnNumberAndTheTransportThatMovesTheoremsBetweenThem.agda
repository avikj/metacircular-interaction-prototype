...
--
--   PairField.ResidueGlue.eqℕ-sound        : (m n : ℕ) → eqℕ m n ≡ true → m ≡ n
...
-- clauses are two functions: `PairField.ResidueGlue.eqℕ m n` and
...
-- proved COMPLETENESS of its tester (m ≢ n → eqℕ m n ≡ false); PairField.ResidueGlue
-- never did, and has gone without it.  The path of §2 carries that theorem
-- backwards, and PairField.ResidueGlue's tester acquires a property it was never
...
समता : (m n : ℕ) → PairField.ResidueGlue.eqℕ m n ≡ NaturalMachine.Obstruction.eqℕ m n
समता zero    zero    = refl
...
एकीभावः : PairField.ResidueGlue.eqℕ ≡ NaturalMachine.Obstruction.eqℕ
एकीभावः i m n = समता m n i

------------------------------------------------------------------------
-- §3 · संक्रमणम् — the transport, forwards.
--
-- PairField.ResidueGlue's soundness theorem, carried along एकीभावः, becomes a
...
संक्रान्त-सौष्ठवम् =
  transport (λ i → (m n : ℕ) → एकीभावः i m n ≡ true → m ≡ n)
            PairField.ResidueGlue.eqℕ-sound
...
संक्रमण-तादात्म्यम् : संक्रान्त-सौष्ठवम् ≡ NaturalMachine.Obstruction.eqℕ→≡
संक्रमण-तादात्म्यम् =
...
--
--   · NaturalMachine.Obstruction holds COMPLETENESS, ≢→eqℕ-false, and uses
...
--     exactly why it proved completeness and PairField.ResidueGlue did not.
--   · PairField.ResidueGlue holds the Fin layer — eqFin, eqFin-sound,
...
-- PairField.ResidueGlue's tester acquires Obstruction's completeness by transport
-- backwards along the same path — a theorem it did not have, obtained with no
-- new induction and no edit to either module.
------------------------------------------------------------------------

संक्रान्त-पूर्णता : (m n : ℕ) → ¬ m ≡ n → PairField.ResidueGlue.eqℕ m n ≡ false
संक्रान्त-पूर्णता =
  transport (λ i → (m n : ℕ) → ¬ m ≡ n → एकीभावः (~ i) m n ≡ false)
            NaturalMachine.Obstruction.≢→eqℕ-false

-- The same, one level up: PairField.ResidueGlue's own eqFin now has a decision
-- procedure that is trustworthy on both answers, because its underlying eqℕ
-- is.  Stated at ℕ; the Fin lift is PairField.ResidueGlue's own eqFin-sound /
-- eqFin-complete pair and is not restated here.
पूर्ण-निर्णयः
  : (m n : ℕ)
  → (PairField.ResidueGlue.eqℕ m n ≡ true → m ≡ n)
  × (¬ m ≡ n → PairField.ResidueGlue.eqℕ m n ≡ false)
पूर्ण-निर्णयः m n = PairField.ResidueGlue.eqℕ-sound m n , संक्रान्त-पूर्णता m n
...
--   · The transport in §5 gives PairField.ResidueGlue's tester completeness at ℕ.
