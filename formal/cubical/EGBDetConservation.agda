...
-- cleanest sector: upper-triangular 2�2 matrices over �.
--
...
--   * the conservation law is pure � algebra:
...
-- NatSolver reflection tactic), (�, �, 1) is a monoid, and detUT is a
...
-- NEEDS subtraction, so over the rig � (no additive inverses) "det"
-- of a full matrix is not even defined � a�d − b�c is not an �-term.
...
-- b � 0 ≡ 0 (crossTermVanishes below), so the �-determinant,
-- restricted to the sector, collapses to the subtraction-free a�d
-- and the whole conservation law lives inside �.  The sector
...
-- The public entry point is the macro `solve�!`, re-exported by
...
------------------------------------------------------------------------
-- The carrier: upper-triangular 2�2 matrices over �
...
-- THE MORPHISM: detUT : (UT, mulUT, idUT) � (�, �, 1).
...
--
-- Over �, det [[a,b],[c,d]] = a�d − b�c.  Over � that subtraction is
...
-- and the �-determinant restricted to the sector IS the
