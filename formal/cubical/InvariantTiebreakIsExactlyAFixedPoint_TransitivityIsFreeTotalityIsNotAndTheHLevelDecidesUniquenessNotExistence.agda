...
-- layer of `MatraVrtta_�`.  j-2's module carries genuine *Chandastra*
-- provenance (Pigala, c. 300�200 BCE) for ITS object, the mtr-vtta;
...
--
--          InvariantTiebreak A �'   ⟺   Canonical A
--
--      (`tiebreakIffCanonical`), with �' the level of the carrier.  The
...
--      uses no hypothesis on � at all (`selectionIsFixedByAssumption`
--      reproduces it with no structure whatsoever).  For j-0 it is a
--      DERIVATION consuming `�-inv`.  And they are INCOMPARABLE: §7.2
--      instantiates j-2's own general `Formation.Symmetry` at a � that
...
-- `X = InvariantTiebreak A �R`, with `tiebreak�canonical` as the map.
j0-is-an-instance :
...
--
-- j-0 proves `InvariantTiebreak A �'' � Canonical A` for every �''.  The
...
--      carries `� (pick e n) ≡ pick e n` as a FIELD, so
...
--      `�-ev` and `�-c`; � need not be invertible, injective, or
--      surjective.  Instantiate it at a � that is none of those.  The
...
--    order the argument does not run: � acting on � by translation
--    carries the usual order � total, antisymmetric, transitive,
--    monotone � with no fixed point and no least element.  Not
--    formalized here (cubical v0.5 in this container has no order on �
