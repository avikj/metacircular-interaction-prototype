...
------------------------------------------------------------------------
-- ������� � the price readout and the symmetry readout are ONE FUNCTION,
...
-- points at � ��� / ��, debt and asset as two readings of one magnitude �
-- is Brahmagupta, *Brhmasphuasiddhnta* 18 (628 CE), carried at �����������
...
-- claim per unit, `��������` � carries no price signal.  Air is not cheap
...
-- where many claims collapse onto one thing, which is `���`, which is the
-- ������ coordinate.  So the price readout is the function "does contention
...
-- WHAT IS PROVED.
--
--   §2  `����-����` � the price readout and `Durnayah_�.��������-����`, the
--       symmetry readout, are the SAME FUNCTION.  Seven `refl`s, one per
--       bhaga.  Not analogous, not corresponding: equal.
--
--   §3  therefore `�������` gives price the same verdict it gave symmetry,
--       and computes it: `�����-������ = refl`, the middle disjunct.  Price
--       identifies ������-����� with ������-����������� � **the fully held and the
--       never asked read identically.**
--
-- Ordinary language already knows this and has one word for both cells:
-- *priceless*, ��������, said of what is beyond price and of what has no
...
-- are not UNDERPRICED � underpricing is a ������ phenomenon and would be
...
-- the signal is a function of one coordinate and `Saptabhangi.�������` says a
-- two-valued function of three seeds must merge two.
--
-- And it is the same defect as `MachineFiber_�§6`'s: an engine whose
...
------------------------------------------------------------------------
-- � � the price readout, per the model above.
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- � � so the merge is the same merge, computed.
------------------------------------------------------------------------
