...
--
--   * `NaturalMachine.RadixSymptoma`, module `Radix`, over an ARBITRARY
...
--
--   * `NaturalMachine.TransportDiv`, on the `Digits` chart:
--
--         modw n []      = 0 mod n
...
-- one module (`NaturalMachine.agda`, the index) against `TransportDiv`'s
...
module NaturalMachine.RadixResidueUnification (k : ℕ) where
...
open import Texts.TransportDiv k
open import NaturalMachine.WalkResidueBridge k using (∣→modw-zero ; decDivides)

import NaturalMachine.FutureBehavior as FB
import NaturalMachine.RadixSymptoma as RS
