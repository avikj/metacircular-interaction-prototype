...
-- The descent law.
--
--   f factors through q  ⟺  f is constant on the fibres of q.
--
...
module NaturalMachine.Descent where
...
module _ {A : Type ℓ} {B : Type ℓ'} {C : Type ℓ''} where
  ...
  -- f is constant on the fibres of q.
  ConstantOnFibres : (A → B) → (A → C) → Type (ℓ-max ℓ (ℓ-max ℓ' ℓ''))
  ConstantOnFibres q f = (a a' : A) → q a ≡ q a' → f a ≡ f a'

  -- One direction is unconditional.
  factors→constant : (q : A → B) (f : A → C)
                   → Factors q f → ConstantOnFibres q f
  ...
  module _ (isSetC : isSet C) (q : A → B) (surj : isSurjection q) where
    ...
    -- what constancy on fibres makes unnecessary: the value is
    -- 2-constant on the fibre, so it descends through the truncation.
    constant→factors : (f : A → C) → ConstantOnFibres q f → Factors q f
    ...
    isPropConstantOnFibres : (f : A → C) → isProp (ConstantOnFibres q f)
    isPropConstantOnFibres f =
      isPropΠ3 λ _ _ _ → isSetC _ _

    -- The descent law.
    descent : (f : A → C) → Factors q f ≃ ConstantOnFibres q f
    descent f =
      propBiimpl→Equiv (isPropFactors f) (isPropConstantOnFibres f)
