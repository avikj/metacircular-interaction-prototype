...
|---|---|---|
| `WalkFast` | "NOT DELIVERED: the payoff instances." `next-characterised` (the exchange rate trading `q ∤ cap m` for `q is a prime power`) is proved; `next-8 : next 8 ≡ 9` exhausts a 3.5 GB heap and "I do not yet know what forces it; the obvious suspect is the `with`-abstraction on `q ≟ next m`". | **STILL OPEN.** An earlier revision of this table read "closed by `WalkFastInstance`" — that was this index believing `WalkFastInstance`'s header, which claimed the exchange had been made. It had not: that module has never typechecked (its run exhausted a 3 GB heap and was killed, and the file reached the repository through an over-broad `git add`). Its header now carries the retraction. The only measured facts are that the generic machinery checks in 3 s and that adding `next-8` blows the heap, so the cost is in the application at m = 8. |
...
| `SequentialNormalizationObstruction` | A retained selected-event history cannot be projected to `BornDistribution₂` at all; "a future repair must retain the complete branch table". | **Closed** by `FullSequentialTableNormalization` (sufficiency only — that file makes no minimality claim). |
| `DigitTowerLimit` | Agda 2.8's `UnsupportedIndexedMatch`: the indexed `Vec` matches "will not compute when applied to transports". Warning retained rather than suppressed. | **Closed** by `DigitTowerFin` (28 warnings → 0 by moving to `Fin n → Digit`) and `DigitTowerFinLimit`. |
| `WalkJumps` | "WHAT REMAINS OPEN, stated plainly": the converse of §(c) is not proved here; §(b) is not formalised here either; no primality decision procedure. | **Closed** by `CoprimeSplitting` (§(c)⇒), `WalkBridge` (§(b)), `WalkPrimePowers` (the composition). Header not updated. |
| `CoprimeSplitting` | "WHAT REMAINS OPEN": the bridge between `IsLCM (range1 n)` and `LeastNonDivisor` — §(b) of the note — "is still not formalised". | **Closed** by `WalkBridge`. Header not updated. |
| `DefectCalculus` §7 | Names its own gap verbatim: "a genuine surjection needs the image quotient (a set-truncation) … what is proved here is the split case". | **Closed** by `EffectiveDescent`, which also reports that half the diagnosis was wrong — `SetQuotients` is not needed at all. |
| `DefectCalculus` §4 ledger | "T15.9 is not proved as 'subgroup' … packaging one here would be scope creep." | **Corrected** by `StabilizerSubgroup`: the parameter already *was* the group object (`Cubical.Algebra.SymmetricGroup`); nothing had to be packaged. |
...
| `WalkFastInstance` | Claims `next-8 ≡ 9`, `next-9 ≡ 11`, `next-10 ≡ 11` via antisymmetry in place of `WalkFast`'s `with`. **Never typechecked** — heap exhausted, run killed. | **unverified draft — do not cite or import** |
