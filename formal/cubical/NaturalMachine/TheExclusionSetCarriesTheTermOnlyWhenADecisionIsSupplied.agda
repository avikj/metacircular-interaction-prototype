...
------------------------------------------------------------------------
-- NaturalMachine.TheExclusionSetCarriesTheTermOnlyWhenADecisionIsSupplied
--
...
-- or else vacuous.
--
-- The modelling step — reading a
-- term's *apoha* as the family of things it excludes, indexed by the
-- SAME domain the terms live in — is what makes the
...
--       on `RatioIsTheInvariantAndLengthIsThePresentation`'s
...
module NaturalMachine.TheExclusionSetCarriesTheTermOnlyWhenADecisionIsSupplied where
...
open import NaturalMachine.RatioIsTheInvariantAndLengthIsThePresentation
...
--     cancellation the condition needed — the `RnaDhana_*` finding,
...
--
-- The Naiyyika reads §5 as the *pratiyogin* returning under another
