...
------------------------------------------------------------------------
-- Ubhayata � from both sides.
--
-- TERM.  ������, on both sides / in both directions.  An ordinary adverb,
...
--
--     add-suc-left-sound : eval (add (suc x) y) � ≡ eval (suc (add x y)) �
--     add-suc-left-sound = refl
--
-- because �'s addition recurses on its LEFT argument, so `suc a + b`
...
--
-- `refl`.  �'s addition recurses on the left, so `suc a + b` IS
