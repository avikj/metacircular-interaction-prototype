...
--     NaturalMachine.CoprimeSplitting: `two-primes→coprime-split` is the
--     factorisation statement in positive form, and
--     `leastNonDivisor-isPrimePower` is (⇒) itself.  Decidable
--     divisibility came in ten lines from `Cubical.Data.Nat.Mod`
--     (`dec∣` there); no valuation and no factorisation theory was
--     needed.  Its coprime-splitting core is the checked term
--     NaturalMachine.WalkForcing.leastNonDivisor-no-coprime-split.
--
--   * §(b) of the note -- that the walk installs exactly the jump points
--     -- is NaturalMachine.WalkBridge: cap is FLAT across the interval
--     the walk skips, and the step becomes a total function
--     `next : ℕ → ℕ`.  The composition of §(b) with §(c) in both
--     directions -- "the walk installs exactly the prime powers, in
--     increasing order" -- is NaturalMachine.WalkPrimePowers.
--
...
--     procedure is `NaturalMachine.PrimalityDecision.decIsPrime :
--     (n : ℕ) → Dec (IsPrime n)`, which dispatches on
...
--     `NaturalMachine.WalkFast.decIsPrimePower` (a least non-divisor is
--     a prime power, not a prime), plus `CoprimeSplitting.primeDivisor`,
