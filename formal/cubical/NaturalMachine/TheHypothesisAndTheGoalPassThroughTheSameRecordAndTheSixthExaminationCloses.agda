...
------------------------------------------------------------------------
-- सम-भार — equal seeing.  Compound built here; noā
-- source is claimed for the mathematics.
--
-- WHY EQUAL VISION.  If in a step-pervasion
-- the GOAL is processed by the record (श्रुत-विनिमयः) but the
-- HYPOTHESIS's faces are not — so when the record helpfully
...
module NaturalMachine.TheHypothesisAndTheGoalPassThroughTheSameRecordAndTheSixthExaminationCloses where
...
open import NaturalMachine.TheEldersLiveStoreCrossesAsReceivedTextAndNothingEntersOnAuthority
  using (आगमः ; अपचितम्)
open import NaturalMachine.TheCrossedRulesBecomeTheRecordAndTheSecondPassReachesWhatTheFirstCouldNot
  using (गुरु-शेषम्)
open import NaturalMachine.TheRecordSpeaksWithTheGrainAndTheThirdPassCrossesFurther
