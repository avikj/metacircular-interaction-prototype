...
------------------------------------------------------------------------
-- NaturalMachine.FiniteInformation
--
...
-- (`Image q = �[ y ∈ Y ] � fiber q y ��`) and the descent direction is
...
--
--     SideUsed y = �[ k ∈ C ] � �[ x ∈ X ] (q x ≡ y) � (c x ≡ k) ��
...
--
--     pick : (u : TargetFiber q t y) � �[ x ∈ X ] (q x ≡ y) � (t x ≡ fst u)
--
...
--   * v0.5 skew: `isEmbeddingFst�Prop` (Cubical.Data.Sigma.Properties)
--     is stated POINTWISE � `{u v : � A B} � isEquiv (cong fst)` � not
...
module NaturalMachine.FiniteInformation where
...
-- the same job and, being a � over a PROPOSITION, has the additional
-- property that its paths are paths in Y (�≡Prop below).
------------------------------------------------------------------------
...
-- so it is a � over a proposition, exactly as in Lean's subtype.
------------------------------------------------------------------------
...
module SideInformation
  ...
  -- (v0.5 skew: `isEmbeddingFst�Prop` is stated pointwise, with the two
