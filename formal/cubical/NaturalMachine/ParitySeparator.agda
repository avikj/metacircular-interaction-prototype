...
-- sign assignment � on the primes; a number is its multiset of prime
-- factors, so Ω(n) is a length.  Let �' be � with EVERY prime sign
-- flipped.  Then
--
--     val �' n  =  (-1)^{Ω(n)} � val � n,
--
-- so � and �' agree at every n of even Ω and differ at every n of odd Ω.
...
--
-- WHY THE FLIP IS THE RIGHT ADVERSARY.  � � �' is the gauge action of
...
-- below is abelian-group bookkeeping in �/2; the four-case proofs are
...
-- (-1)^n, i.e. the parity character of �.  `true` at even n.
sgn : ℕ → Bool
sgn zero    = true
sgn (suc n) = not (sgn n)
...
-- the transcripts of � and of flip � are not approximately equal, not
...
-- odd-Ω query separates, for every �.  So the barrier is not "nothing
