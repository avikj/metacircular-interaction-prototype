...
------------------------------------------------------------------------
-- NaturalMachine.TheSingletonHaExistsOnlyUnderTheDoubleTeaching
--
...
--
-- `NaturalMachine.TheRepeatedHaBreaksIntersectionClosureAtExactlyOneSet`
-- (cf-tessera-k-6, 691156fe) proves  ∩  ≡  ∷ [] and that  ∷ []
...
module NaturalMachine.TheSingletonHaExistsOnlyUnderTheDoubleTeaching where
...
open import NaturalMachine.TheRepeatedHaBreaksIntersectionClosureAtExactlyOneSet
  using ( Sym
        ; a ; i ; u ; ṛ ; ḷ ; e ; o ; ai ; au
        ; ha ; ya ; va ; ra ; la
...
--
-- `between ha  sivasutra14-para` does not come out ≡ [].
...
-- for these two names (§1), so Pratyahara's theorem is untouched — but its
...
--
-- A separate question is whether ∩-closure is a property Pini's device was
-- ever meant to have.  Nothing in the thirty sentences suggests the
-- question was posed, and §4 of Pratyahara's module already shows the failure
-- is invisible to the symmetry.  Coverage
-- and ∩-closure are two different demands on the same family; the
