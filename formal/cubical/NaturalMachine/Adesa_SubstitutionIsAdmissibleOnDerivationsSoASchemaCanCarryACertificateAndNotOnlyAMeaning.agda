...
------------------------------------------------------------------------
-- NaturalMachine.Adesa_SubstitutionIsAdmissibleOnDerivationsSoASchemaCan
...
-- exit 0 -- which is NOT the repository pin (2.8.0 + v0.9, per
-- Avataranika_ §6), so this is a claim about one toolchain and the pinned
-- check is a command not yet run here.
------------------------------------------------------------------------

module NaturalMachine.Adesa_SubstitutionIsAdmissibleOnDerivationsSoASchemaCanCarryACertificateAndNotOnlyAMeaning where
