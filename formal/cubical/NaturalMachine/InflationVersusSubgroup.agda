...
--
-- Exhaustiveness is not used by any statement below: `no-section`
-- quantifies over an ARBITRARY function `H2 → H4`, so it is a
-- statement about the model's H¹ as enumerated, and a larger hom-set
-- could only be a different model, not a hole in this one.
--
-- HEADLINE TERMS
