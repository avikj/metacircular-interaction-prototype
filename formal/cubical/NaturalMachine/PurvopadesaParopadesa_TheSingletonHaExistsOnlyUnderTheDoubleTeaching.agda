...
--       ‡‡‡ = ‡ ‚à [] ‚î the unnameable singleton itself.  ¬ß3 records that
...
-- ‚î that the extractor would report the vanished name as the empty
...
-- for these two names (¬ß1), so k-6's theorem is untouched ‚î but its
...
-- as given: nonempty, and equal to { ‡ } (k-6 ¬ß5).
...
--
-- Settles: the counterexample to ‚à©-closure is not incidental to the
-- string.  It is the trace of the repetition, and the repetition is what
-- Kielhorn I.27.2‚ì20 argues is forced.  The two pratyhras in k-6's
...
--
-- Does not settle: whether ‚à©-closure is a property Pini's device was
-- ever meant to have.  Nothing in the thirty sentences suggests the
-- question was posed, and ¬ß4 of k-6's module already shows the failure
-- is invisible to the symmetry.  The honest statement is that coverage
-- and ‚à©-closure are two different demands on the same family, that the
