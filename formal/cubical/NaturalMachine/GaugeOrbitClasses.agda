...
-- classical (the square-class group ℚ^×_{>0}/(ℚ^×_{>0})², whose F₂-
-- dual is the space of such functions). Only the checked statement,
-- and the scope correction of §6, are contributed. §7 proves the
-- concatenated form `val σ (m ++ (k ++ k)) ≡ val σ m`, which is the
-- core and avoids permutation machinery. * Nothing about Goldbach,
-- twin primes, W3, or `BARRIER.md` Problem 2. * `ChargeCriterion` is
-- NOT refuted. Every statement in it is true as written; §6 refutes
-- only an over-reading its wording invites.
--
...
module NaturalMachine.GaugeOrbitClasses where
