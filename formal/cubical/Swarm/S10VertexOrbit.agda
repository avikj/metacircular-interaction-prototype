...
--
-- Encoding.  A vertex pins m-1 = n coordinates at bounds: s of them at �,
-- u = n - s of them at β, with compensator c = -s� - uβ.  Writing a = � � 0
-- and b = -β � 0 (both naturals in any common additive scale) the vertex is
-- feasible, i.e. β � c � �, exactly when
--
--     s�a � (u+1)�b       (that is  c � �)
--     u�b � (s+1)�a       (that is  β � c)
--
-- which is `Feas s u` below.  Only the ordered-abelian-group structure of
-- the exponents is used, so � is not a restriction on the argument, only on
-- the scale in which � and β are expressed.  No real analysis enters: the
-- whole content is that the sequence s � c_s is arithmetic with common
-- difference exactly the width � - β of the feasibility window, so it
...
-- a = �  (the upper log-radius, � 0),  b = -β  (minus the lower one, � 0)
module Cage (a b : ℕ) where
  ...
  -- it is 0 ∈ [β, �].  At the step, comparing (u+1)�b with (s+1)�a decides
  -- whether the new pinned coordinate goes to the lower or the upper bound;
  -- �'s order is total, so one of the two always works.  This is the exact
  ...
  -- upper direction (� > 0, i.e. A > 1) a second feasible index can only
...
-- x � -x, so the cage (A, B) with (�, β) = (a, -b) becomes the cage
-- (1/B, 1/A) with (�', β') = (b, -a): the SAME module with a and b
...
--     A^{s+1} B^{n-s} = 1;
--   * in that case c_s = � and c_{s+1} = β, so the two indices name the
--     radius multisets  {�^s, β^u, �}  and  {�^{s+1}, β^{u-1}, β},  which
--     are the same multiset  {�^{s+1}, β^{u}}.
--
...
--
--   sharp cage  (A, B) = (�2, ���):  A^{s+1} B^{n-s} = 1 would need
--     2^{s+1} = �^{2(n-s)} with s+1 � 1.  If n = s the right side is 1 and
--     the left is � 2.  If n > s then �^{2k} = (L_{2k} + F_{2k}�5)/2 with
