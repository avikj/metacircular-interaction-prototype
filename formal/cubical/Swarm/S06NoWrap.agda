...
--
-- The finite Fourier projector (3.1)-(3.2) of that note computes the
-- pushforward of a graded coefficient along ℤ ↠ ℤ/q.  It recovers the
...
-- unbounded: at fixed modulus Q the separated support may be spread over
...
-- a bound on both entries bounds the gap
...
--    which is exactly injectivity of  (S ; mod q).
------------------------------------------------------------------------
...
∣<→≡0 : ∀ q d → q ∣ d → d < q → d ≡ 0
∣<→≡0 q d h lt with discreteℕ d 0
... | yes p = p
... | no ¬p = ⊥.rec (<-asym lt (m∣n→m≤n ¬p h))

------------------------------------------------------------------------
-- Theorem A (§3 of RATIONAL_PAIR_CHANNEL, generalised and proved):
-- an interval of length < q is separated.
------------------------------------------------------------------------
...
... | (zero  , e) = ⊥.rec (¬-<-zero (subst (0 <_) (fst (m+n≡0→m≡0×n≡0 (sym e))) qpos))
... | (suc c , e) = ∣ c , inj-m+ {q} e ∣₁

-- the residue obstruction:  q never divides  c·q + r  when 0 < r < q
q∤ : ∀ q c r → 0 < r → r < q → ¬ (q ∣ (c · q + r))
q∤ q zero r rpos rlt h =
  ¬m<m (subst (_< r) (∣<→≡0 q r (subst (q ∣_) refl h) rlt) rpos)
q∤ q (suc c) r rpos rlt h =
  q∤ q c r rpos rlt
    (∣-cancel-add q (c · q + r) (≤<-trans zero-≤ rlt ) -- 0 < q
...
--   occupies {B,…,2B-1}.  Disjoint, so S is separated modulo Q.
--   Diameter of S is k·Q + 2B - 1 = (k+1)·Q - 1, unbounded in k.
------------------------------------------------------------------------
...
module Family (b k : ℕ) where
  ...
  sep (false , i) (false , j) h =
    narrow→Sep S Q (λ _ _ → tt-narrow _ _) (false , i) (false , j) h
    where
      tt-narrow : (u v : Idx) → gap (S u) (S v) < Q
      tt-narrow = λ _ _ → low-gap i j   -- placeholder, replaced below
  ...
  diam =
      cong suc (gap-le 0 (base + b) zero-≤)
    ∙ cong suc (cong (_∸ 0) refl)
