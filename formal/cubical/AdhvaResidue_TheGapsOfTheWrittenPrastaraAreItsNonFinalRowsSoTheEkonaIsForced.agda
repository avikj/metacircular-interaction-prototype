...
------------------------------------------------------------------------
-- ����-���� � ����������� ������-��������� ���� ����-������� ����� �� �
-- (adhva-ea � the written extent of the prastra IS the census of the
--  remainders it carries.)
--
-- ����� (the verse this module is about), �������������, �������������� �.� :
--
--     ��������� ���������������� ���������� ������������ �
--     ��������������������� ����������� ��������������������� �
--
...
--    of a vtta one agula, and below it likewise an agula."
--
...
--   one agula, and the total is 2�sakhy − 1 because the last row has
--   no gap below it.  That subtraction � ������� � is what this module
--   proves is forced rather than conventional.
--
-- SOURCES, with dates, and what is and is NOT claimed of each.
--
--   �������������, �������������� �.���.�, the a-pratyaya chapter.  Text:
...
--   this module does not adjudicate.  �.� names the six pratyayas:
--   prastra, naa, uddia, eka-dvy-di-laga-kriy, sakhyna,
--   adhva-yoga.
--
--   �������, �����������������, commentary (GRETIL kvrtrsuu.htm, from a
--   Patan manuscript, input by Dhaval Patel).  On �.� it works the
--   four-syllable class: *caturakarajtau y oaasakhy syd
--   dvigu dvtriati ekon ekarahit ekatriati* � sakhy 16,
--   doubled 32, less one 31.  On �.� it computes the other two numbers
--   used below: the laga-kriy row 1+4+6+4+1 = 16, and the uddia
--   doubling column 1+2+4+8 = 15, *saika oaa*, plus one 16.
--
--   The next-row rule, �.���.�, supplies the word this module is named
...
--
--   ���-�������   ����� n � (Unit � ���� n)
--                 Every row of the prastra is either the last one �
--                 ����-����, Kedra's stopping condition *yvat
--                 sarvalaghur bhavet* � or is RECOVERED from the ea
--                 it carries into the next row.  A bijection, not a
--                 count.
--
--   ����-�������   (����� n � Unit) � (����� n � ����� n)
--                 Verse �.� as a type equivalence, with no subtraction
...
-- warnings.  ����� and ���� are both defined BY RECURSION ON � rather
...
------------------------------------------------------------------------
-- �������� � ����� �� ������ �  The whole alphabet of the prastra: Kedra
...
------------------------------------------------------------------------
-- ���� � that which "������� ��� �����" carries down unchanged.
--
...
-- n-syllable prastra are exactly the patterns of length < n � and
...
--
-- At n = 0 there is no ea at all: a one-row prastra never steps.
------------------------------------------------------------------------
...
-- ��� � the reading-off of �.���.�.
--
--   ����� , v  � the first syllable IS the first guru.  It flips to
--               laghu, nothing precedes it to fill with guru, and v is
--               carried whole: the ea is v.
--   ���� , v  � the first syllable is already laghu, so the first guru
--               lies further in; recurse.  If v holds no guru then
--               neither does ���� , v; the row is ����-���� and *yvat
--               sarvalaghur bhavet* ends the prastra � no next row and
...
------------------------------------------------------------------------
-- �����-���� � the inverse: rebuild the row from what it carries.
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- ����� � the written extent: one agula per row, one per gap below a
-- row.  �.� measures exactly those two things, so the type of
...
------------------------------------------------------------------------
-- ����-������� � verse �.� with no subtraction in it:
--
--     (����� n � Unit) � (����� n � ����� n)
--
...
-- the ����-���� row, which is precisely the row with no gap under it.
-- So ������� names a specific missing object, not an off-by-one.
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- ����� � the same three quantities as numbers, so that Sulhaa's
-- worked instance can be checked against them.
--
--   ������� n      = 2�, the count of �.�.
--   ���-����� n   = the uddia doubling column 1+2+�+2��� summed �
--                   �.�'s SECOND derivation � which is also the
--                   length-graded census of the eas, one term 2� per
--                   ea-length k.  �.� sums this column to reach the
...
------------------------------------------------------------------------
-- �.� and �.� at the level of numbers, again with no subtraction:
...
------------------------------------------------------------------------
-- ��������� ��������� � the four-syllable class, the instance the
...
------------------------------------------------------------------------
-- �������-�������� � the control.  Is the "less one" doing work?
--
-- If gaps and rows were equinumerous, �.� would read *dvigu* with no
-- *ekon*.  They are not, and one syllable refutes it: there are two
-- one-syllable rows, ����� and ����, and exactly one gap between them.
--
...
-- what comes out is ���� ≡ �����, refuted by a discriminating family.
--
-- Discrimination happens on ��������, reached through fst.  Matching on
-- ����� 1 directly would need injectivity of suc, which Cubical Agda
-- rejects; here ����� 1 REDUCES to �������� � Unit, so fst is honest.
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- �-�������� � non-vacuity.  The types above are inhabited and have more
