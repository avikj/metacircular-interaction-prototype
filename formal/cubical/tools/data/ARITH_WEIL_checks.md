...
Notation as in the note: q = 4�1_[0,1/4], f = e^{-4s}(q*q), g = f*f�, λ_k = 2k + 1/2, x_j = j/4,
...
g(t) = (f*f�)(t) = � f(s) f(s−t) ds.  For t ∈ [0,1/4] the integrand changes form at s = 1/4 and s = t+1/4, so
...
**g = �_j b_j E(t − x_j).**  Resolving |t − x_j| on each of the three sign-patterns (0<t<1/4), (1/4<t<1/2),
(t>1/2), the stencil sum equals the piecewise formulas of §1 on the first two ranges and is identically 0 on
the third [PASS �3]; by evenness the same holds for t < 0.  Therefore (D²−16)² g = �_j b_j δ_{x_j}, i.e. (5.1).
...
Consequently, using g = �_j b_j E(� − x_j) and �_0^∞ e^{−λu}[g(t−u)+g(t+u)]du = �_j b_j (e^{−λ|�|}*E)(t−x_j):
...
**Expansion of the archimedean kernel.**  e^{−u/2}/(1−e^{−2u}) = �_{k�0} e^{−(2k+1/2)u}, u > 0; monotone
convergence justifies exchanging �_k and �_0^∞ termwise because the bracket 2g(t)−g(t−u)−g(t+u) is O(u²)
...
(residues A = 1, B = C = −1/2, A+B+C = 0 [PASS]).  For A+B+C = 0 the sum telescopes:
  �_{k�0}[A/(k+a) + B/(k+a−2) + C/(k+a+2)] = B(1/(a−2) + 1/(a−1)) − C(1/a + 1/(a+1))
(partial sums to K equal this plus (A+B+C)�_{k�K}1/(k+a) − B[1/(K−1+a)+1/(K+a)] + C[1/(K+1+a)+1/(K+2+a)]).
...
(−2−x)/(32(x−2)³) at x = −2 are both 0).  So the sum is (1/256)[�_{k�0}(k+a−2)^{−2} − �_{k�0}(k+a+2)^{−2}]
...
�_{k�0}(k+a−2)^{−2} = 1/(a−2)² + 1/(a−1)² + ψ�(a) = 16/49 + 16/9 + ψ�(1/4);
�_{k�0}(k+a+2)^{−2} = ψ�(a) − 1/a² − 1/(a+1)² = ψ�(1/4) − 16 − 16/25;
ψ�(1/4) = �² + 8G (Catalan) [PASS, 55 digits];
−2�_{k�0} 1/((k+a−2)(k+a+2)) = −½[1/(a−2)+1/(a−1)+1/a+1/(a+1)] = −½(−4/7−4/3+4+4/5) = −152/105 [PASS].
Hence H(0) = (1/256)[2(�² + 8G) + 16/49 + 16/9 − 16 − 16/25 − 152/105], which is (5.4).
...
e^{−λ_k d} = e^{−λ_K d} e^{−2(k−K)d}.  Hence 0 � �_{k�K} e^{−λ_k d}/(λ_k²−16)² � e^{−λ_K d}/(λ_K²−16)² �
�_{m�0} e^{−2md} = e^{−λ_K d}/((λ_K²−16)²(1−e^{−2d})).  The note's K � 3 is therefore (more than) sufficient.
[PASS; numeric d = 0.3, K = 3: tail 2.38e−4 � bound 4.58e−4]

## 6. Closed forms used by the evaluator

ψ(1/4) = −γ − �/2 − 3 log 2 [PASS, 55 digits]; the Arb implementation additionally checks this closed form
...
nontrivial zeros (Arb `acb.zeta_zeros`), Z = �_{γ>0} 2|F(iγ)|² cos(γt).  Crude truncation bound 7.5e−8.
...
| g = � b_j E(�−x_j), zero outside support | PASS | symbolic |
| (5.1) stencil, incl. jump values b_j | PASS | symbolic |
| per-λ integral identity | PASS | symbolic (λ≠4) + numeric |
| � −32/(λ_k(λ_k²−16)) = 352/105 | PASS | telescoping + 60-digit numeric (SymPy `summation` wrong) |
| � λ_k/(4(λ_k²−16)²) = 3229/44100 | PASS | symbolic + telescoping + numeric |
