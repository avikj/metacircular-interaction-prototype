...
-- fibre. §१ is that phenomenon with nothing else attached.
--
-- §२ is why Piṅgala's numbers ADD across such a split. If the two
-- fibres are finite — `fiber f c ≃ Fin m` and `fiber g c ≃ Fin n` — then
--
--     §२   fiber विकल्पः c  ≃  Fin (m + n).
--
-- This is §१ composed with the library's `⊎-equiv` and
-- `Fin+≅Fin⊎Fin`: the two receipts, chained, turn the twofold of fibres
...
-- fibre theorem below, or worked with fibres, coproducts or Fin at all.
-- The term names the SHAPE — a map given by a twofold alternative — and
-- nothing more. The mathematics (the fibre of `rec`, its equivalence,
...
module SourcedProofs.Vikalpa_TheFibreOfATwofoldMapIsTheTwofoldOfItsFibresAndSoTheCountsAdd where
...
------------------------------------------------------------------------
-- १ · विकल्प-तन्तुः — the fibre of the case-map is the case of the fibres.
...
------------------------------------------------------------------------
-- २ · विकल्प-गणना — two finite receipts on the two fibres flatten to one
