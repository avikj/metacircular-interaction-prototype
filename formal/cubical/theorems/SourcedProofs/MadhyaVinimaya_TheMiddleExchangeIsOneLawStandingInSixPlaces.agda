...
--     Primes.PairField.PairComposition.interchange           (Brahmagupta's bhāvanā, split legs)
--
--   (a + b) + (c + d) ≡ (a + c) + (b + d)
...
-- §3 makes this exact: Primes.PairField.PairComposition.prodComp — the module's statement
...
--     the split (leg) coordinates Primes.PairField.PairComposition works in, the surviving
...
module SourcedProofs.MadhyaVinimaya_TheMiddleExchangeIsOneLawStandingInSixPlaces where
...
import Vargana
import Primes.PairField.PairComposition
import MeruKarna
...
भावना-तादात्म्यम्
  : (a b c d : ℕ) → Primes.PairField.PairComposition.interchange a b c d ≡ मध्य-विनिमयः-गुणे a b c d
...
घातगुण-भावना-तादात्म्यम्
  : (a b c d : ℕ) → Vargana.चतुर्-विनिमयः a b c d ≡ Primes.PairField.PairComposition.interchange a b c d
घातगुण-भावना-तादात्म्यम् a b c d = isSetℕ _ _ _ _

-- And the same identification one level up, at the statements the two
-- modules actually use.  Primes.PairField.PairComposition.prodComp p q — "the composed
...
भावना-प्रयोगः-तादात्म्यम्
  : (u₁ v₁ u₂ v₂ : ℕ)
  → Primes.PairField.PairComposition.prodComp (u₁ , v₁) (u₂ , v₂)
...
-- independently: सङ्घातः at (ℕ,·) IS Primes.PairField.PairComposition.prod, ⊛ IS its
-- compose, and युग्म-धर्मः IS prodComp.
युग्म-धर्मः-भावना-तादात्म्यम्
  : (p q : ℕ × ℕ)
  → युग्म-धर्मः _·_ ·-assoc ·-comm p q ≡ Primes.PairField.PairComposition.prodComp p q
...
--   · Primes.PairField.PairComposition enters from भावना, where the medial step is used
...
-- Primes.PairField.PairComposition says as much in its own header when it records that in
