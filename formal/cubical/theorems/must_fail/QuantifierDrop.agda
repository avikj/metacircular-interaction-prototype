...
--  Â§"Version-skew notes (v0.9 migration, 2026-08-14)" records 2.6.3 +
...
--  `ReachabilityWithoutStart`) each label the same numbers correctly â”
--  as "container toolchain â¦ BUILD.md pins 2.8.0 + v0.9, check
...
--
--  PIN CHECK: NOT OUTSTANDING â” it was already done, by someone else,
...
--  failing at **80.26-41** with `rollover (val s + 0 Â val s) != mod5 â¦`
--  â” the same line and the same [UnequalTerms] site as the container
