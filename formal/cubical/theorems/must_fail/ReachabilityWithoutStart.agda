...
-- `3` are unreachable from the DFA start" â” without the premise
-- `start = 0`.  Asserted below: the conclusion of
-- `ReachableFromStart.closed-from-start` with its
-- `st â‰¡ s0` argument deleted, exactly as the sentence deletes it.
--
...
-- merely unproved but false: `run s1 (false âˆ []) â‰¡ s3`
-- (`escape-from-s1`), and `dropped-premise-false` in the companion
-- module derives âŠ from precisely the type asserted here.
--
-- This instruments a defect with NO LEXICAL SIGNATURE.  The message's
-- sentence contains no wrong word â” it contains one fewer clause than
-- its "so" requires â” so no grep can find it, and a type can.
--
...
-- Â§6.1), `LC_ALL=C.UTF-8 agda --library-file=<v0.9>
...
--   error: [UnequalTerms]
--   st != s0 of type S
--   when checking that the expression refl has type st â‰¡ s0
--
-- Read it: what the theorem still wants, and what the shortened
-- sentence does not supply, is `st â‰¡ s0` â” the start-state premise,
...
-- readmitted exactly the defect FULL_READ_DRAW_6 Â§B1 catalogued.
------------------------------------------------------------------------

module ReachabilityWithoutStart where
...
closed-without-start : (st : S) (w : List Bool) â†’ run st w â‰¡ s0
