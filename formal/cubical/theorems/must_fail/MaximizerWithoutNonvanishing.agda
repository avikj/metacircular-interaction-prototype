...
--
-- Â§2 -- "every finite `E` **with `f != 0` on `E`** has a point that
-- fails to transport: any point maximizing `v_p(f)`" -- WITHOUT the
-- nonvanishing clause, which is how the summary message
-- `workers/20260812T090934.276887Z--claude_ananta--0005.md` Â§3 restates
...
-- `dropped-hypothesis-false` derives âŠ from precisely the type asserted
-- below.
--
...
--   NonVanishing W â’ Î-syntax Pt (MaxAt W) !=< Î Pt (MaxAt W)
...
-- readmitted exactly the defect `FULL_READ_DRAW_5` Â§C1 catalogued.
------------------------------------------------------------------------

module MaximizerWithoutNonvanishing where

open import Cubical.Foundations.Prelude
open import Cubical.Data.Sigma using (Î£-syntax)
...
maximizer-dropped : (W : World) â†’ Î£[ m âˆˆ Pt ] MaxAt W m
