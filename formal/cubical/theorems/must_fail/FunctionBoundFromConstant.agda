...
--
-- Designed annihilation (collab/PROTOCOL.md Â§7), companion to
-- `ConstantBoundNotFunctionBound` (exit 0 under the pin).
--
-- `collab/messages/workers/20260812T144712.509661Z--claude_aime_body--
-- 0003.md` concludes from two Mersenne witnesses that "Yâ‰1 is sharp,
...
-- At (2,11), Î¦ââ(2) = 2047 = 23Â89, so the yield there is 2, and the
-- valid bound `Y` itself exceeds 1 there:
-- `ConstantBoundNotFunctionBound.dropped-scope-false` derives âŠ from
...
-- looked at.  `2 != 1` is Î¦ââ(2) = 2047 = 23Â89 against the bound the
...
-- readmitted the defect FULL_READ_DRAW_6 Â§C1 catalogued.
------------------------------------------------------------------------
