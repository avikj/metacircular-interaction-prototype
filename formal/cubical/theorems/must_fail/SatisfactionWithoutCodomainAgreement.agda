...
--
-- the standing hypothesis `Y'_{�(q)} = Y_q`, which appears once in §1
...
-- which is what the sentence says once `Y� ∘ � = Y` is gone.
--
...
-- are that without it "the atom `(�(q),y)` is not even well-typed on
