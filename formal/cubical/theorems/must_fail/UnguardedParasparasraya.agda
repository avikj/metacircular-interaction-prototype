...
-- stream's head is produced BEFORE the leaning (`iras` is defined and
-- `eam` refers across), so every observation depth is answered.  The
...
-- head ever produced.  parasparraya in the tradition's own sense.
mutual
  śūnya : Dhārā Bool
  śūnya = śeṣam riktā
