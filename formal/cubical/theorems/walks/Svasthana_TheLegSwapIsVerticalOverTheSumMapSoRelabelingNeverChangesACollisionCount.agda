...
------------------------------------------------------------------------
-- स्वस्थान — one's own station.  The graded fiber tower of the sum
-- spectrum (owner's three-multiplicity message), structural half:
--
--   level 0   frequencies s                    (the base)
--   level 1   pair-witnesses (γᵢ, γⱼ) over s = γᵢ + γⱼ   (the fibers)
--   level 2   relabelings between witnesses    (paths within a fiber)
--
...
-- hence restricts to an involution of EACH fiber of the sum map.  A
...
-- transport within a fiber, and interference counts fibers' sizes.
--
-- CHECKED (over ℤ pairs, the sum map σ (a,b) = a + b):
--   §1  swap covers the identity:  σ ∘ swap ≡ σ  (pointwise, +-comm).
--   §2  hence swap RESTRICTS to each fiber:  fiber σ s → fiber σ s,
--       an involution (swap² ≡ id, lifted to the fiber with its witness).
--   §3  the induced action on the base is the identity — the swap is
--       vertical, level 2 acts on level 1 over a FIXED level 0.
--
-- FENCE.  The weighted count (that |fiber| ≥ 2 with weights gives the
...
------------------------------------------------------------------------
-- §2 · THE SWAP RESTRICTS TO EACH FIBER.  A witness over s is carried to
-- a witness over the SAME s: level 2 acts within a station.
Fiber : ℤ → Type
Fiber s = Σ[ p ∈ Pair ] (σ p ≡ s)

swapF : (s : ℤ) → Fiber s → Fiber s
swapF s (p , e) = swap p , vertical p ∙ e

-- the restricted action is still an involution — on the underlying
-- witness, on the nose.
swapF² : (s : ℤ) (w : Fiber s) → fst (swapF s (swapF s w)) ≡ fst w
swapF² s (p , e) = swap² p
...
-- collision count is a function of the fiber, and the swap never leaves
-- the fiber.  Level 2 cannot reach level 0.
induced-identity : (s : ℤ) (w : Fiber s)
