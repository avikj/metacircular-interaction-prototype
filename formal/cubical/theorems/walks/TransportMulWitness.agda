...
-- TransportMulWitness
--
-- The digit multiplier of `TransportMul`, RUNNING, at
...
--
-- A digit is an element of `Fin 10 = �[ i ∈ � ] (i < 10)`, and in
-- cubical `i < n` unfolds to `�[ j ∈ � ] j + suc i ≡ n`, so the second
-- component of `d�` is the numeral `9 − i` and the third is `refl`.
...
open import Digits 8 using (Word ; Digit ; value ; b)
open import TransportMul 8 using (mulw ; shiftw ; repAdd)
...
------------------------------------------------------------------------
-- and the chart map agrees with � on what was computed
------------------------------------------------------------------------
...
module Base2 where
  open import Digits 0 using () renaming (Digit to Bit ; value to value₂ ; b to b₂)
  open import TransportMul 0 using () renaming (mulw to mulw₂ ; shiftw to shiftw₂)
