...
--     `Samorderna_TransportCarriesStructureAndTruncationâ¦` â” path one:
...
--   [Was "is exactly"; only `Writable âŸ MP` is proved.  See Â§2.]
--
...
--  proved below is `writableâ’MP` and only that: Writable âŸ MP.  That is a
--  LOWER BOUND -- writing defects costs AT LEAST MP -- and it is not an
--  identification.  `MP â’ Writable` is not proved here and is not proved
...
--  carries `MPâ’Witnessed` AND `Witnessedâ’MP`, both checked, and
...
--  terminating unbounded search, and LEM does not repair that (Â§3).  What
...
--  statement about `â• â’ Bool`, while `Writable` quantifies over all types
...
-- The reading of Â§à under audit: that path two is AVAILABLE whenever
