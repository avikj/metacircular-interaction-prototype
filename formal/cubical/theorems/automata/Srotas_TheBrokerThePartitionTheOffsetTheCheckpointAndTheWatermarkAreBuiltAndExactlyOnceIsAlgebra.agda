...
-- depends only on the SET of records delivered (§�).  Exactly-once is
...
-- WHAT IS CHECKED
--
--   §�  `Key`, `Delivery`   partitions and offsets; a delivery is a
--                           LIST, so duplicates and reordering are
--                           representable rather than assumed away.
--   §�  `consume`           the consumer's state, and `consume-is-fold`
--                           that it IS the idempotent merge � the merge
--                           is a theorem about the consumer, not its
--                           definition.
--   §�  merge laws          idempotent, commutative, associative, with
--                           the empty state its unit.
--   §�  `same-set�same-state`  EXACTLY ONCE, BY ALGEBRA.  Two deliveries
...
--   §�  `no-safe-compaction`  and the cost, exactly: dropping a
...
------------------------------------------------------------------------
-- � � the merge laws the whole design rests on.
------------------------------------------------------------------------
