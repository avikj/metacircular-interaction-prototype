...
--       already-checked NerodeCongruence (MyhillNerodeMinimalMachine).  So
...
------------------------------------------------------------------------
-- §2  The set form: T18.6 is NerodeCongruence
--
...
-- different name, as the centre of MyhillNerodeMinimalMachine: with
...
module SetForm {X : Type ℓ} {A : Type ℓ} {O : Type ℓ}
  ...
  -- The pointwise form, which is how MyhillNerodeMinimalMachine states NerodeCongruence.
  nerodeCongruence : X → X → Type ℓ
  nerodeCongruence x y = (w : List A) → observe (evolve x w) ≡ observe (evolve y w)
  ...
  obsKernel→nerodeCongruence : {x y : X} → obsKernel x y → nerodeCongruence x y
  obsKernel→nerodeCongruence p w i = p i w

  nerodeCongruence→obsKernel : {x y : X} → nerodeCongruence x y → obsKernel x y
  nerodeCongruence→obsKernel h = funExt h
  ...
  nerodeCongruence→now : {x y : X} → nerodeCongruence x y → observe x ≡ observe y
  nerodeCongruence→now h = h []
  ...
  separated→¬nerodeCongruence :
      {x y : X}
    → Σ[ w ∈ List A ] (observe (evolve x w) ≡ observe (evolve y w) → ⊥)
    → nerodeCongruence x y → ⊥
  separated→¬nerodeCongruence (w , sep) h = sep (h w)
