...
--
--
-- that it IS safe. That every safe relation is contained in it is
-- `ExtremalDescription.greatest-safe` (three lines, by induction on
-- `n`, using exactly the `iterT` bracketing §1 chose). With it, §3's
-- witness sharpens: `ExtremalDescription.instant-not-invariant` shows
-- `ker P` fails safety at the INVARIANCE clause, not at soundness.
-- Pointer comment only; nothing here edited.
--
...
module ObservabilityQuotient where
...
open import CompressionDefect using (module Observability)
