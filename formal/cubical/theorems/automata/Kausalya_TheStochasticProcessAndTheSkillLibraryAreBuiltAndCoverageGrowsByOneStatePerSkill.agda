...
-- learner: §� proves the covered set of a library is contained in the
-- training states of its members, so CAPABILITY GROWS BY AT MOST ONE
-- STATE PER SKILL, and §� proves that no finite library covers an
...
-- proved to be the Dirac special case (§�), and the SKILL LIBRARY.
--
-- WHAT IS CHECKED
--
--   §�  `Dist`, `expect`, `dirac`   finite distributions, �-weighted.
--   §�  `SMDP`, `sret`              the stochastic process and its
--       `sret-det`                  return; the deterministic process
--                                   embeds, with equal returns.
--   §�  `Trace`, `len`, `two-traces`  routes, and that a route is not
--                                   determined by its endpoints.
--   §�  `Memo`, `enabled-isContr`   THE MEMORISING SKILL FIRES AT
--                                   EXACTLY ONE STATE: its enabled set
--                                   is contractible, centred on the
--                                   training state.
--   §�  `library-coverage`          so a library's covered set lies
--                                   inside its training states.
--   §�  `library-misses`            and no finite library covers � �
--                                   the missed state is constructed.
--   §�  `no-trace-from-endpoints`   the generalising form cannot be
...
------------------------------------------------------------------------
-- � � finite distributions, weighted by counts.
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- � � the STOCHASTIC decision process, and the deterministic one inside it.
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- � � routes, and that the endpoints do not determine one.
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- � � THE MEMORISING SKILL, AND WHERE IT FIRES.
--
...
------------------------------------------------------------------------
-- � � SO COVERAGE GROWS BY AT MOST ONE STATE PER SKILL.
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- � � AND NO FINITE LIBRARY COVERS AN INFINITE STATE SPACE.
--
...
------------------------------------------------------------------------
-- � � AND THE GENERALISING FORM CANNOT BE INSTALLED.
--
