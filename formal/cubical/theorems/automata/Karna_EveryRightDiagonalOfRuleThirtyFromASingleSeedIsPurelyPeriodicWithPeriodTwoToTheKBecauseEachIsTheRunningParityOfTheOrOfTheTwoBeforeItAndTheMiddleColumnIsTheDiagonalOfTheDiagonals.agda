...
-- is PURELY periodic with period dividing 2^k (Theorem R).  The centre
-- column is the diagonal of the diagonals, c t = d t t (§5): it samples
-- d t once, at a time no larger than its period 2^t can have elapsed, so
-- the periodicity of every diagonal gives the column nothing.  That is
...
-- computed periods 1,2,2,4,8,8,16,32,32,64,�); submitted to the machine
-- (sadhana.patra) and accepted by its kernel, 2026-09-11.
------------------------------------------------------------------------

module Karna_EveryRightDiagonalOfRuleThirtyFromASingleSeedIsPurelyPeriodicWithPeriodTwoToTheKBecauseEachIsTheRunningParityOfTheOrOfTheTwoBeforeItAndTheMiddleColumnIsTheDiagonalOfTheDiagonals where
