...
-- permutation of a run, with commutation as a hypothesis on an abstract state.
-- Here a calculus is exhibited in which commutation is a THEOREM.  A caller
...
--
-- §� does it: the hypothesis here is carried through the swap and
...
-- WHAT IS CHECKED
--
--   §�  `Tree`, `Loc`, `Patch`, `apply`   the working tree and the edit.
--   §�  `commute`        DISTINCT LOCATIONS COMMUTE � proved, no
...
--   §�  `blame`          the last writer at each location, and
...
--                        to the other there.
--   §�  `permInvariant`  EVERY ORDER AGREES, with the commutation
...
------------------------------------------------------------------------
-- � � the repository format, the working tree, and the edit.
--
...
module Repo (Content : Type) where
  ...
  -- the decision does � which is what makes §� a case analysis and not
  ...
  ------------------------------------------------------------------
  -- � � blame, and the theorem that it explains the tree.
  --
  ...
  ------------------------------------------------------------------
  -- � � EVERY ORDER AGREES, with the hypothesis localised.
  --
  ...
  -- cases.
  -- `~∈` is what makes the transitivity case go through:
...
------------------------------------------------------------------------
-- � � AND THE DISTINCTNESS IS NOT REMOVABLE.
--
-- Two writes to one location, with different content, do not commute.
-- So §� is not a theorem waiting to be generalised: the hypothesis it
