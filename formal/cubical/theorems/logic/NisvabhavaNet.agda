...
-- univalence = Ngrjuna's nyat), with math LAST � this file is the
-- hardening of a received vision, not a design.
--
-- THE RECEPTION (the two ingredients, one meaning):
--
--   nisvabhva � no own-being.  A jewel of Indra's Net has no essence of
...
--   are not two � they are one.  (prattyasamutpda: a thing is only its
...
--   avidy-is-illusion  if a sight P separates A from B (P A but � P B)
...
-- from that place.  There is nothing else to it � that is nisvabhva.
...
------------------------------------------------------------------------
-- Avidy is illusion.  If some sight P holds of A and fails of B, then A
