...
-- ObsBridge
--
-- `àà•àà•à°à â‰  àà®à¾à¨àà¾;  àà•àà•à°à = ààà¦à°à•ààà¿à àà¨ààµà¾à¦àà¾à²à®à`
--
...
--
--   ChuAdvance.Obs X T  =  X â’ T â’ Bool          -- Bool-valued, finite tests
--   ObstructionCalculus.Obs X V                  -- V-valued, indexed family
--
-- Neither subsumes the other.  Bool-valuedness is what makes
-- `Shrink(ð’¯) â’ Î´â“` statable over a finite test list; arbitrary `V` is what
-- lets the sign defect be read by `absâ` and the identity, which are â-valued
...
--
-- `àµà¿àµà¾à¦ = àà®ààà¾àµà¿à àààžà¾à¨àà‹à²à‹à¨à‰à®à`.  Two presentations disagreed; the response
-- is to measure the map between them, not to erase one.
------------------------------------------------------------------------

module ObsBridge where
...
open import ObstructionCalculus using (Obs ; Sep)
open import ChuAdvance using () renaming (Obs to ChuObs)
...
decBool : {A : Typeâ‚€} â†’ Dec A â†’ Bool
decBool (yes _) = true
decBool (no _) = false

decBool-refl : (dec : Discrete V) (v : V) â†’ decBool (dec v v) â‰¡ true
decBool-refl dec v with dec v v
... | yes _ = refl
... | no Â¬p = Empty.rec (Â¬p refl)

decBool-neq : (dec : Discrete V) (u v : V) â†’ Â¬ (u â‰¡ v)
            â†’ decBool (dec u v) â‰¡ false
...
chu : (O : Obs X V) â†’ Discrete V â†’ ChuObs X (O .Index Ã— V)
...
Sepâ†’chu :
  ...
  â†’ Sep O x y
  â†’ Î£[ t âˆˆ (O .Index Ã— V) ] (Â¬ (chu O dec x t â‰¡ chu O dec y t))
Sepâ†’chu O dec x y (i , ne) =
  (i , O .read i x) ,
  Î» q â†’ trueâ‰¢false
    ( sym (decBool-refl dec (O .read i x))
    âˆ™ q
    âˆ™ decBool-neq dec (O .read i y) (O .read i x) (Î» p â†’ ne (sym p)) )
...
chuâ†’Sep :
  ...
  â†’ Î£[ t âˆˆ (O .Index Ã— V) ] (Â¬ (chu O dec x t â‰¡ chu O dec y t))
  â†’ Sep O x y
chuâ†’Sep O dec x y ((i , v) , nb) =
  i , Î» p â†’ nb (cong (Î» w â†’ decBool (dec w v)) p)

-- The translation is distinction-preserving in both directions.  Neither
-- presentation sees a pair the other cannot.
bridge :
    (O : Obs X V) (dec : Discrete V) (x y : X)
  â†’ (Sep O x y â†’ Î£[ t âˆˆ (O .Index Ã— V) ] (Â¬ (chu O dec x t â‰¡ chu O dec y t)))
  Ã— (Î£[ t âˆˆ (O .Index Ã— V) ] (Â¬ (chu O dec x t â‰¡ chu O dec y t)) â†’ Sep O x y)
bridge O dec x y = Sepâ†’chu O dec x y , chuâ†’Sep O dec x y
...
--     makes `Shrink(ð’¯) â’ Î´â“` a theorem.  The image of `chu` is indexed by
...
--     `absField` read `absâ` directly rather than as a family of predicates.
--     Under `chu` that single â-valued observation becomes a â-indexed family
