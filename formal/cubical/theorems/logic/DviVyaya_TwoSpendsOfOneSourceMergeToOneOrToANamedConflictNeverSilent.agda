...
-- �������� � "double expenditure" (dvi = two, vyaya = spending/outgoing).
...
--                  Saptabhangi.�����-��-����: the saha pair surfaced, not voted on).
...
-- `Spend src = �[snk] (spend src ≡ snk) = singl (spend src)`, which is ALWAYS
-- contractible (��������: isContr (singl x)); so its `Exclusive` is
