...
-- scalar reads SETTLED), yet its fiber over tt is Bool ‚î two points,
-- not a proposition (fiber debt > 0: curvature remains).  So
-- scalar-stationary ‚ fiber-stationary; moka-of-the-count is not
-- moka-of-the-geometry.  The stationary condition of the field is
-- isEquiv (every fiber contractible = ‡‡ï‡≤‡æ‡¶‡‡ = kevala), and the count
...
-- FIBER DEBT POSITIVE.  The fiber over tt has two distinct points ‚î
-- (true, refl) and (false, refl) ‚î so it is NOT a proposition.  This is
-- the retained distinction the scalar cannot see: curvature.
fiber-not-prop : ¬¨ ((x y : fiber observe tt) ‚Üí x ‚â° y)
fiber-not-prop pr = true‚â¢false (cong fst (pr (true , refl) (false , refl)))
...
scalar-does-not-imply-geometry :
  ...
  √ó (¬¨ ((x y : fiber observe tt) ‚Üí x ‚â° y))                          -- ‚Ä¶ yet fiber not settled
...
-- fiber contractible, ‡‡ï‡≤‡æ‡¶‡‡, kevala ‚î which STRICTLY implies both
