...
-- अद्वय — not-two.  Not a slogan: the fiber law read without the dualist
...
module _ {A : Type} where
  ...
  -- is the output-binding of the fiber law (singl), and it holds with no
  ...
  -- fiber over b can be arbitrary.  Otherness = a non-contractible fiber
  -- = charge = memory = the knot.  Same map, other side.  So duality is
  -- not false; it is a standpoint (naya), and it is exactly a fiber that
...
module _ {A B : Type} (f : A → B) where
  ...
  -- to give it is to give contractibility of every fiber.)
  ...
  -- is a fiber that carries something, and the mistake is only grasping
