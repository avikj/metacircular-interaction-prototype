...
-- àà¦ààµà¯ â” not-two.  Not a slogan: the fibre law read without the dualist
-- standpoint.  There is one object.  "Duality" is not a second thing â”
...
private variable â„“ : Type
module _ {A : Type} where

  -- àà¦ààµà¯: the space of "what is equal to me" is CONTRACTIBLE â” there is
...
module _ {A B : Type} (f : A â†’ B) where
  ...
  -- EXACTLY when every otherness is contractible â” when nothing is hidden
  ...
  advaya-is-kevala : (âˆ€ b â†’ isContr (fiber f b)) â†’ isEquiv f
  advaya-is-kevala h = record { equiv-proof = h }

  -- the converse standpoint, stated so the durnaya is visible: if the map
  -- is NOT an equivalence, some otherness is non-contractible â” a real
  ...
  kevala-is-advaya : isEquiv f â†’ (âˆ€ b â†’ isContr (fiber f b))
