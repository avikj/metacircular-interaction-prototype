...
--
--   level ‡ ‚î the fibre is the WHOLE source, and nothing anywhere sees
--             the difference;
--   level ‡© ‚î the fibre is a PROPER PART, and other maps out of the
...
-- maps' fibre censuses are pointwise equivalent ‚î so no reading of the
...
-- fibre is crowded at all, because ‡µ‡ø‡ï‡≤‡æ‡¶‡‡'s own evidence ‚î two fibre
...
-- level-‡ archetype at `Bool` is equivalent to the single fibre of the
-- level-‡® archetype.  A census is a function on fibres; two maps whose
...
--
-- The note's level-‡© side reads: the fibre is a proper part, and *other
...
-- difference: two distinct points of one fibre of `‚à_‚à‚`, and `idfun`
-- separating them.
--
