...
-- says agreement permits it.  Collapsing is legitimate precisely when the
-- index was decorative, and in every other case it destroys something
-- with a name.
--
-- The two hypotheses are not complementary.  A family can be neither
-- sydastinsti (no standpoint denies) nor uniformly equivalent, and then
-- NEITHER theorem applies — yet collapse is still unavailable.  A counterexample
...
-- transports, or they are not and no collapse exists, and "pick the
-- better view" is not among the two.  The two branches are not
-- complementary, so this pair is not a dichotomy (see §5).
collapse-dichotomy :
  {S : Type ℓ} (P : S → Type ℓ') (s₀ : S) →
  (((s : S) → P s ≃ P s₀) → Σ[ Q ∈ Type ℓ' ] Collapses P Q)
  × (syādastināsti P → (Q : Type ℓ') → ¬ (Collapses P Q))
