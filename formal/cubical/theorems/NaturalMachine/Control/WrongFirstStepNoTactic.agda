...
-- WHY A TWIN EXISTS.
--
-- `WrongFirstStep.agda` imports `NaturalMachine.GenerativeLoop` (for
-- `generative-step`) and `NaturalMachine.CompileBridge` (for
...
-- `NaturalMachine.Transport`, and `Transport` line 46 imports
--
...
-- `NaturalMachine.Obstruction` — which typechecks in both toolchains —
...
module NaturalMachine.Control.WrongFirstStepNoTactic where
