...
-- note `notes/GENERATIVE_LOOP_ARITHMETIC_BOUNDARY.md`, whose F/G
...
-- and the budget-two statement are `notes/RESIDUAL_LANGUAGE_GROWTH.md`.
--
...
--
-- Substrate: exactly `NaturalMachine.Obstruction` (`Tm`, `Vocab`,
-- `Over`, `plug`, `unfold`, `unfold-elim`), plus `size` from
-- `NaturalMachine.WitnessPolicy` and `deficit` from
-- `NaturalMachine.GenerativeLoop`.  No new term constructor is added —
...
module NaturalMachine.TypedUnfold where
...
open import NaturalMachine.GenerativeLoop using (deficit)
...
--
-- The algebra of `notes/RESIDUAL_LANGUAGE_GROWTH.md`, on ℤ³:
--
