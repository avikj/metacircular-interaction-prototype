...
--
-- §�d's observation is that the third is provably not reachable from the
...
-- returns only by a separating query", and it is ��������� read from the
...
-- positions and a void, and `Saptabhangi.������-����` proves exactly that
-- as an equivalence.  So four combinations are missing from the table.
--
-- §� below is that transported onto the presentations.  It is a
...
--    independence is assumed in the saptabhag.  §� is conditional on
...
--    §� names the four the counting forces.  Whether a method exists at
--    each is mathematics, not bookkeeping, and none is claimed.
-- 3. THAT TAO'S METHOD IS �� RATHER THAN �����.  §�d reads it that way �
...
--    `Saptabhangi.�����-��-����` (successive � simultaneous) and
--    `Arpitanarpita_�.��-����������-���������` (simultaneous is NOT
--    associative), so "combination" is not one operation and the four
--    below are not the iterated pairwise ones.
--
-- SOURCES. The sevenfold and its 2³−1 counting: the Jaina sydvda tradition
-- � Umsvti, ��������������� (~2nd�5th c.); Samantabhadra, �����������; the
-- �����/�� distinction fixed by Akalaka and Vidynandin. Second-hand
-- throughout. What is taken is the counting of positions
...
------------------------------------------------------------------------
-- � � ���������� � the three presentations of BARRIER.md §2, as a type,
--     and the identification of each with a seed.
--
--     The names are the interfaces, not the methods: a presentation is
--     WHAT A METHOD MAY READ.  That is the whole content of §�d's
--     reading and of ��������� � what a rule may observe decides what it
--     can derive.
------------------------------------------------------------------------
...
-- method reads it.  That is literally ������ � the seed-selection triple.
संयोगः : Type
संयोगः = समावेश

-- the three singletons, named, so §�'s content is legible
...
------------------------------------------------------------------------
-- � � ���� ��������� � THREE INDEPENDENT PRESENTATIONS GENERATE SEVEN.
--
--     Transported, not re-derived: `Saptabhangi.������-����` is the
...
--     of them, which is �-�����������, no predication at all, and is not
--     an eighth position.
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- � � �������� � �������� � THE FOUR THE TABLE DOES NOT LIST.
--
...
------------------------------------------------------------------------
-- � � The reading that makes §� worth having, stated as the note's and
--     not as a theorem.
--
--     `SamagraDarsana` §�d observes that the global-multiplicative
--     presentation sits where ����������� sits � the position NOT reachable
--     from the other two by ����� � and that Tao's entropy decrement
...
--     two presentations at once, and §� says which further combinations
--     exist.
--
...
--     (`Saptabhangi.�����-��-����`) and simultaneous is not associative
--     (`Arpitanarpita_�.��-����������-���������`).  So "combine two
--     presentations" names two different operations, and the four above
--     are not the iterated pairwise ones.
--
--     §� is the one further thing that IS a theorem here.
------------------------------------------------------------------------

------------------------------------------------------------------------
-- � � �����-������ � the void is not a position, and that is the answer
--     to Problem 3's real content.
--
--     `Saptabhangi.�����-�������` proves no bhaga maps to the empty
...
--     give 2� − 1 positions and one void.  Three gives seven.  A fourth
...
-- carries ��� in at least one slot, and ��� � �.
