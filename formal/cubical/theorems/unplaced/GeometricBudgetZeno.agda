...
-- therefore �_j 2^{-j}, which is FINITE.  So merely detecting a nonzero
...
--
-- The exact arithmetic behind �_j 2^{-j} < ∞ is the finite geometric
-- telescback, here with no subtraction and no rationals:
--
--     suc (�_{i<n} 2^i)  ≡  2^n .            (geo)
--
-- Equivalently the partial budget stays strictly below the finest-scale unit,
-- �_{i<n} 2^i < 2^n (boundedBudget).  Scaled by 2^{-(n-1)} this is
-- �_j 2^{-j} < 2: bounded, exactly the Zeno-accessibility of the UV boundary
