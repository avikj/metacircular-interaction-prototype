...
------------------------------------------------------------------------
-- ����������������� � THE QUESTION IS A TYPE, � IS TOTAL, THE GATE
-- HOLDS TO SIXTEEN, AND NO CONVERSE IS WRITTEN.
--
-- Lehmer's question (1947), still open: is �(n) ever zero?  Nobody
...
-- two-layer treatment:
--
--   � IS TOTAL HERE.  �At n reads the n-th coefficient of the Δ
...
--   differences (a , b) standing for a − b; �(n) = 0 is exactly the
...
------------------------------------------------------------------------
-- §1  The Δ product at every truncation: � total.
------------------------------------------------------------------------
...
-- � at n, from the product truncated exactly where n lives: total.
τAt : ℕ → D
...
-- �(n) = 0 is exactly the balance of the computed difference pair.
Balances : ℕ → Type
