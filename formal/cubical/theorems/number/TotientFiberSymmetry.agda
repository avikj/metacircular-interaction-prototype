...
-- totient-fiber collapse arises from a *transitive* group action (Theorem E,
...
--   transitively on each of its fibers: the observational stabilizer itself.
...
------------------------------------------------------------------------------
-- §1.  Orbits refine fibers.  (The easy half of "orbits = fibers", and the
...
module Transposition {X : Type₀} (dis : Discrete X) (a b : X) where
  ...
  -- A transposition *inside* a fiber lies in the observational stabilizer.
...
------------------------------------------------------------------------------
-- §3.  THEOREM T (trivialization).  Every fiber of every observation is an
...
fiber-is-an-orbit : {X : Type₀} (dis : Discrete X) (observation : X → ℕ)
                    (a b : X) → observation a ≡ observation b
                  → Σ[ e ∈ (X ≃ X) ]
                      (Stabilizes observation e) × (equivFun e a ≡ b)
fiber-is-an-orbit dis observation a b h =
...
-- Theorem T applied to the very fiber the open item names: a group acts
-- transitively on {1,2} …
