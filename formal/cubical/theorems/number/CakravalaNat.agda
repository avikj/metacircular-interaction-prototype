...
------------------------------------------------------------------------
-- CakravalaNat � the cycle's step over �, every subtraction cleared.
--
...
-- unusable for certifying an actual run.  Cubical's � multiplication is
-- UNARY �
--
--     pos (suc n) � m = m + pos n � m        (Cubical/Data/Int/Base.agda)
--
-- � so a `refl` on the nine-digit integers the cakravla produces at D = 61
-- asks the kernel for about 10�� unfoldings and never returns.  An attempt
-- had to be killed.  Cubical's � is `Agda.Builtin.Nat`, whose `_+_` and
-- `_�_` are GMP-backed, so the identical statement over � is instant.
--
-- THE REPAIR IS THE ONE THIS REPOSITORY ALREADY USES.  `BhavanaSemiring`
-- found that bhvan is FALSE over � as classically written, because monus
...
--
-- Over � that IS `cakravalaCleared`, by adding the same two terms to both
-- sides.  Over � it is true for ALL naturals, with no hypothesis, and it is
...
-- cycle over �, and clearing the subtraction removes it rather than handling
-- it.
--
