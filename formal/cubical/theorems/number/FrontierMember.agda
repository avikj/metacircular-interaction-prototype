...
-- refl` in that module still check, which is the test that matters ‚î
...
-- from `p ‚à m'` is where primality finally has to be used.
--
...
-- above.  ¬ß7's ESTIMATE WAS WRONG AND HAS BEEN CORRECTED ELSEWHERE.
--
-- ¬ß7 says the coprimality piece "is the only one of the three that needs
...
--
--     IsPrime p = (1 < p) ó ((d : ‚ï) ‚í d ‚à p ‚í (d ‚â° 1) ‚ä (d ‚â° p))
--
-- a common divisor of `p` and `m` is 1 or `p`, and the hypothesis `p ‚à m`
