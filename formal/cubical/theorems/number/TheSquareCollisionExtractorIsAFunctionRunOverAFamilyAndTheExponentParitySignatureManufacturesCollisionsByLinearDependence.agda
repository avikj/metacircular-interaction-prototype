...
-- extractor (§A) turns into a factor.  Once inside the smooth chart,
-- the whole pipeline is arithmetic and
-- linear algebra the kernel computes and hands back.
--
------------------------------------------------------------------------

module TheSquareCollisionExtractorIsAFunctionRunOverAFamilyAndTheExponentParitySignatureManufacturesCollisionsByLinearDependence where
