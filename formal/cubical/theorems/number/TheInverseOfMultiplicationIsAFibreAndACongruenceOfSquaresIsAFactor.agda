...
-- multiplication, on one set �.  Each is a map that is CHEAP FORWARD
-- and, in general, ONE-WAY BACKWARD:
--
--     multiply : (p , q) � p�q        easy;  factoring is its inverse
--     exponentiate : x � g�           easy;  discrete log is its inverse
...
--     answer-space is `singl (f a) = �[ y ] (f a ≡ y)`, which is
...
--     The answer-space is the FIBRE `fiber f b = �[ a ] (f a ≡ b)`.  It
...
--
--   � Over �:  `fermatFactorization` � a representation N = w²−r² hands
--     you the factor pair (w−r , w+r).
--
--   � Over �/N:  `congruentSquaresGiveZeroDivisor` � a nontrivial square
...
module CongruenceOfSquares (R : CommRing ℓ-zero) where
  ...
  -- divisor.  Over �/N with a � �b this is a proper factor of N via gcd
...
--
-- � carries TWO operations, and number theory studies how they
-- interfere.  The MULTIPLICATIVE face holds factoring, discrete log, and
-- � through the zeros of � � the Riemann hypothesis.  The ADDITIVE face
...
--     P(z) = � �(n) e^{−nz},   K(w,r) = �(w−r)��(w+r),
-- which turns multiplicative data (von Mangoldt �, which knows the
...
-- prime-count error as a sum over the zeros of �, so the zeros ARE the
...
-- bound � is the open problem, localized here to the derivation rather than
-- the answer.
------------------------------------------------------------------------
