...
--     Φ_{p^a}(t) = t^a − t^{a−1}.  Each is an instance of Φ-closed-form
--     once ω(mn) = ω(m) + ω(n) for coprime m, n (resp. Ω(p^a) = a and
--     ω(p^a) = 1) is known.
--   * "Mbius-coherent" in §5.4.
--   * The partition of unity Σ_{d|n} Φ_d(t) = t^{Ω(n)} of
--     ChargePolynomialFinite §5.
