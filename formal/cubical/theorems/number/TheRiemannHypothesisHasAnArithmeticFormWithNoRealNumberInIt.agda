...
--
-- SYĀT -- WHAT IS AND IS NOT CLAIMED.  §§1-3 are definitions with computed
-- values.  §4 is a TYPE and this module does not inhabit it, does not refute
-- it, and does not prove it equivalent to the Riemann Hypothesis.  That
-- equivalence -- RH ⟺ M(x) = O(x^{1/2+ε}) -- is classical analytic input
-- (it is the explicit formula plus a Mellin/Perron argument) and it is NOT
-- formalised here; it is the debt this file states rather than hides.  What
-- IS established is narrower and is the whole point: the statement can be
-- written with no real number, no limit, and no unexplained term, so its
-- content is visible as an integer growth bound on one sequence.
--
