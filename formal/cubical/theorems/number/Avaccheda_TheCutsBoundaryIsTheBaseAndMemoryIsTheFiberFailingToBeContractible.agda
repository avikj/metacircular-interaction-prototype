...
--
--  ¬ß‡ß  A ‚â Œ[ b ‚àà B ] fiber f b.  The history set decomposes as
...
--
--  ¬ß‡®  "No memory" is `isEquiv f`, i.e. every fiber contractible.  Then
...
--      MEMORY IS THE FIBER FAILING TO BE CONTRACTIBLE ‚î and the failure
...
--          ‡‡‡     memory required, and the fiber IS the amount
...
--
-- Œ[ b ‚àà B ] fiber f b ‚â A.  The left side is (boundary datum, the
...
-- ‡∞‡ø‡ï‡‡‡Æ‡ : a profile no history induces.  ‚ä ‚í Unit has empty fiber at tt.
