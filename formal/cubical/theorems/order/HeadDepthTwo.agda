...
--   (group)  (�/2^a)^� has order 2^{a−1}; blindness makes ord(b) divide
...
-- index q^{a−1} = 2^{a−1} = �(2^a) � only "cyclic" must be dropped from
-- W4's proof, since (�/2^a)^� is not cyclic for a � 3; uniqueness of
...
------------------------------------------------------------------------
-- Fast arithmetic on builtin � (as in HeadDepthMerge)
