...
-- with संरक्षणम् (f ∘ Φ ≡ f pointwise) give a flow INSIDE every fiber
-- (ध्रुव-तन्तौ), and the conserved quantity is the fiber index.  That
...
--
-- THE CONSERVED QUANTITY IS THE FIBER INDEX — the sum n.  The shears
...
--
-- §३–§६ together say: **fiber योग n is a ℤ-torsor.**  The fiber has no
...
-- in the title is exactly §५+§६ (each fiber point is carried to each
...
-- §४ · freeness: a shear fixing any fiber point is the zero shear ----

मुक्तता : (k n : R) (x : fiber योग n) → प्रवाह k n x ≡ x → k ≡ 0r
मुक्तता k n ((a , b) , _) h =
  L2 a k ∙ cong (_- a) (cong (λ w → fst (fst w)) h) ∙ L3 a

-- §५ · transitivity: any two fiber points differ by a shear ----------
