...
------------------------------------------------------------------------
-- ��������� � the coarse weight is the sum over the observation fiber,
-- and branch multiplicity is the whole of it.
--
-- TERM.  ����� / �������� (gross / subtle) is the standard  pair
-- for the coarse and the fine grain of an object; ��� is weight, load.
-- The compound �����-���, "the coarse weight", is built HERE and no
...
-- an (a+b)-th part of � (����� h ≡ � over the whole orbit), then the
-- coarse weight of the m-branch IS the m-fold sum ����� h over that
-- branch.  "m/(m+n)" is a NAME for m�h under (m+n)�h ≡ �; the theorem
...
-- `SamaVibhaga` (which forces w = const h) rather than presupposing �.
--
-- THE OBJECTS.  Micro-outcomes: X = Fin (suc a) � Fin (suc b) � the
-- coarse observation is literally the tag, ������ = Bool by inl/inr.
-- The fiber of ������ over true is the left summand; the coarse weight
-- is total weight of that summand.  This presentation makes "sum over
-- the fiber" definitional rather than a subset-enumeration lemma.
--
-- WHAT IS PROVED.
--
--   �����������      the total over the disjoint sum is the sum of the
--                  two branch totals � fiber additivity, proved by
--                  induction on the left branch, consuming exactly ONE
--                  algebraic law: associativity of _+�_, taken as a
...
--                  result.)
--   ���������       on the uniform weight const h, the coarse weight of
--                  the left branch is ����� a h (that is, (a+1)�h) and
--                  of the right branch ����� b h � multiplicity is the
--                  whole content.
--   �������        and the two coarse weights recompose to � under the
--                  orbit normalization (suc a + suc b)�h ≡ �.
--
------------------------------------------------------------------------

module SthulaBhara_TheCoarseWeightIsTheSumOverTheObservationFiberAndMultiplicityIsTheWholeOfIt where
...
module _ {W : Type ℓ} (_+ᵂ_ : W → W → W)
  ...
  -- the fold of a left branch onto a residue R � "sum over the fiber
  ...
  -- the total over the disjoint sum: left fiber folded onto the right
  -- fiber's total.
  ...
  -- fiber additivity: the whole is the left fiber's total plus the
  -- right fiber's total.
  ...
  -- and under the orbit normalization the two branches recompose to �:
  -- (a+1)�h +� (b+1)�h ≡ total of the constant family over the sum.
  पूर्णता : (𝟙 h : W)
          → उभययोगः (λ _ → h) ≡ 𝟙
