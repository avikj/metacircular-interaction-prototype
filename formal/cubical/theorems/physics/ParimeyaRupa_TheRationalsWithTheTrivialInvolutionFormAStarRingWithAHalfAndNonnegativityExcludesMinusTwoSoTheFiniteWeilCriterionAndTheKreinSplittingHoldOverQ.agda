...
-- positivity "0 � x".
--
--   §1  � AS A COMMUTATIVE RING, from the library's ring laws on the
...
--       � (0 � −2) � the last read off on representatives in �.
--   §4  THE THEOREMS AT �: the finite Weil criterion and the Krein
--       splitting, with every parameter concrete; and the smallest
--       configuration with an off-line mode, n = 2 with � the swap,
--       whose negative vector δ� − δ� has �-form −2.
--
...
------------------------------------------------------------------------
-- � � The *-ring: trivial involution, ½ = [1/2].
------------------------------------------------------------------------
...
-- � (0 � −2): on representatives this is pos 0 +pos k ≡ negsuc 1 in �.
...
-- the finite Weil criterion over �, every parameter concrete
...
-- the vector δ� − δ� has �-form −2: the configuration is not positive
...
------------------------------------------------------------------------
-- � � An off-line transport in numbers.  On the swapped pair take
--     E = (2 , ½): the reflection law E i � E (� i) = 1 holds, so the
--     �-form is preserved for every vector � but (E 0)* � E 0 = 4 ≠ 1,
...
-- so the plain form is not preserved: �-unitary, not unitary
