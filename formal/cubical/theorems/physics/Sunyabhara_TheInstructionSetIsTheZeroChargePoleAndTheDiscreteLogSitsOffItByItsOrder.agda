...
--   `SvaTantuVasa_�`    �����-������� : isEquiv f � isContr (�[ Φ ] ���������� f Φ)
--                       � a lossless observable's conserving-flow space
--                       is a POINT (itself `invIso �-Π-Iso` evaluated).
--   `GhataTantu_�`      powg = ���� g in C�, whose fibre over ε is not
--                       contractible: 0, 3 and 6 all land there.
--
-- §� composes the first two: **every instruction of the machine has a
...
-- hidden.  `Dhruva_�`'s ����� read at the machine: no loss, no symmetry,
-- and the machine is all equivalences.
--
-- §��� exhibit the contrast at the map the crypto arc runs on, and the
-- contrast is what keeps §� from being vacuous.  Translation by the
-- ORDER conserves `powg`, and it is not the identity flow, so `powg`'s
-- conserving-flow space is not a point � hence `powg` is not an
-- instruction of this machine.  §� is that as a one-line corollary.
--
-- WHAT THE JOIN SAYS, and it is a reading of the terms and not a further
-- theorem: by `����` a conserving flow IS a section of the map's own
...
-- can move without being seen are one fibre.**  §� states the limits of
-- that sentence rather than leaving it to be over-read.
--
-- ��������������������������������������������������������������������
--
-- TERM.  �����-��� is `Dhruva_�` §�'s own compound for a contractible
-- fibre carrying no charge, and is reused rather than coined.  �������
...
------------------------------------------------------------------------
-- � � THE MACHINE'S POLE.  Every instruction has a contractible space of
...
--
-- `���� g 3 ≡ ε` is refl in `BijamulaKrida`, so three successor steps
...
------------------------------------------------------------------------
-- � � TWO FLOWS, SO NOT A POINT.  The identity flow and the period flow
...
------------------------------------------------------------------------
-- � � THE COROLLARY.  So the power map is not an instruction of this
--     machine � not "hard to invert", but not in the instruction set at
--     all, and §� names by how much: its own period.
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- � � ���� � the limits of the reading in the header.
--
-- `���� powg` carries �������-������-�������� to the section
-- `a � (3 + a , �������� a)`: at every exponent, the next exponent with
-- the same public value.  That section is a point of the same fibre
-- family whose non-contractibility at ε is `GhataTantu.������-�������`.
...
-- translations � §� exhibits two flows and refutes contractibility; it
-- does not compute the space, and the sections of a fibre family over �
...
-- `GhataTantu`'s owed general coset statement.  §� is a witness, not a
-- census.
--
-- NOT settled here: anything about C�'s successors.  §�'s `refl` is a
