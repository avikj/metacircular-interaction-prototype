...
-- fiber is contractible, so the section space is contractible — §२
...
--
-- TERM.  स्व (own), तन्तु (thread, fiber), वास (dwelling) are ordinary
-- .  तन्तु for the fiber of a map is THIS CORPUS's rendering
-- (declared in `Tantujala_….agda`'s header),
-- and− the compound संरक्षकसमूह is built here.
------------------------------------------------------------------------

module TheConservingFlowsOfAnyObservableAreTheSectionsOfItsOwnFibers where
...
open import TheZeroDivisorEdgeIsPricedAtItsWholeDomainAndTotalLossIsExactlyTotalSymmetry
  using (सर्व-नाशः)
open import TheConservingFlowsOfAdditionAreExactlyTheShearFields
...
module _ {A B : Type ℓ} (f : A → B) where
  ...
  ------------------------------------------------------------------
  -- §१ · THE LAW.  Conserving flows ≃ sections of one's own fibers.
  --
  ...
  ------------------------------------------------------------------
  -- §२ · THE NEAR POLE, STRENGTHENED.  Zero loss: contractible fibers,
...
-- induces a map of conserving flow spaces — so larger fibers admit more
-- flows, functorially, with no order on observables needed: the order
-- IS the fiber-wise maps.  Three lines, because after §१ it is only
-- Π-postcomposition.
------------------------------------------------------------------------
