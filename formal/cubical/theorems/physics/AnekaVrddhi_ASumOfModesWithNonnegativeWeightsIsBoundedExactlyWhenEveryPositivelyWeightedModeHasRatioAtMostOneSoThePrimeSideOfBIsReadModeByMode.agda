...
-- proof note's signal carries POSITIVE weights �(n)/�n.  Atiorder read
...
--       Atiorder a t with q K < m_i^t, so c_i m_i^t > K � cancelling the
...
module _ (n : ℕ) (c m : ℕ → ℚ) (0≤c : (i : ℕ) → 0 ≤ c i) (0≤m : (i : ℕ) → 0 ≤ m i) where
  ...
  ... | gt 1<m = ⊥-elim (PT.rec isProp⊥ go (guṇaka (c i) 0<ci))
    where
    go : Σ[ q ∈ ℕ ] 1 ≤ ι q · c i → ⊥
    go (q , 1≤qc) = PT.rec isProp⊥ go′ (atiorder (m i) 1<m (ι q · K))
      where
      0<q : 0 < ι q
