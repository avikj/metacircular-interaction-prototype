...
-- are new terms.
--
--  1. WITNESS (cited, not reproved).  � � CanWord is the constructed,
--     checked equivalence (Digits), and �-Monoid ≡ CanWord-Monoid is
--     its SIP upgrade (Transport §8).  Nothing about the witness is
--     re-derived here.
--
--  2. THEOREM, � SIDE.  Commutativity of _+_ � a law proved on the
...
--         subst CommutativeOp �-Monoid≡CanWord-Monoid +-comm
...
--     through the decoder using �'s +-comm directly, in the exact style
...
--    operation), one equivalence (� � CanWord, base b = 2 + k,
...
module TransportInstance (k : ℕ) where
...
open import FreeMonoid using (ℕ-Monoid)
open import Transport k
...
open CountedComposition.Odometer k using (digitsC-resume)
open import AcceptanceTest k using (resume ; cost ; exec)
...
-- Station 2: the theorem on the � side.  `�-Monoid`'s operation is
...
--     SIP path �-Monoid ≡ CanWord-Monoid (Transport §8, cited).  Note
...
--     transport: through the decoder, using �'s +-comm directly.  This
