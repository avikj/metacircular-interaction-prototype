...
module TheOrbitRelationIsAlreadyAnEquivalenceWithoutAnInverseAndTheChargeDescendsToTheQuotient where
...
open import TheSymmetryLivesInTheFibreAndWithoutALossThereIsNoSymmetry
  using (संरक्षणम्)
open import TheChargeIsConstantAlongTheWholeOrbitAndNotOnlyAcrossOneStep
...
-- from a" — that is the asymmetric relation, and it is not the orbit's.
------------------------------------------------------------------------
...
--     stronger hypothesis.
--
-- Rung (a) proposed requiring `Φ` to be an equivalence.
