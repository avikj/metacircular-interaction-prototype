...
--
-- The digit presentation carries structure that � does not: word
...
-- value map to a function on � --- each fact proved here by exhibiting
...
--
--   * E commutes with � = "delete the most significant digit";
--   * D does not; instead D exchanges � with � = "delete the least
...
--
-- Constructed from the �-witness of `to� d < b`, so the defining
...
--
-- The two truncations of a digit word.  � deletes the most significant
-- digit (the one the profinite limit �_b is built from); � deletes the
...
-- E IS compatible with �: the complement descends to every �/b� chart
