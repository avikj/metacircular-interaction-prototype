...
------------------------------------------------------------------------
-- ���� � pla, "a bundle, a sheaf (of grass)".  The ordinary 
...
--   `Universal_�`         a family is a map into the universe, and � is
--                         its total space
--
...
--       action.  `����` : the translating element is UNIQUE, imported as
--       a live use of `EkaTransport`'s `regular` rather than restated.
--       `������` : hence the orbit map Carrier � Pts is an equivalence.
--
--   §2  ���� / ����� / ������-������ � a principal G-bundle over B is a map
--       B � Torsor.  That IS its classifying map: by the object
--       classifier (`Universal` §1) a family and a map into the universe
--       are the same thing, so "the bundle" and "the map into the type
--       of torsors" are not two objects.  The total space is �, the
--       projection is `fst`, and the fibre over b is the torsor over b.
--
--   §3  ��������� � TRANSPORT IN A BUNDLE IS EQUIVARIANT, by path
...
--
--   §4  ����������� (`���������`) � the holonomy of a loop, measured at a point
...
--       uniqueness.
--
--   §5  ��������� � CHANGING THE POINT OF THE FIBRE CONJUGATES THE
--       HOLONOMY:
--           ��������� � (k � p) ≡ (k � ��������� � p) � inv k.
...
--
--   §6  ���������� � matter.  A representation is a family on the type of
...
--       operation, at the associated bundle.  `�������` exhibits the
...
module _ (Grp : Group ℓ) where
  ...
  -- ���� � the translating element is exactly one.  `EkaTransport`'s
  ...
  ----------------------------------------------------------------------
  -- §2  ���� � the principal bundle.  A map B � Torsor IS the bundle and
  --     IS its classifying map; by the object classifier those are not
  --     two objects.  The total space is � and the projection is fst.
  ----------------------------------------------------------------------
  ...
  -- HoTT Lemma 4.8.1 at this family; `Universal` §2 is the same lemma at
  ...
  ----------------------------------------------------------------------
  -- §4  ��������� � the holonomy of a loop, measured at a point of the
  ...
  module _ {ℓ' : Level} {B : Type ℓb} (P : पूलः {ℓ' = ℓ'} B) {b : B} where
    ...
    ------------------------------------------------------------------
    -- §5  ��������� � the gauge transformation law.  Moving the point of
  ...
  ----------------------------------------------------------------------
  -- §6  ���������� � matter is a section of an associated bundle.
  --
