...
------------------------------------------------------------------------
-- ���� � time is indivisible samayas, and the maximal motion (one loka per
-- samaya) gives a discrete causal cone with a light-speed invariant.
--
-- SOURCE.  Umsvti, *Tattvrthastra*, adhyya 5, on kla:
--   5.22  kla ca (Digambara recension: "and time [is a substance]");
--         the vetmbara reads it as an upacra-dravya (5.38�39).  Either
--         way, kla is the substance whose function (upakra) is vartan �
--         continuance/change � and duration/measure (5.22 bhya).
--   The SAMAYA is the indivisible atom of time (the instant), the smallest
--   unit below which time does not divide � Anuyogadvra-stra, and the
...
--   the loka � a free paramu, or the soul's ju-gati at liberation �
...
--   pradeas per samaya.  (Sourced to the bhya/karma literature, not to
--   a Tattvrthastra mla-stra; hence marked.)
--
-- THE PHYSICS, exactly: discrete time (samaya : �), discrete space
-- (position in pradeas : �), and a speed cap D = the loka-span, the most
-- pradeas any motion covers in one samaya.  A cap on speed is a causal
...
-- Lorentzian light cone encodes, here from Jain kla rather than from a
-- metric � and it is the causal backbone under this corpus's
-- "computational spacetime" (`Jiva`'s null/timelike sectors).
--
-- WHAT IS PROVED:
--
--   §2  ����� is the speed law: a motion x : samaya � pradea obeys the cap
--       when each samaya advances by at most D (mabreadraved either way).
--   §3  �������� � THE CONE.  Under the cap, position after n samayas is
...
--   §4  ������������� � attainment.  The maximal motion (advancing exactly D
--       each samaya) reaches n�D in n samayas, and D � one whole loka-span
--       � in ONE samaya.  The cap is tight; the paramu crosses the loka
--       in a single instant.
--   §5  ����-��������� � the samaya is indivisible: there is no time
--       strictly between n and its successor.  Time does not subdivide
--       below the instant (the doctrine's atom of time), exactly as the
--       paramu is the atom of matter (`Pudgala.����-���������`).
--
...
------------------------------------------------------------------------
-- §2  ����� � the speed cap: each samaya advances by at most D pradeas.
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- §3  �������� � the cone.  Under the cap, x n � x 0 + n � D.
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- §4  ������������� � attainment.  The maximal motion advances exactly D each
...
शीघ्र-नियमः D n = subst (शीघ्रगति D (suc n) ≤_) eq ≤-refl
  where
  -- ���������� D (suc n) = suc n � D = D + n�D = n�D + D = ���������� D n + D
...
------------------------------------------------------------------------
-- §5  ����-��������� � the samaya is indivisible: nothing strictly
