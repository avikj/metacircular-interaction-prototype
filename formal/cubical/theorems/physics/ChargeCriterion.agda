...
-- cannot separate � from its gauge flip, whatever post-processing it
...
-- criterion below quantifies over query sets and not over �.
------------------------------------------------------------------------
...
--
-- Read the answer to the odd query and return it.  On �� every value is
-- +1 (`val-��`); on the flip the value at an odd-Ω argument is −1
-- (`val-��`).  The induction is over the position of the odd query, and
