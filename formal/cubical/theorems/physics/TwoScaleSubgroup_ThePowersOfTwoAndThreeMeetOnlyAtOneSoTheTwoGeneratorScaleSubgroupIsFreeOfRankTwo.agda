...
------------------------------------------------------------------------
-- TwoScaleSubgroup � 2�� = 3� only when m = n = 0.  Hence the subgroup
-- {m log 2 + n log 3} of � is free abelian of rank two, i.e. log 2/log 3
-- is irrational, which is the arithmetic input behind the density of
-- the two-generator sample set �(m, n) in the C(�²)-descent statement
-- of the RH line: a trace bounded on the samples is bounded everywhere.
--
--   §1  2�� is even for m � 1, 3� is odd for every n (a witness k with
--       3� = 1 + 2k, built by 3(1 + 2k) = 1 + 2(3k + 1));
--   §2  2�� ≡ 3� � m ≡ 0 and n ≡ 0.
--
-- SYT.  Elementary parity over �.  Density in �, continuity of the
-- trace, and the Riesz representation on �² are NOT here; this is the
