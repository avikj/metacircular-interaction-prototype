...
module _ (ds : List Bool) (cs : Caps ds) (vs : List (Vec ds)) where
  ...
  mixedStratumIsSelectedByTheFlippedTest :
    AllBounded ds cs vs
    â†’ mixedStratum ds vs
      â‰¡ filterDec FlippedTest (Î» u â†’ decIsParetoMaximal (f u) (map f vs)) vs
  mixedStratumIsSelectedByTheFlippedTest ab =
    filterRespectsOn
      (MixedMaximal ds vs)
      FlippedTest
      (decMixedMaximal ds vs)
      (Î» u â†’ decIsParetoMaximal (f u) (map f vs))
      vs
  ...
  thePublishedFormIsItsImage :
    AllBounded ds cs vs
    â†’ map f (mixedStratum ds vs) â‰¡ stratum (map f vs)
  thePublishedFormIsItsImage ab =
      cong (map f) (mixedStratumIsSelectedByTheFlippedTest ab)
    âˆ™ sym (filterMapCommutes f
            (Î» y â†’ IsParetoMaximal y (map f vs))
            (Î» y â†’ decIsParetoMaximal y (map f vs))
            vs)
