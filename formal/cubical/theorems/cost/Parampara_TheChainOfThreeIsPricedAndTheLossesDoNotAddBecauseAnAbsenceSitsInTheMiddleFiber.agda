...
--
-- **�������** � parampar � transmission through a SUCCESSION OF LINKS,
...
-- by succession, and 4.2b says *sa kleneha mahat yogo naa* � by the
-- long lapse of time that yoga was LOST here.  A chain of links, and a
-- loss along the chain, in one verse.
--
-- A SECOND TERM, AND ITS SCHOOL, NAMED BEFORE IT IS USED.  §� and §�
-- turn on an **����** (abhva) � an absence.  That is
-- **Nyya�Vaieika**, not a neutral technical word: Kada,
-- *Vaieikastra* 9.1 (~2nd c. BCE � 2nd c. CE), systematised in
-- Praastapda's *Padrthadharmasagraha* (~6th c.) and stated compactly
-- at Annabhaa, *Tarkasagraha* §§57, 80 (~1600).  The one feature of
-- on: an absence is a RELATION with named slots, never a bare negative
-- verdict � it is the absence OF something (its **pratiyogin**,
-- counterpositive) IN something (its **anuyogin**, locus).  §� names both
...
-- with two slots.  Nothing here is a joint Nyya�Jaina construction.  One
...
-- composite is the identity (§� below, where it is derived from the same
...
--            (3)                    (4)                (2)             (1)
--
--   ������ (inl b) = (b , false)      �������� = fst        ������� _ = tt
--   ������ (inr tt) = (true , true)
--
--  §�  the four fibers of ������, each named AS A TYPE, not as a count:
--         fiber ������ (b , false) � Unit   for both b
--         fiber ������ (true , true) � Unit
--         fiber ������ (false , true) � �   �� THE ABSENCE
--  §�  fiber �������� c � Bool, UNIFORMLY in c.  One bit, everywhere.
--  §�  fiber ������� tt � Bool.  One bit.
--  §�  the middle composite ����� = �������� ∘ ������:
--         fiber ����� false � Unit      (contractible � proved)
--         fiber ����� true  � Bool      (NOT contractible � proved)
--      so the composite's fiber is NOT uniform, and §�'s uniformity has
--      been destroyed by composing with a map that loses nothing.
--  §�  ��� INSTANTIATED at (�������� , ������), the RHS computed by hand
...
--  §�  the whole chain.  fiber ����� tt � Bool � Unit � THREE.  The
...
--  §�  and the defect is not a number.  There is no uniform Φ for ������
--      (proved), so §� of `Sesa_�` � the one clause under which logs add
--      � has a FALSE HYPOTHESIS here, and the witness of its falsity is
--      the absence of §�: pratiyogin `(false , true)`, anuyogin
--      `image ������`.  The missing 1 = 4 − 3 is that absence, and it is
--      exhibited as a type.
--  §�  `Unit � Bool � Unit` � the cancelling chain � DERIVED from the
--      same mechanism.  Same shape, same absence, opposite-looking
--      arithmetic.  So "losses cancel" and "losses fail to add" are one
--      phenomenon, and §� prices both.
--
-- ��������������������������������������������������������������������
--
...
-- ledger fails for every chain � §�'s agreement is exactly the statement
...
-- composite's fiber.  They do not; the alignment term does, and §� names
-- it.  Nothing here is about rank, dimension, entropy, area or
-- shadow of §� and is not used.
--
...
private
  -- the empty type at every fiber position we need it, and the only
...
-- (`λ x � g (f x)`), so the instantiation in §� needs no coercion.
...
-- � � the four fibers of ������.
--
-- ������ is injective � three points into four � so every fiber is a
-- proposition, and each is EITHER Unit OR �.  Which one, at which point,
-- is the entire content of §� and §�, so all four are written out and
-- none is left to a general lemma.
--
-- The second component of a `fiber ������ y` is a path in `Bool � Bool`,
-- which is a set, so `�≡Prop` reduces every uniqueness obligation below
...
------------------------------------------------------------------------
-- � � the fiber of ��������, and it is UNIFORM.
--
-- fiber fst c = �[ y ∈ Bool � Bool ] (fst y ≡ c), and the second
-- coordinate of y is free.  One bit lost, at every c alike.  This is the
-- hypothesis of `Sesa_��������` holding, for this map; §� shows it
-- FAILING for ������, which is why the chain does not simply multiply.
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- � � the fiber of �������.  One bit, and the only point of the base.
------------------------------------------------------------------------
...
-- UNIFORM, although ������'s failure of uniformity is a failure between
-- Unit and � and ��������'s fiber is uniform.  Both halves proved: the
...
-- �� and now the right-hand side computed BY HAND, without ���. ��������
--
-- Over `false` the base `fiber �������� false` has two points �
-- ((false , false) , refl) and ((false , true) , refl) � and §� says the
-- ������-fiber over the first is Unit and over the SECOND IS �.  So the
-- � has one inhabitant, not two: the second branch of the base
...
-- a point of `Bool � Bool` that is in the image of ������ and has first
...
-- and ((true , true) , refl) � and §� says BOTH ������-fibers are Unit.
-- So the � has two inhabitants and the alignment term is zero here.  The
...
हस्त-शेष-सत्ये = isoToEquiv (iso to from ri li)
  ...
  -- every ������-fiber over a point of this base is a proposition:
...
-- �� THE AGREEMENT. ���������������������������������������������������
--
-- §� computed the left side by hand; the two paragraphs above computed
-- the right side by hand; `���-�����` connects them without either.
...
-- `((������ x , q) , (x , refl))`, and reading the second coordinate of
-- `������ x` returns `false` for `inl b` and `true` for `inr tt` � which
-- is what §�'s `to` returns, on the nose.
--
...
-- THE ADDITIVE PREDICTION.  ������ is injective, so the naive ledger
-- charges it nothing (Unit); �������� costs one bit (§�, uniformly);
-- ������� costs one bit (§�).  Multiply the fibers � which is exactly
-- what `Sesa_��������` licenses WHEN ITS HYPOTHESIS HOLDS � and the
...
-- rounding: the predicted fiber and the actual fiber are not equivalent
-- types at all.
...
--
-- `Sesa_��������` � the clause under which the fibers multiply and the
-- logs add � has as its hypothesis that the first map has a UNIFORM
-- fiber: some Φ with fiber f y � Φ for every y.  ������ does not.  The
-- refutation needs both of §�'s ends at once and nothing else:
------------------------------------------------------------------------
...
-- THE ABSENCE, WITH BOTH ITS SLOTS FILLED (Nyya�Vaieika; see header).
--
--   pratiyogin (counterpositive � what is absent):  (false , true)
--   anuyogin   (locus � where it is absent):        the image of ������
--
...
-- absence is limited to the fiber of �������� over `false`, and §�'s two
-- hand computations are exactly the statement that under the OTHER
-- limitor � the fiber over `true` � there is no absence at all.  A bare
...
------------------------------------------------------------------------
-- � � THE CANCELLING CHAIN IS THE SAME PHENOMENON.
--
--     Unit --�������--> Bool --�������--> Unit
--
...
-- is not an anomaly.  It is §� again, with the same alignment term:
-- `fiber ������� tt` has two points and �������'s fiber over one of them
-- is EMPTY, so the � collapses to one � exactly as it did over `false`
-- in §�.
------------------------------------------------------------------------
...
--
-- (a) ONE chain is priced.  The result that generalises is §�'s, and it
...
--     two such edges behaves like §�, and the �-indexed case is the next
--     honest step.  `YugmaPurana_�`'s �/2 cut composed with `�����` is the
--     nearest chain in the corpus whose middle is not enumerable.
--
-- (c) The absence in §� was FOUND, not derived: the chain was chosen so
--     that ������ would miss a point of ��������'s fiber over `false`.
--     Whether a route through the corpus's directed graph can be
--     SEARCHED for its alignment terms � which points of which fiber a
