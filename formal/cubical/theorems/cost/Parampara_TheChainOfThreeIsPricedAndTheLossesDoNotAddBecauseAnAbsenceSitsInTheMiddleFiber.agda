...
-- *Gītā* states no map, no fiber, no composition law, and no theorem
...
-- one — because that is what a composite of three maps and its fibers
-- are.  The mathematics is cubical type theory (Voevodsky's univalence
-- and the fiber of a map), this repository's one admitted non-Indian
...
-- `Sesa_…शेष` — fiber (g ∘ f) z ≃ Σ[ p ∈ fiber g z ] fiber f (fst p) —
...
--
--  §१  the four fibers of प्रवेश, each named AS A TYPE, not as a count:
--         fiber प्रवेश (b , false) ≃ Unit   for both b
--         fiber प्रवेश (true , true) ≃ Unit
--         fiber प्रवेश (false , true) ≃ ⊥   ←— THE ABSENCE
--  §२  fiber प्रथमांश c ≃ Bool, UNIFORMLY in c.  One bit, everywhere.
--  §३  fiber विस्मरण tt ≃ Bool.  One bit.
--  §४  the middle composite मध्यम = प्रथमांश ∘ प्रवेश:
--         fiber मध्यम false ≃ Unit      (contractible — proved)
--         fiber मध्यम true  ≃ Bool      (NOT contractible — proved)
--      so the composite's fiber is NOT uniform, and §२'s uniformity has
...
--  §६  the whole chain.  fiber पूर्ण tt ≃ Bool ⊎ Unit — THREE.  The
...
-- composite's fiber.  They do not; the alignment term does, and §७ names
...
------------------------------------------------------------------------
-- १ · the four fibers of प्रवेश.
--
-- प्रवेश is injective — three points into four — so every fiber is a
...
------------------------------------------------------------------------
-- २ · the fiber of प्रथमांश, and it is UNIFORM.
--
...
------------------------------------------------------------------------
-- ३ · the fiber of विस्मरण.  One bit, and the only point of the base.
------------------------------------------------------------------------
...
-- Unit and ⊥ and प्रथमांश's fiber is uniform.  Both halves proved: the
...
-- प्रवेश-fiber over the first is Unit and over the SECOND IS ⊥.  So the
...
-- and ((true , true) , refl) — and §१ says BOTH प्रवेश-fibers are Unit.
...
हस्त-शेष-सत्ये = isoToEquiv (iso to from ri li)
  ...
  -- every प्रवेश-fiber over a point of this base is a proposition:
...
--
-- The total fiber, by hand: पूर्ण is constant, so its fiber over the
...
--
-- `Sesa_…शेषसमता` — the clause under which the fibers multiply and the
...
-- absence is limited to the fiber of प्रथमांश over `false`, and §५'s two
...
-- `fiber विस्मरण tt` has two points and स्थापना's fiber over one of them
...
-- … and शेष says where the bit went: into a base point whose fiber is
...
--     that प्रवेश would miss a point of प्रथमांश's fiber over `false`.
