...
-- `� = ΔReach/ΔKill` is not encodable without a cost model.  Both statements
...
--
--   B.  `� = 1` is proved for the sign defect --- not as a ratio of two
...
--
-- `Γ�(δ) : δ � (� : f � g)` promotes the defect to an identification and
...
------------------------------------------------------------------------
-- B.  `� = 1` for the sign defect.
--
-- `� := ΔReach(��) / ΔKill(Γ�)`, with `� = 1` the golden boundary between
...
-- What Γ^ closes: distinct points that the normalizer `abs�` identifies.
Killed : ℤ → ℤ → Type₀
Killed x y = (absℤ x ≡ absℤ y) × (¬ (x ≡ y))

-- Blindness of the divisibility field is exactly agreement of absolute
-- values.  (`��`-elimination is available because � is discrete; nothing
-- classical is used.)
...
-- `� = 1`: the widening opens exactly what the repair closes.  Not a measured
