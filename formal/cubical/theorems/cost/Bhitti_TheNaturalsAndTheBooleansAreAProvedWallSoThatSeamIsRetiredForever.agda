...
------------------------------------------------------------------------
-- ������� � a wall.  The first ford-graph receipt of the SECOND kind.
--
...
-- boundaries, anekntavda as network topology" � but the walls are not in
...
-- by `�`/`� � �` � 6 and 7 banks, joined into 13 by
-- `SetuYugma_�AndVivekaIsTheNaturalNumbers`.  So `� � Bool` is the single
...
-- equivalent to �; pigeonhole).  No source is claimed for it.  ������ is
...
------------------------------------------------------------------------
-- � � �������������� � the pigeonhole, at its smallest: of any three booleans,
...
------------------------------------------------------------------------
-- � � ������� � � � Bool.  Three naturals, two booleans; the collision the
-- pigeonhole forces is undone by injectivity into a false equation on �.
------------------------------------------------------------------------
...
-- is data and not a type: the pair (⟨lib⟩.�, ⟨lib⟩.Bool) is now a WALL and
-- must never again be counted as an unminted seam.  Suggested row shape for
--
--     wallId                        bankA        bankB
--     Bhitti_�ForEver.�������        ⟨lib⟩.�      ⟨lib⟩.Bool
--
