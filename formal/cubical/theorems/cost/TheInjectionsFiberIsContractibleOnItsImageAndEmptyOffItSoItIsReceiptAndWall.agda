...
------------------------------------------------------------------------
-- प्रवेशतन्तुः — the fiber of an ENTERING (a coproduct injection).
--
-- THE DUAL OF प्रक्षेपतन्तुः.  `PraksepaTantu_…` computed the fiber of a
-- PROJECTION `fst , snd : A × B → A , B`: a projection FORGETS a factor, and
-- its fiber IS the whole discarded factor — a genuine loss, priced at that
...
module TheInjectionsFiberIsContractibleOnItsImageAndEmptyOffItSoItIsReceiptAndWall where
...
module _ {A : Type ℓ} {B : Type ℓ'} where
  ...
  -- `inl a ≡ inr b` transports (encode) to `Lift ⊥`, so the fiber is empty.
  ------------------------------------------------------------------------
  वाम-प्रवेश-बहिः : (b : B)
