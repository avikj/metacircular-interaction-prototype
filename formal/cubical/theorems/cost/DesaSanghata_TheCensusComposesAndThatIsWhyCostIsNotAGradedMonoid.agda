...
------------------------------------------------------------------------
-- ���-��������� � the CENSUS composes, and the composition law is the reason
-- there is no shortest-path formulation of routing in this corpus.
--
-- WHAT WAS ALREADY THERE, and is used rather than reproved:
--   * `���-���������` (SankramanaSesa) � fibres compose:
--         ��� (g ∘ f) z  �  �[ w ∈ ��� g z ] ��� f (fst w)
--   * `���` (Loss.SakalaVikalaDesa) � the census as a TERM, three
--     constructors carrying their evidence: ����������� (empty fibre, nothing
--     lost, ����������), �������� (contractible), ��������� (two points, exhibited).
--
...
--
-- §� ���������-������� � an empty OUTER fibre is ABSORBING.  � ��� g z forces
--    � ��� (g ∘ f) z, whatever f is.  So arbitrary loss upstream of an
--    inexpressible point is INVISIBLE in the composite.  This kills
--    monotonicity: extending a path can hide cost already paid.
--
-- §� ����-���������� � a contractible OUTER fibre is TRANSPARENT.  With centre
--    (b , p), ��� (g ∘ f) z � ��� f b: the composite's census at z IS f's
...
--    is not needed.
--
-- §� ���������� � the cancellation, as a mechanism.  A CROWDED outer fibre
--    whose points have empty inner fibres except one contractible entry
--    yields a CONTRACTIBLE composite.  ��������� ∘ ����������� = ��������.  Two
--    genuine defects annihilate.  §� instantiates this at Unit � Bool �
...
-- associative accumulation, monotone under extension.  §� refutes
-- monotonicity and §� refutes any accumulation at all � the composite's
-- census is not a function of the two censuses, because §�'s outcome
...
-- � itself: cost is a SECTION over the codomain, and composition is
...
-- identifies as "every point of the census is ��������".
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- � � ��� and the composition law, restated locally at the level the
-- corpus states them, so this file stands alone under the kernel.
-- (���-��������� is SankramanaSesa's theorem; the proof term
...
------------------------------------------------------------------------
-- � � ���������-������� � the empty outer fibre swallows everything upstream.
...
------------------------------------------------------------------------
-- � � ����-���������� � the contractible outer fibre is transparent.
...
------------------------------------------------------------------------
-- � � ���������� � the cancellation, as the general mechanism.
--
-- The composite is the � of the inner fibres over the outer fibre.  So a
...
-- fibre.  ��������� above ∘ ����������� below can be ��������.
--
-- Stated as the exact criterion the � gives: the composite is contractible
-- exactly when the � is, and the � can be contractible while the outer
-- fibre is not.  §� exhibits that, minimally.
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- � � The minimal witness, recovered as a COROLLARY of §� rather than as
-- an example standing on its own.  SakalaVikalaDesa §3 computes these three
-- censuses by hand; here the third follows from the first two through the �.
------------------------------------------------------------------------
...
-- outer (����� at tt) is CROWDED: false and true both sit over tt
...
-- inner over `false` is EMPTY � this is the point the � deletes
...
--
-- §� is stated as "if the � is contractible then the composite is".  It is
-- NOT a function from (��� g z) and (��� f) to (��� (g ∘ f) z), and no such
-- function exists: §�'s outcome depends on WHICH point of the outer fibre
