...
--                           Î©Finâ‰Sym : Î©(Type, Fin n) â‰ Sâ™.
--
--  2. â•-algebra-Aut-trivial The initial (1 + X)-algebra is rigid, while
--     swap01-â‰-id           â• as a bare type is not.  Structure is what
--                           cuts symmetry down; that is the SIP.
--
--  3. â•â‰Tally / â•â‰CanWord   Three presentations, defined independently,
--                           with equivalences CONSTRUCTED (the digit one
--                           through the odometer `sucw` and injectivity
--                           of `value` on canonical words).
--
--  4. transport-+-is-âŠ•      Transporting â•'s addition along `ua` yields
--                           literally the schoolbook ripple-carry
--                           algorithm defined natively on digit words.
--     â•-Monoidâ‰¡CanWord-Monoid
...
--                           value map; complement is Ï-equivariant,
--                           reversal is not, and reversal instead
--                           exchanges the two truncations.  Place value
--                           is a chart.
--
--  6. â•â‰ÏâFinSet            â• is Ïâ of FinSet â” the set-truncation
--     cardâ‰¡MereEq           equivalence â• â‰ âˆ FinSet âˆâ assembled from
--     FinSetLoopâ‰Sym        the fiberwise statement cardâ‰¡MereEq; the
--                           numeral names a connected component, and
--                           what it forgets is the loop space Sâ™.
--
--  7. Controls              Canonicity is load-bearing, the big-endian
--                           misreading is refuted, and two deliberately
--                           wrong statements fail to type-check â” the
--                           raw equivalence (Control/WrongEquivalence)
--                           and CompileBridge Â§G1 at the wrong
...
-- The same two statements in the bare-probe-pool register of GTER Â§1 -- no
...
-- gain on the BUDGET coordinate.  Refutes SIXTEEN_MINDS_ONE_THEOREM Â§2's
-- open door 1.
...
-- S4 (D0026_BUILD_QUEUE Â§4): the same certificate language widened to
...
-- imports, not `public`: both carry local list predicates (All, _âˆˆ_)
...
import WalkBridge
-- independent second derivation of Â§(b), which also removes WalkBridge's
...
-- transport/fibre/sector toolkit, and the wÂr centre-relative instance.
...
import RepairTorsor
-- Delta 15 Â§Â§15.3, 15.4, 15.6 (owner-supplied, collab/upstream/raw/D0015):
...
-- `steps w â‰¡ suc (length w)` against the unary test's Î˜(value) is the whole
...
-- SÂ has local facts but no global section, while pulling it back to its
...
-- The relational SÂ obstruction and the tensor reconstruction obstruction
...
-- Cech/HÂ carrier: edge signs modulo context gauge, with cycle parity
-- descended to the quotient and identified with the derived Pauli sign.
import PMGaugeCohomology
-- Generic finite-graph Câ°â’CÂ gauge translation and representative-
...
-- Pauli X/Z and Zâ global phase preserve the checked norm.
...
-- functors from its path âˆž-groupoid, gauge changes are natural, and graph
...
-- a field the maps do not determine; `no-invariant-response-sees-Ï` is that
-- statement as a term.  Î“â is min-plus over neighbours -- the same operator
-- as DSOMinPlusFinite/DSOBellmanFinite on other data.
import Residual
-- ð’¦ := âˆ âˆ˜ Î“ and the trichotomy of its step: decay (the orbit reaches 0),
-- resonance (stationary), branching (never reaches 0).  Three theorems about
-- one â•-valued obstruction measure; the spectral radius is the sign of the
-- step, not a measurement.
import KFlow
-- Î´_end, by Lawvere/Cantor: for every quotation ââˆ’â : ð’ â’ (ð’ â’ Bool) the
...
-- and the two put together: the flow halting is a theorem about âˆ, Î´_end is
-- a theorem about ââˆ’â, and `halting-does-not-close` shows the second
-- survives the first -- completeness does not imply termination of enquiry.
import QuestionMachine
-- Chu(X,ð’¯,e): the defect is monotone in the test list, so a vanishing defect
-- is a statement about ð’¯ and never about X (the empty test list makes every
-- pair agree).  Also Î´_Ï = 0 â Î´_Ï^base = 0: the base can be flat while the
...
-- UsefulEscape > 0 is exactly Ï â‰ 0, hence a strictly cheaper presentation;
-- the non-theorem Î´ = 0 â’ Advance is exhibited as failing.
import AdvanceGate
-- Î“â's soundness was weaker than its own proof term: the witness it
-- returns is a MEMBER of the neighbour list.  With optimality and the
-- greatest-lower-bound clause this certifies Î“â to BE the minimum rather
...
-- The converse of `modw-zeroâ’âˆ`, hence `decDivides`; and the line that
-- makes it useful, `decDividesâ•-agrees`: the charted test is EQUAL to
-- CoprimeSplitting.decâˆ, so it substitutes in the walk lane without
...
-- = cap m Â capQuot m with capQuot m fixed by one residue-automaton pass
...
-- The walk's frontier, broken: next 8 â‰¡ 9, next 9 â‰¡ 11, next 10 â‰¡ 11, each
...
-- at least b^(Lâˆ’1), so the speedup is quantified over every base, every
...
-- recovered as the instance Î¼ = id.  The naive converse is false; what is
-- forced is exactly one strict decrease along the orbit.
import KFlowWF
-- Lawvere 1969, of which Î´_end is the Bool/`not` corollary.  Nothing here
...
-- by the charted test, with nextw â‰¡ next for every m and no side
...
base2-is-2 : Base2.b â‰¡ 2
base2-is-2 = refl

base10-is-10 : Base10.b â‰¡ 10
base10-is-10 = refl
...
--          cardinality shadow;
--   Â§8(a)  a certified normalizer emits Derivations, so learn = install âˆ˜ normalize
--          closes discoveryâ’installation on one Tm (reach unchanged: Siddhasadhana).
...
-- tr(AH), tr(AÂ²H) do not descend through it: witness pair over â, both
...
-- diag(n,n,âˆ’2n) (an injection of â•), and the pushforward of the true QÌ to
...
-- discriminant (linear algebra over â on ten coefficients, with the
-- symbolic derivative checked against the analytic one by the solver), and
-- â injects into the pushforward fibre over a (Q,R) point by scaling the
-- Hessian direction.
...
-- O holds exactly when ker O âˆ© (C âˆ’ C) = {0}; over â — â one observation
...
-- TriangularPair: the shear pair u^Ï = (0, a, Ïv) shares every Ï-even stress
-- invariant and has Ï-odd continuation, so continuation does not descend
...
-- paired with the receiver's autocorrelation â” positivity at h = f âˆ— fÌ is
-- an identity.
import BoundaryBlock_ThePrimeSideMeanSquareOfAReceivedSignalIsThePairFieldPairedWithTheReceiversAutocorrelation
-- BoundaryBlockGeneral: the same identity at every width â” for any finite
-- receiver and coefficient signal, Î (Î c_k S^k f)Â² â‰¡ Î c_k c_l Ï(|kâˆ’l|).
...
-- CyclicParseval: given orthogonal characters, Î_Î bÌ(Î)(Î) â‰¡ NÂÎ_t b_t c_t â”
...
-- an exact Fourier calculus on the torus â” divergence-free, nonlinear term
-- (0,0,Ï aâˆâv) itself divergence-free so the pressure is constant, coarse
...
-- FiniteExplicitFormula: Newton's identities â” power sums of the roots equal
-- the coefficient recursion, i.e. âˆ’zPâ²/P = Î p_k zµ coefficientwise â” and
...
-- ancestry interference (X+YÏ‰)(X+YÏ‰Ì) = (X+Y)Â² âˆ’ XY(2âˆ’Ï‰âˆ’Ï‰Ì); at the
-- orthogonal cube's Ï‰ = e^{2Ïi/3} in â[Ï‰] the amplitude is half the envelope.
...
-- TwoScaleSubgroup: 2µ = 3â¿ only at m = n = 0 â” the arithmetic behind the
-- density of the (2,3)-scale samples in the C(ð•‹Â²)-descent reading of RH.
import TwoScaleSubgroup_ThePowersOfTwoAndThreeMeetOnlyAtOneSoTheTwoGeneratorScaleSubgroupIsFreeOfRankTwo
-- HistoryCompletion: the one construction under both frontiers â” the value
...
-- truncations, and â–¡ (at every depth) refuted by one truncation and decided
...
-- with the exact one to depth M â” the support lemma of convolution with a
