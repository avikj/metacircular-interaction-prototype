...
open import Cubical.Data.SumFin.Properties using (SumFinâ‰¡Fin)
...
carriersAreEqual : (n : â„•) â†’ SF.Fin n â‰¡ F.Fin n
carriersAreEqual = SumFinâ‰¡Fin
...
isSetRidesAlong :
  (n : â„•) (s : isSet (SF.Fin n))
  â†’ PathP (Î» i â†’ isSet (carriersAreEqual n i)) s (subst isSet (carriersAreEqual n) s)
isSetRidesAlong n s = subst-filler isSet (carriersAreEqual n) s

symmetricGroupsAreEqual :
  (n : â„•) (s : isSet (SF.Fin n))
  â†’ SymGroup (SF.Fin n) s
  â‰¡ SymGroup (F.Fin n) (subst isSet (carriersAreEqual n) s)
