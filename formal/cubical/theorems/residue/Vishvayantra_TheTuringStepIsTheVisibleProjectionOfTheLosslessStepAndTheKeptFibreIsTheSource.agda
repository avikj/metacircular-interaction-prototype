...
------------------------------------------------------------------------
-- ������������� � the universal machine, inside the lossless one.
--
...
--
--  2. `lossless` � for ANY map f : A � B,   A � � b (fiber f b),
...
--     EQUATION.  A bare equivalence  e : A � � a' T(a')  need not have
--     visible projection `step`; the field `visible : �� ∘ e ∼ step`
...
--
-- The equivalence alone is not the law: `e : A � � a' T(a')` could
...
module _ {X : Type ℓ} (L : LawfulStep X) where
  ...
  private
    ...
    -- � X Trace � X � � X (fiber step); the homotopy slides the base
