...
--   A3 `�-recursor-unique`   the honest minimum the task names: the type
--                            of algebra maps � � � commuting with zero
--                            and suc is contractible.
--   A4 `�-algebra-endo-is-id`
--                            ... and its centre is the identity, so
--                            every such endomorphism is PATH-equal to
--                            `idAlgHom`.  This is the strengthening of
--                            `PathIsSymmetry.�-algebra-Aut-trivial`,
...
--
--   B1 `BS`                  BS� = �[ X ∈ Type� ] � X � Fin n ��, the
--                            type of n-element types (note §3.1).
--   B2 `OrdTotal`            �[ X ∈ BS� ] LinOrd n ⟨X⟩.
--   B3 `forgetTrunc`         OrdTotal n � �[ X ∈ Type� ] (X � Fin n):
...
--                            path space �[ X ∈ Type� ] (X ≡ Fin n).
--   B5 `isContrOrdTotal`     THE THEOREM: �[ X ∈ BS� ] LinOrd(X) is
--                            contractible.
--   B6 `linOrd-torsor`       the fibrewise companion (note Theorem 2.5):
--                            given one order on X, the type of orders on
--                            X is equivalent to S� = (Fin n � Fin n).
...
--    half lives in `FreeMonoid`, which builds � � Tally
...
--    `Decategorification`.  No universe-polymorphic BS�, no comparison
--    with `Cubical.Data.FinSet`'s `FinSet`, and no claim that BS� is
--    connected (it is, by the truncation, but connectedness is not used
--    and not stated).
--
--  * B6 exhibits an equivalence, not a group action.  No torsor
--    structure is packaged, no S�-action is defined, and freeness and
...
-- entire point: contractibility of this � is a strictly stronger
...
------------------------------------------------------------------------
-- 2.  � is the initial (1 + X)-algebra, and the comparison type is
...
------------------------------------------------------------------------
-- A2, RE-DERIVED FROM THE LIBRARY AND STRENGTHENED.
--
-- `Cubical.Data.Nat.Algebra`, a module of the
-- library this file already depends on, following
-- Awodey-Gambino-Sojakova (arXiv:1504.05531), proves
--
--     isNatHInitial� : (M : NatAlgebra �) � isContr (NatMorphism NatAlgebra� M)
--
...
module _ {B : Type ℓ} (b₀ : B) (bs : B → B) where
  ...
  -- interchangeable: record eta and � eta on one side, and the fact
...
-- (`AlgHom �Alg B` reduces to `AlgHom∞ (pt B) (op B)`).
ℕ-isInitial : (B : Alg ℓ) → isContr (AlgHom ℕAlg B)
ℕ-isInitial B = ℕ-isInitial∞ (pt B) (op B)

-- A3.  The honest minimum: algebra maps � � � commuting with 0 and suc
...
------------------------------------------------------------------------
-- 4.  Theorem 3.2: the total space of orders over BS� is contractible.
--
-- `LinOrd` is DEFINED as `X � Fin n`.
-- What is checked is that the truncation in BS� is redundant here, and
...
-- a statement about rank listings; the passage to genuine linear orders is
-- `LinearOrderFinite`'s fibrewise equivalence.
LinOrd : ℕ → Type₀ → Type₀
LinOrd n X = X ≃ Fin n
...
--     the space of orders is not contractible but a copy of S�.
--
...
-- truncates": the orders on one set form an S�-worth of choices, and
