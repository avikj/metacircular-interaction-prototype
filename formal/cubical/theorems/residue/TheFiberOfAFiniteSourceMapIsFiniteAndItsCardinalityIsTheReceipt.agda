...
------------------------------------------------------------------------
-- सङ्ख्या-तन्तुः — the fiber of a finite-source map is finite, and its
-- cardinality is the  receipt.
--
-- For any map f : A → B between FINITE SETS, every fiber is a finite set
...
-- itself the अभिज्ञान (the untruncated identification `fiber ≃ Fin k`),
-- which needs the actual bijection and is available only for a concrete
-- map.
------------------------------------------------------------------------

module TheFiberOfAFiniteSourceMapIsFiniteAndItsCardinalityIsTheReceipt where
...
module _ (A B : FinSet ℓ) (f : A .fst → B .fst) where
  ...
  ------------------------------------------------------------------------
  -- १ · the fiber is a finite set — the library's isFinSetFiber, named
  ...
  ------------------------------------------------------------------------
  -- २ · सङ्ख्या — the cardinality of the fiber, the count receipt.
