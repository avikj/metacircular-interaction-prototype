...
------------------------------------------------------------------------
-- �������-�������� � collision-interference.  The sum-spectrum "variance"
...
--
--   V∞ = �_s | �_{γ_i+γ_j = s} W_{ij} |²     (variance: group by
...
-- landing on ONE frequency), over � (the structural identity needs a
-- ring, not the reals; conjugation is trivial on �, W_{ij}² = |W_{ij}|²
-- for real weights):
--
...
-- conflate it with �������.
--
-- CHECKED under the pin (Agda 2.8.0 + cubical v0.9) for the � ring
