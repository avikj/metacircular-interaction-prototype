...
-- land in ℕ.  A cut with profiles on BOTH sides needs `up` to produce a
...
------------------------------------------------------------------------
-- THE NON-EMPTY-ROW RIGHT ADJOINT.  Folding `maxP` over `Rows (j ∷ js)`
...
-- `dnNE` takes its rows in `j ∷ js` form, so the restriction is recorded
