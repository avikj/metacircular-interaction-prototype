...
--
-- The note adjudicates D0020 §7's splicing defect �_�� against D0019
-- §D's δ_�.  Its Theorem 4.2, under the note's hypothesis (H), reads
--
--     �_�� = [δ_�]  in  K�(�(��,��)),    hence  δ_� � 0 � � = 0,
--
-- and its §4.3 refutes the converse with the two-line witness in
-- D^b(Vect_k): ���� = 0 between two copies of k gives
-- cofib(0) = k ⊕ k[1] � 0 while �(k ⊕ k[1]) = 1 − 1 = 0.  The note's
-- §4.3 conclusion � §7's own conditional "� = 0 � �� sufficient" is
-- UNSOUND, its contrapositive sound � is what this module makes a term.
--
-- WHAT IS FORMALIZED
--
-- The information-loss structure: a "defect" valued in a
-- type D with a distinguished zero, an invariant � : D � A killing that
...
-- even/odd total ranks, � � �, replaces K� � �-by-Euler-characteristic
-- with the honest difference m − n : �, and replaces the note's
-- k ⊕ k[1] with (1 , 1).  Under that replacement the note's witness is
-- reproduced exactly: the model's �(1,1) = 1 − 1 = 0 while
-- (1,1) � (0,0).
...
--       vanishes��-vanishes   trivial direction: d ≡ 0 � � d ≡ 0
...
--       sound-contrapositive  � d � 0 � d � 0, unconditionally
--       unsound-certificate   the certificate rule "� d ≡ 0 � d ≡ 0"
--                             is refuted by a witness
--
--   §2  the finite model, and the witness inside it
--       Rank, ��, witness-�-vanishes, witness-nonzero, model-unsound
--
--   §3  exactly when � reflects zero, in the model:
--       kernel-char : �� (m , n) ≡ 0  �-in-content-to  m ≡ n
...
--       discrete): no witness � ReflectsZero.  So "� reflects zero"
...
--
-- `D` stands for the defect object (the note's δ_�, an object of the
-- stable hom-category), `A` for the receptacle of the invariant (the
-- note's K�), `�` for the class map, `0D`/`0A` for the two zeros.  The
-- only hypothesis is �-zero � which is the note's [0] = 0, Thm 4.2(3).
------------------------------------------------------------------------
...
module Invariant {D : Type ℓ} {A : Type ℓ'}
  ...
  -- The easy direction.  This is Thm 4.2(3): δ_� � 0 � � = 0.
  ...
  -- The property that the converse REQUIRES: � is injective at the
  ...
  -- A witness that the certificate "� vanished, so the defect vanished"
  ...
  -- inference rule "� d ≡ 0A, therefore the construction is
...
-- Euler characteristic = the difference in �.  The note's cofib(0) for
-- ���� = 0 : k � k is k ⊕ k[1], whose ranks are (1 , 1) and whose
-- Euler characteristic is 1 − 1 = 0.
--
-- THE MODEL.  It proves that the abstract shape of §1 is inhabited by
-- a concrete �, which is all that is needed to make `Unsound` a
...
--
-- In the model the kernel of � is precisely the diagonal: �(m,n) = 0
...
------------------------------------------------------------------------
-- §4.  When does � reflect zero?  Exactly when there is no witness.
--
...
-- discrete � as it is in the model.  With it, "� reflects zero" and
...
-- And so, closing the circle: in the model � does NOT reflect zero,
