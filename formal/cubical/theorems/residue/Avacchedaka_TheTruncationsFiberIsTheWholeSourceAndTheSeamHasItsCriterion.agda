...
-- crowded fibers, both landing in विकलादेश.  Level ३ is recoverable only
...
-- both.
--
-- them — the SIZE of the fiber relative to the source — and marks the
...
-- ────────────────────────────────────────────────────────────────────
--
-- ESTABLISHED (§२): for propositional truncation, EVERY fiber is
...
--
-- NOT ESTABLISHED: that "fiber ≃ whole source" is THE criterion
-- separating ३ from ४ in general.  §२ settles the level-४ side by
-- exhibiting the archetype; the note's level-३ side (`fiber अनर्पणम्
...
------------------------------------------------------------------------
-- १ · तन्तुः — the fiber of the truncation unit over a point of ∥ A ∥₁.
...
--
--     Every fiber of ∣_∣₁ is equivalent to the whole source.
--
...
--
-- ~~This is what विकलादेश could not say.  विकलादेश says the fiber has two
...
-- map that drops exactly one bit � `Residue_�` §5, which its own struck
...
-- level-२ archetype, and the two fiber censuses are pointwise equivalent.
--
-- §२ of THIS file is untouched and still true: every fiber of `∣_∣₁` is
...
-- is the level-३ side: that a level-३ collapse's fiber is a PROPER part,
...
-- all; and the level-३ half quoted here — a proper fiber, with something
