...
--    unbounded â” the empty meet is `âˆž`, which â• does not have â¦ a
--    two-sided cut exists over a non-empty residual index set, or over
--    `â• âŠ âˆž`, and not over â• with an arbitrary index set."
--
-- That is wrong.  The burden side is ordered by `_âŠp_`, REVERSE
...
-- the residual index and no `âˆž` anywhere.  `dnNE`'s non-emptiness hypothesis
