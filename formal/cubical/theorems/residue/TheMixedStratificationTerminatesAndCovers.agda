...
------------------------------------------------------------------------
-- TheMixedStratificationTerminatesAndCovers
--
...
-- `theMixedStrataAreOrdered`, DARWIN §5.2's three output properties
...
module TheMixedStratificationTerminatesAndCovers where
...
open import PerCoordinateCapsAreTheHonestVersionAndOneCapIsTheSpecialCase
  using (Caps)
open import TheParetoMaximumTransfersToCostCoordinates
  using (MixedMaximal ; AllBounded ; mixedMaximalExists)
open import TheMixedStratumIsExactlyTheFlippedStratum
  using (decMixedMaximal ; mixedStratum)
open import TheWholeMixedStratificationIsTheFlippedOne
  using (mixedRemainder ; mixedStrata ; allBoundedFilterOut)
open import TheMixedLayerCoversAndBeatsTheMixedRemainder
  using (mixedLayerCovers)
open import TheMixedStrataArePairwiseDisjointAndOrdered
  using (MemSomeM)
