...
------------------------------------------------------------------------
-- घाततन्तु — the discrete log is the fiber of Piṅgala's power, and Shor's
...
-- repository's ONE theorem — the quotient/fiber law (`
...
--   and घात g 3 are BOTH ε, two distinct points of one fiber, so the
--   fiber over ε is not contractible — proof, not assertion.
--
...
--                                                          determined: 0 and 3
--                                                          both land on ε
--   §3  क्रमः-तन्तुः  : the fiber over ε contains a full period — 0, 3, 6
...
-- machine has no such query for घات's fiber — that is the hardness
...
module TheDiscreteLogIsTheFiberOfPingalasPowerAndShorsPeriodQueryIsWhatReadsIt where
...
open import AConcreteKeypairRunsInACyclicGroupWhereTheModThatExhaustsTheHeapIsNotNeeded
...
------------------------------------------------------------------------
-- §3  The secret road is not: the fiber over ε is not contractible,
...
-- the period is visible in the fiber: 0, 3, 6 all land on ε, spacing r=3
षट्कः : fiber powg εC
...
------------------------------------------------------------------------
-- §4  The reading, stated: the fiber-law's two bindings ARE the crypto
--     asymmetry.  §2 is Abhijnana's अभिज्ञानं-मुक्तम् at f = powg (the
--     public value is free); §3 is the input-binding fiber being
--     non-contractible (the secret is undetermined).  Shor's period query
--     reads the r=3 spacing of §3's fiber; classically there is no such
