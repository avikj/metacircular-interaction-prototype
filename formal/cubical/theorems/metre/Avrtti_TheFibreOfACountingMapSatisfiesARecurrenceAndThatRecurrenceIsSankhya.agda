...
-- WHAT IS PROVED.
--
-- ¬ß‡ß  the general recurrence, for ANY element type: the fibre of `length`
...
--     recurrence is the fibre's recurrence.  ‡‡®‡‡¶‡‡‡æ‡‡‡‡‡∞‡Æ‡ ‡Æ.‡®‡‚ì‡®‡Æ
...
-- is the reason: the fibre over `suc n` is `Syllable ó (fibre over n)`,
-- and a syllable is ‡≤‡ò‡ or ‡ó‡‡∞‡ and nothing else.  **The count's
-- recurrence IS the fibre's recurrence** ‚î the procedure and its ground,
...
-- charges 1 for ‡≤‡ò‡ and 2 for ‡ó‡‡∞‡, so its fibre over `n` decomposes as
-- the fibre over `n ‚à 1` plus the fibre over `n ‚à 2`, and each summand
...
--
-- ~~The general shape it would have: for `f : List X ‚í ‚ï` with
...
-- proof, which is why the weighted case is a rung above ¬ß‡ß and not a
-- corollary of it.~~
--
-- **STRUCK 2026-08-23 ‚î THE SHAPE CANNOT HOLD AS STATED, and the rung is
...
-- constructs a fitting proof.  `f (x ‚à xs)` REDUCES to `w x + f xs`, so
-- the path is carried across unchanged and both round trips close by
-- `refl` ‚î the guard this paragraph anticipated does not appear.  The
-- `Œ[ m ]` form above is equivalent anyway, since `Œ[ m ] (f xs ‚â° m) ó ‚¶`
...
-- exit 0.  The rung IS above ¬ß‡ß ‚î that part stands ‚î but for the reason
