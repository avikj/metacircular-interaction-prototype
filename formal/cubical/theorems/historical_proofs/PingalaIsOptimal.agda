...
--
--     uddistaIso : (n : ‚ï) ‚í Iso (Vak n) (Fin (count n))
--
...
-- LOSSLESS OBSERVATION WITH AN EXPLICIT DECODE ‚î the positive pole of
...
--       ‚í Injective obs ‚í count n ‚â card Y
--
...
VakFinSet n =
  Vak n , count n ,
  ‚à£ compEquiv (isoToEquiv (uddistaIso n)) (invEquiv (sumFin‚âÉFin (count n))) ‚à£‚ÇÅ

card-Vak : (n : ‚Ñï) ‚Üí card (VakFinSet n) ‚â° count n
card-Vak n = refl
...
--
-- `uddistaIso n : Iso (Vak n) (Fin (count n))` ‚î the target has
...
------------------------------------------------------------------------
-- 4.  The mtr-vtta case, which ¬ß3's "not claimed" listed as unrun.
--
