...
------------------------------------------------------------------------
-- ����������-�������� � ����� ����������� ������� �
-- KuttakaSamapti � the vall is finite, for every pair.
--
-- SOURCE AND DATE.  RYABHAA, �����������, ��������� ����� (499 CE): the
-- ����������, the pulverizer, for the linear indeterminate equation; the
-- procedure spelled out step by step by BHSKARA I, ���������������� (629 CE).
-- Its rule is one sentence � divide, KEEP THE REMAINDER AND RECURSE ON IT,
-- and write each quotient into the �����, the column � and CLAUDE.md quotes
...
-- remainder, and a remainder is smaller than what one divided by.  "���� �����,
-- ������� ������������" (AHIMSA_SUTRA_VISTARA §16, §17) is a descent as much as
...
-- given a `Run a b g`, back-substitution up the vall yields x, y with
...
--
--     "Termination is carried by the evidence � the run IS ryabhaa's
--      vall, a checked trace � so no well-founded recursion is needed."
--
...
-- someone handing over such evidence, and `Kuttaka.example` � the vall of
...
--
--   �����            the descent as evidence, over �, CARRYING ITS MEASURE:
...
--   ����������-��������    TERMINATION.  For EVERY pair (a, b) of naturals a vall
--                   exists, by well-founded recursion on b � ryabhaa's own
...
--   ���������-����       THE BOUND, DERIVED.  The vall of (a, b) has at most b
--                   rows.  Not measured, not fitted, not "observed to be
--                   short": an induction on the vall whose only input is the
--                   `r < b` each row carries.
--   ���-�����        THE REMAINDER OF A BOUNDED RUN.  A run given f rows of
--                   fuel either finishes � handing back a whole vall � or
...
--   �����            THE BRIDGE.  A � vall IS a `Kuttaka.Run` over �, so
--                   every theorem in `Kuttaka.agda` becomes unconditional:
--   ����-�������      for EVERY pair of naturals there are x, y with
--                   pos a � x + pos b � y ≡ pos g, with g the terminal value,
--   ������-�������    and g divides both and is divisible by every common
...
--   * The ����� section � reduction of the solution family to its least
...
------------------------------------------------------------------------
-- � � ����� � the descent as evidence, carrying its measure.
--
-- One constructor per division of the pulverizer.  `��������` is the row
-- where the remainder has become 0 and the column stops; `����` is one
...
------------------------------------------------------------------------
-- � � ����������-�������� � TERMINATION.  A vall exists for every pair.
--
...
private
  ...
  -- a ≡ q�b + r that a row of the vall wants.
  छेद-समीकरणम् : (n a : ℕ)
...
------------------------------------------------------------------------
-- � � ����������� � the length of the column, and THE BOUND, derived.
--
-- The vall of (a, b) has at most b rows.  The proof reads off the `r < b`
...
-- AHIMSA_SUTRA_VISTARA: "����� � �������� � ����", the vall is not a
...
------------------------------------------------------------------------
-- � � ���-����� � a BOUNDED run, and what it did not reach.
--
-- This is §8 of AHIMSA_SUTRA_VISTARA carried into the engine: "��� ���� ����
-- ��� � ������������" � what states its remainder does not collapse.
--
-- `���-����� a b` has exactly two constructors, which is §6's two paths and
-- no third: either the run FINISHED, and hands over the whole vall, or it
...
------------------------------------------------------------------------
-- � � ���������� � THE FUEL IS ENOUGH, which is what §3's bound is FOR.
--
...
-- and so the cap `b` never fires: stated as the vall the bounded run
...
------------------------------------------------------------------------
-- � � ����� � THE BRIDGE.  A � vall IS a `Kuttaka.Run` over �.
--
...
-- THE KUAKA, UNCONDITIONALLY.  For every pair of naturals, coefficients.
बेजु-सर्वत्र : (a b : ℕ)
          → Σ[ x ∈ ℤ ] Σ[ y ∈ ℤ ]
...
------------------------------------------------------------------------
-- � � �-�������� � non-vacuity, at the stra's own numbers.
--
-- AHIMSA_SUTRA_VISTARA §17 works the kuaka on (137, 60) and prints its
-- vall: 2 3 1 1 8.  The kernel is asked for the same column, from
-- `����������-��������` and not from a hand-built run, so this checks that the
-- termination proof COMPUTES the algorithm rather than merely asserting a
-- vall is out there somewhere.
------------------------------------------------------------------------
