...
------------------------------------------------------------------------
-- ���������� � every element of a finite group is annihilated by the
...
-- DischargedOnACyclicGroup.agda` proves `����-�������` for a group GIVEN
...
--      cardinality; nothing below counts anything.  §� is the cyclic
...
--
--     lagrange : card FG ≡ index �� order        (order = card of H).
--
...
--
-- WHAT IS PROVED.  For every group `G : Group �` (the library's
-- `Cubical.Algebra.Group`), every `finG : isFinSet ⟨ G ⟩`, and every
-- element `a : ⟨ G ⟩`, writing `a ^ n` for the n-fold product:
--
--   §1  `Least`      for a decidable proposition-valued `P : � � Type`,
--                    a least witness from any witness, and a decision of
--                    `� k < n, P k` by bounded search;
--   §2  `Powers`     `a ^ (m + n) ≡ a ^ m � a ^ n`, `a ^ (m �� n) ≡ (a ^ m) ^ n`,
...
--                    `����` vocabulary of Bijamula/Avarta, and hence RSA
--                    correctness (`������-������`) with NO Euler hypothesis
--                    and no cyclic hypothesis: only the pulverizer's
--                    witness `e �� d ≡ |G| �� k + 1` remains.
--
...
-- `e` elements.  (iii) Lagrange gives `|G| = index �� e`.  (iv) Then
-- `a ^ |G| ≡ (a ^ e) ^ index ≡ 1g ^ index ≡ 1g`, which is exactly the
-- arithmetic Avarta's `����-������` used, now with `e` supplied by
-- counting rather than assumed of a generator.
--
-- WHAT IS **NOT** CLAIMED.
--   * Nothing about ANY particular (�/n)� being finite-as-a-`FinSet`,
...
--
-- ��������������������������������������������������������������������
-- ON THE NAME.  ���� (all) + ����� (a turning; the word Avarta chose for
...
-- Indian, and is not dressed as Indian; only §7's vocabulary (����,
...
-- � multiplication is `_��_` throughout, the group operation is `_�_`
...
-- the pigeonhole is stated for the �-built `Fin` of Cubical.Data.Fin.
...
------------------------------------------------------------------------
-- §1  Least witnesses of a decidable predicate on �, by bounded search.
------------------------------------------------------------------------
...
--     `����` vocabulary, and RSA correctness with no Euler hypothesis
