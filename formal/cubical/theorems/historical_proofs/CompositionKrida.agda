...
------------------------------------------------------------------------
-- BhavanaKrida � ������������, the composition as something a child holds.
--
...
--
-- A ���� (patra, "leaf, card") of index k is a pair of numbers TOGETHER WITH
...
--
--   * the single move, �����, takes two cards and returns a card, and its
--     TYPE carries what happened: ���� D k� � ���� D k� � ���� D (k� � k�).
--     Playing is proving.  The child never checks anything; the checking is
--     what the move IS.
--
--   * reaching `���� D (pos 1)` is winning, and it is a solution of
...
-- value for �2:
--
--     Baudhyana ulbastra 1.61�62 (c. 800 BCE)
--     ������� ������� �������� ���� ���������������������������� �������
...
-- here.  Nobody multiplies 577 by itself.  The `�����` field of rung eight
-- is constructed by ����� out of the algebra, so the equation holds because
...
-- the same fact as `N ���� (pos 577) (pos 408) ≡ pos 1` does not finish
-- inside ninety.  Cubical � multiplication is unary recursion on its first
...
--
-- (`�����-��������` verifies one small rung the slow way, at 17 and 12, so
-- that the two routes agree at least once where the agreement is cheap.)
--
-- Contents: no postulates, no holes, --safe.
--   ����            a card: two numbers and the proof, indexed by the norm
--   �����           the one move; Brhmasphuasiddhnta 18, 628 CE
--   ���             the seed at D = 2
--   ������ .. ������  the eight rungs, each with its index in its type
--   ��������� .. �     the coordinates of every rung, by refl
--   ���             rung eight: a solution of x² − 2y² = 1
--   �-������         a pair that is not a card, checked
--   �����-��������    one rung cross-checked by computation
------------------------------------------------------------------------
...
-- 1.  The card.
--
-- `N D x y` is x² − D y².  A ���� D k is a pair carrying the proof that its
...
-- 2.  The move.
--
-- BRAHMAGUPTA, Brhmasphuasiddhnta 18 (628 CE), the samsa-bhvan.
...
-- BAUDHYANA.  ulbastra 1.61�62, c. 800 BCE: 577/408.
...
--
-- `���� ���� (pos 1)` is a solution of x² − 2y² = 1.  Rung eight is one,
-- and its `�����` field IS the equation 577² − 2�408² = 1 � held, not
-- computed.
------------------------------------------------------------------------
...
-- above plus §4.  `���` has type `���� ���� (pos 1)`, whose ����� field is
-- exactly N ���� x y ≡ 1 for that card's own x and y; `���������` says those
...
--
--     ��������� : N ���� (pos 577) (pos 408) ≡ pos 1
--
-- I wrote it, twice � once directly as `����� ������`, once as a transport
...
-- and cubical � multiplication is unary recursion on its first argument, so
...
--
-- The type is the referee.  There is no ���� ���� (pos 1) with coordinates
