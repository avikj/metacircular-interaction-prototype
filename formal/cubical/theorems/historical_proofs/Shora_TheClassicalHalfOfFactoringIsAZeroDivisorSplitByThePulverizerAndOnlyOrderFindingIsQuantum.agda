...
------------------------------------------------------------------------
-- ���� � the classical half of quantum factoring is a zero-divisor split
...
--      ���� a r ≡ 1 (mod N).  The one step no classical algorithm is
...
--      SAME inverse-of-���� that breaks RSA and DH (`Bijamula` §4,
--      `Samvit` §4, `GhataViparyaya`).
--   3. THE SPLIT.  Given such an r, set y = ���� a (r/2).  Then y�y ≡ 1
...
--       makes the product vanish.  In �/N with y � �1 neither factor is
--       0, so N divides (y−1)(y+1) without dividing either � which is
--       exactly a non-trivial common factor, extracted by gcd.
--
--   §3  the same, read as the factorisation of ���� a r − 1:
--       �������-���� shows ���� a r ≡ 1 with y = ���� a (r/2) gives the
--       zero-divisor, connecting order-finding's output to the split.
--       (y�y ≡ ���� a r is `Bijamula.����-������`; here taken as hypothesis
--       `sq` so this module stays ring-only and does not re-import.)
--
-- WHY THIS MATTERS FOR THE NIGHT'S ARC.  RSA and DH break because ���� is
...
module Shora_TheClassicalHalfOfFactoringIsAZeroDivisorSplitByThePulverizerAndOnlyOrderFindingIsQuantum where
...
module _ {A : Type ℓ} (R : CRing A) where
  ...
  -- derived: � is a right annihilator, and (� x) ⊗ y ≡ � (x ⊗ y)
  ⊗𝟘 : (x : A) → x ⊗ 𝟘 ≡ 𝟘
  ⊗𝟘 x =
  ...
  ⊗neg : (x y : A) → x ⊗ (⊝ y) ≡ ⊝ (x ⊗ y)
  ⊗neg x y =
    -- (x⊗y) ⊕ (x⊗(�y)) = x⊗(y ⊕ �y) = x⊗� = �, so x⊗(�y) is the inverse
      sym (⊕id (x ⊗ (⊝ y)))
  ...
  ------------------------------------------------------------------------
  -- §3  Read as order-finding's output.  If y = ���� a (r/2) and ���� a r
  --     ≡ �, then y⊗y ≡ � (that step is Bijamula.����-������, here the
  --     hypothesis `sq` so this file stays ring-only), and §2 delivers
  --     the zero-divisor.  The factor of N is then gcd(y � �, N) � the
