...
-- NOT claimed as Piṅgala's or Virahāṅka's — neither wrote a list, a fiber,
...
-- self-equivalence — an involution — of every duration-fiber.  The
-- substrate (List, rev, univalent fibers) is cubical type theory
...
-- splits `fiber छन्दः (2+n)` into two smaller fibers; it never touches the
...
------------------------------------------------------------------------
-- ४ · THE FIBER EQUIVALENCE (A ≃ A, inhabited by reversal, not identity).
-- Because छन्दः is a set-map, each duration-fiber's witness is a
...
-- Applying विलोम twice is the identity on each fiber (rev-rev), so विलोम is
