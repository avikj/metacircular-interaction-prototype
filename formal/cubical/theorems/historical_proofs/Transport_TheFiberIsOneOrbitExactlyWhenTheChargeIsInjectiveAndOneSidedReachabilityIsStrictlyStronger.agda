...
--
-- (when is the fiber exhausted by one orbit ‚î the converse of descent.)
--
...
-- every ordered pair in the fiber ‚î and `‡‡Æ‡æ‡®‡ï‡ï‡‡‡‡Ø‡æ` is the TWO-SIDED
...
-- equivalence, the fiber being one orbit in the meeting sense) while
...
-- `‡‡ô‡‡ï‡‡∞‡Æ‡‡Æ‡ b` implies no invariant separates the fiber; that theorem
...
-- "the flow carries one point of a fiber to another" is `Kaksya_‚¶agda`'s
...
module _ {A B : Type ‚Ñì} (f : A ‚Üí B) (Œ¶ : A ‚Üí A)
  ...
  -- ‚ü : the fiber is one orbit, so the charge separates classes
...
--
--     isEquiv ‡‡µ‡‡‡∞‡‡‡  ‚â  isSurjection f ó (fiberwise transitivity)
--
...
--
-- Conservation is `refl`.  The single fiber is all of `Bool`.
--
...
--     the fiber IS one orbit in the only sense the quotient can see.
--
...
private
  ...
  -- TWO-SIDED: any two points of the single fiber meet at (1 , 1)
...
--
-- **Sufficient conditions are not surveyed.**  ¬ß‡ reduces "the fiber is
