...
-- identification graph: 10 nodes, diameter 3, hub `�` at degree 7.  Among
-- its pairs at DISTANCE 2 is
--
--       Pingala.������  ��[ Pingala.������≡� ]��  �
--                       ��[ Digits.�≡CanWord ]��  CanWord
--
-- and the two banks had never been joined: nothing in this corpus puts
-- Pigala's �������� and a base-b positional numeral in one statement.
-- `Setubandha_ThePrastarasNextRow�` walked the OTHER distance-2 route out
-- of ������, through `�` to `Tally`.  This file walks this one, reusing
...
-- case split on ����/�����, no digit recursion, no carry lemma.  The one
-- definition that looks like a construction � `��������������` � repackages
...
--
--   �  �������������� � Pigala's next-row rule, transported along the
...
--      abstract stand-in: `������-�����` says it is "read both words, add
--      the values, re-digit", and `�����-������` says `valueC` is a
--      homomorphism for it.  So it really is base-b addition � obtained
--      without ever writing the addition-with-carry algorithm.
--
--   �  THE THIRD BANK.  Composing with `FreeMonoid.�-Monoid≡Tally-Monoid`
...
------------------------------------------------------------------------
-- � � THE SECOND HALF OF THE ROUTE, made explicit.
--
--     `Digits.agda` proves `value-sucw`: the odometer computes the
--     successor.  Read across `ua ��CanWord` that says exactly: the
...
--
--     `Monoid � = �[ A ∈ Type � ] MonoidStr A`.  Transporting the
...
--     cannot be recognised.  It can:  `������` is "read both words, add
...
--     the composite is a path in `Monoid �-zero` and required no third
--     argument.
------------------------------------------------------------------------
...
-- � � THE SCOPE, EXACTLY.
--
--   * That `������` is ryabhaa's or anyone's ADDITION ALGORITHM.  It is
...
--   * That `������≡CanWord` respects metrical structure.  It is built
--     from `�����`, a numerical encoding, so the induced operations on
--     ������ are operations on ��������-indices, not on syllables.
