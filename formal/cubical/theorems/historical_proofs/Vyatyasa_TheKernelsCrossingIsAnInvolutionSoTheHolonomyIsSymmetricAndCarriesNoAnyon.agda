...
------------------------------------------------------------------------
-- �������� � the crossing, and the fact that it undoes itself.
--
-- TERM.  �������� � vyatysa � interchange, transposition, reversal of
...
--     `eval : Tm � Env � �` provably cannot see � its �-meaning is `refl`,
--     forced, because `isSet�`.  Holonomy with no set-valued observable.
--
...
--
--   §4  AND THE CROSSING IS AN INVOLUTION.  �� ∘ �� and �� ∘ �� are the
...
-- group B� the generators have INFINITE order; �² = 1 is exactly the extra
-- relation that collapses B� onto the symmetric group S�.  Non-abelian
-- anyonic statistics live in the monodromy �² � that is the whole content
...
-- Here �² IS the identity, provably, at the level of paths in the
-- universe.  So:
--
--   THE KERNEL IS REVERSIBLE, LOSSLESS, AND CARRIES GENUINE HOLONOMY THAT
--   NO SET-VALUED READOUT CAN SEE (`Ankapasa_` §4, `Residue_` §4) � AND THAT
...
-- interpretation of `add` by `�` inherits �² = 1 and can do no better.  A
-- braiding with �² ≠ 1 would have to come from a different interpretation
-- of `add`.
--
...
--
-- `⟦ add var (add var var) ⟧ �` with every coordinate read by the same
...
--
--   �����-������  fires `add-comm` on the INNER pair: `�-equiv id �-swap-�`.
--   �����-�������� fires it on the OUTER pair, which needs the reassociation
