...
-- than inventing a  label").  The substrate â” `transport`, `Iso`,
...
toFrom : (t : M) â†’ toTuple (fromTuple t) â‰¡ t
toFrom (a , b , c , d) = refl

fromTo-pt : (A : Mat 2 2) (i j : Fin 2) â†’ fromTuple (toTuple A) i j â‰¡ A i j
fromTo-pt A zero       zero       = refl
fromTo-pt A zero       (suc zero) = refl
fromTo-pt A (suc zero) zero       = refl
fromTo-pt A (suc zero) (suc zero) = refl

fromTo : (A : Mat 2 2) â†’ fromTuple (toTuple A) â‰¡ A
...
homMul : (A B : Mat 2 2) â†’ toTuple (A â‹† B) â‰¡ mul (toTuple A) (toTuple B)
homMul A B i =
