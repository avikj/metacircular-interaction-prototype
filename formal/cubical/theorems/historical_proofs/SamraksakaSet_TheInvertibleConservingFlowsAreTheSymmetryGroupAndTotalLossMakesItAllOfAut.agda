...
--
-- ��������������������������������������������������������������������
-- WHAT THIS IS.  `SamraksakaGana_�.agda` §�(b) hands one remainder
-- forward: "The invertible elements of the gaa � flows with conserving
...
--
--   §�  inverse data is a PROPOSITION (inverses in the gaa are unique,
--       by the usual two-line monoid argument, executed not cited), so
--       the carrier �[ � ] �����������-��� � adds no structure to the flow;
--   §�  the carrier is a `Group` of the library's own algebra, with
--       every law a ���� away from refl and both inverse laws being
--       LITERALLY the stored inverse evidence;
--   §�  the group rides ���� onto the sections: a unit's image under
--       the section identification is �-invertible, each direction one
--       `cong` � because `���-������` was refl, the group structure was
--       in the identification all along;
--   §�  the poles.  Zero loss: every unit IS the identity (�����-�������
...
--
-- ��������������������������������������������������������������������
-- TERMS.  ��������� and ��� as in `SamraksakaGana` (��� attested as the
-- gaapha's device, Pini, Adhyy ~500 BCE; the application to
-- flows built in this corpus).  ���� for "group" follows this corpus's
...
-- (a,−b).  The compound ������������� is built here.
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- §� � THE UNITS, over set carriers.  Inverse data is a proposition:
...
module समूहे {A B : Type ℓ} (setA : isSet A) (setB : isSet B) (f : A → B) where
  ...
  private
    ...
    -- the gaa's laws, each refl on the map component
  ...
  ------------------------------------------------------------------
  -- §� � THE GROUP.  Both inverse laws are the stored evidence.
  ------------------------------------------------------------------
  ...
  ------------------------------------------------------------------
  -- §� � THE GROUP RIDES ����.  A unit's image among the sections is
  -- �-invertible, each direction one `cong` � ���-������ was refl, so
  ...
  ------------------------------------------------------------------
  -- §�a � NEAR POLE.  Zero loss: the symmetry group is trivial.
  ------------------------------------------------------------------
...
------------------------------------------------------------------------
-- §�b � FAR POLE.  Total loss: the symmetry group is ALL of Aut(A).
------------------------------------------------------------------------
...
module अन्ध-समूहे {A B : Type ℓ} (setA : isSet A) (setB : isSet B)
  ...
  -- (e µ f' applies f' first), so that the comparison below is refl.
  आत्म-समूहः : Group ℓ
...
------------------------------------------------------------------------
-- §� � ���� � unchanged from SamraksakaGana §� except (b), which this
...
-- here: the section-side units as a packaged `Group` with ���� a
-- `GroupEquiv` onto it � §� gives the pointwise evidence and stops.
------------------------------------------------------------------------
