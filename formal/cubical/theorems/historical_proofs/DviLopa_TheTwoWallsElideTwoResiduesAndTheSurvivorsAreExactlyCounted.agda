...
------------------------------------------------------------------------
-- ����-����� � the two walls elide two residues, and the survivors are
...
-- field (����������: ���������� p a y = y � �a) removes, at each prime chart,
-- either two residues (walls distinct) or one (walls merged, exactly
-- when p � 2a � ����������'s ������-�������/��� iff).  ����������-�����'s Lemma 2�3
...
--
--   ���-�����  :  (�[ j ∈ Fin (suc m) ] � i ≡ j)              � Fin m
--   ����-����� :  (�[ y ∈ Fin (2+m) ] (� a ≡ y) � (� b ≡ y))  � Fin m
--                                                  (given � a ≡ b)
--
...
-- what this corpus means by a receipt (����� �: ���������� �����������, �
-- ��������).  The numerators of the singular series' local factors
-- (1 − ω_p/p) are the sizes of these types; here they are the types.
--
-- ON THE NAME.  ����� � Pini, Adhyy 1.1.60, adarana lopa
...
-- README's movement 9 already reads it.  ����-���� built here, 2026-08-23.
--
-- THE INSTRUMENT is an involution, not an induction: ������� exchanges
-- the wall with fzero (its own inverse, three decidable cases), the
-- zero-wall case is a pattern match on the underlying �, and the
...
--
-- ���������, scope.  The walls here are abstract distinct points of
-- Fin p � that the arithmetic walls �a (mod p) are distinct exactly
-- when p � 2a is ����������'s theorem on the � side; the mod-p bridge
-- (� � Fin p respecting the divisibility relations) is named, not
-- built.  The product over charts (the period count �(p − ω_p), the
-- CRT tensor of ����������-����� Lemma 3) is likewise named, not built: this
...
------------------------------------------------------------------------
-- � � ������� � the exchange of a chosen point with fzero.  Its own
...
------------------------------------------------------------------------
-- � � �����-����� � eliding fzero: the base case, by pattern on the
-- underlying number.
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- � � ���-����� � eliding ANY point: the base case conjugated by the
...
एक-लोपः {m = m} i =
  ...
  where
    -- (i ≡ j) � (fzero ≡ ������� i j): exchange both sides, then slide
    -- the left endpoint along ��-�����.
...
------------------------------------------------------------------------
-- � � ����-����� � two distinct walls leave p − 2 residues, identified.
...
द्वि-लोपः {m = m} a b a≢b =
  ...
  where
    ...
    -- (b ≡ y) � (���� ≡ image of s): lift to the survivor subtype (the
...
-- Fin (p−1); of two distinct walls, Fin (p−2) � ����������-�����'s ω_p
-- dichotomy (= ����������'s p � 2a iff) carried as identifications.  The
