...
--
-- A statement with a real-valued logarithm — there is no `log` in
-- cubical v0.9 and the corpus does not define one. The Virahka-inverse
-- form and the 2^k form above are what "O(log b)" means over ℕ, and
-- both are checked; nothing sharper is claimed. Nothing here touches
-- the इष्ट section or the cakravāla, which stay as the earlier module
-- leaves them.
------------------------------------------------------------------------
...
-- in particular below the least such n, the Virahka inverse of b.
