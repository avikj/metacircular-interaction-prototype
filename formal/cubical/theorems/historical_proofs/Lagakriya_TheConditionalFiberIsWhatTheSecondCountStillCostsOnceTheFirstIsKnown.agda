...
-- states procedures and counts; he states no type, no fiber, no
-- equivalence.  The claim is that the objects his pratyayas enumerate
-- ARE the fibers of the counting maps — a fact about the definitions in
...
--       RESTRICTED TO THE FIBER OF f — the conditional receipt: once f
...
--
--   §३  मुक्त-लगक्रिया  if f's fiber at b is contractible, the joint
--       fiber collapses to a single path `g a₀ ≡ c`.  When f is already
...
--       `Chandomudra_….युग्म-तन्तुः` which prices the joint fiber.
--
...
module _ {A : Type ℓ} {B : Type ℓ'} {C : Type ℓ''} (f : A → B) (g : A → C) where
  ...
  -- §१  the joint fiber IS the conditional fiber.
  --
  ...
  -- §२  the sum rule.  The fiber of f is the sum, over the values of g,
...
-- `Chandomudra_….युग्म-तन्तुः` says each joint fiber is `Chosen n k`,
...
-- sum of the conditional fibers IS the वर्णवृत्त, and Piṅgala's own
