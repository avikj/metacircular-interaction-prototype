...
-- fibre. ¬ß‡ß is that phenomenon with nothing else attached.
--
-- ¬ß‡® is why Pigala's numbers ADD across such a split. If the two
-- fibres are finite ‚î `fiber f c ‚â Fin m` and `fiber g c ‚â Fin n` ‚î then
--
--     ¬ß‡®   fiber ‡µ‡ø‡ï‡≤‡‡‡ c  ‚â  Fin (m + n).
--
-- This is ¬ß‡ß composed with the library's `‚ä-equiv` and
-- `Fin+‚âFin‚äFin`: the two receipts, chained, turn the twofold of fibres
...
------------------------------------------------------------------------
-- ‡ß ¬ ‡µ‡ø‡ï‡≤‡‡-‡‡®‡‡‡‡ ‚î the fibre of the case-map is the case of the fibres.
...
------------------------------------------------------------------------
-- ‡® ¬ ‡µ‡ø‡ï‡≤‡‡-‡ó‡‡®‡æ ‚î two finite receipts on the two fibres flatten to one
