...
------------------------------------------------------------------------
-- ������������ � the p-adic splitting, as a composition of what is there.
--
...
--
--   `Drdha_�`  : ����� (= prime), ��� (list product), �����, _������_,
--                �������� (every n � 1 is the product of a list of firm
--                numbers), ��������-�������� (Euclid VII.30), ���������� (a
--                firm p dividing the product of a firm list occurs in it).
--   `Uniqueness_�` : �������� (two firm lists with one product are a Perm),
--                ������-����� (hence same count of every p), and the
--                valuation ����� p n pos := count of p in Drdha's list,
--                well defined by ���-�������.
--
...
-- WHAT IS PROVED.
--
--   §�  ���� p L � L with every occurrence of p struck out (a filter by
--       `discrete�`), and the product identity, by induction on L:
--           ��-���� : ��� L ≡ p ^ ����� p L � ��� (���� p L)
--       together with: ���� preserves �����, p is not a member of
--       ���� p L, and hence (by ����������) p � ��� (���� p L) when L
--       is firm.
--
--   §�  ������������ � EXISTENCE of the p-adic splitting.  For firm p and
--       n � 1:
--           �[ m ∈ � ] ((n ≡ p ^ ����� p n pos � m) � (� (p � m)))
--       with m := ��� (���� p L) for L Drdha's list of n.  The exponent
--       is LITERALLY Uniqueness's valuation, not a new counter.
--
--   §�  ���-�������� � UNIQUENESS of the exponent.  If n ≡ p ^ e � m with
--       � (p � m) then e ≡ ����� p n pos.  Proof: expand m into its own
--       firm list M (Drdha §�); ���� e p ++ M is a firm list with product
--       n, so ���-������� gives its p-count, e + ����� p M, equals the
--       valuation; and ����� p M ≡ 0 because p ∈ M would give p � m
--       (�����-������).  No cancellation of p's is needed for this half.
--       ����������-�������� adds that m is unique too, by one cancellation
--       of p ^ e (which is positive since p > 1).
--       Corollaries: ���-������� (p � n � ����� p n ≡ 0) and ���-�����
--       (����� p (p ^ e) ≡ e).
--
--   §�  ���-������� � the valuation is MULTIPLICATIVE-TO-ADDITIVE:
--           ����� p (a � b) posab ≡ ����� p a posa + ����� p b posb
--       by ������-����� (through ���-�������) on the concatenation of the
--       two Drdha lists, using ��-++ and �����-++, and �����-++.
--
--   §�  ���? � a bonus that costs four lines given Drdha's ���������:
--       firmness of n > 1 is DECIDABLE.  Used only to make the kernel
--       tests below honest (the prime hypotheses are computed, not
--       hand-built).
--
--   §�  �������� � the kernel runs the splitting: for n = 12 the p-free
--       rests at p = 2, 3, 5 are 3, 4, 12 by refl, and the valuation
--       identity 2 = ����� 2 12 = ����� 2 4 + ����� 2 3 is refl on both
--       sides.
--
-- ��������������������������������������������������������������������
-- WHAT IS **NOT** CLAIMED.
--
--   � ����� is Drdha's predicate ("1 < p and every divisor is 1 or p"),
...
--     and the splitting THEY say are missing, phrased over �����.  The
--     bridge ����� p � IsPrime p is a separate (easy) module and is not
--     written here.
--   � The SIMULTANEOUS splitting n ≡ smooth n � rough n over several
--     primes at once (the (v� , v� , v�) of SieveRoughBridge) is not
--     stated; it is three applications of §� and §�, and the extra
...
--   � `����� p n pos` for p NOT firm is a count in a list of firm numbers
...
------------------------------------------------------------------------
-- � � ���� � the list with every p struck out, and the product identity
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- � � ���-�������� � UNIQUENESS of the exponent (and of the rest)
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- � � ���-������� � the valuation adds over products
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- � � ���? � firmness of n > 1 is decidable (Drdha's search, both arms)
------------------------------------------------------------------------
