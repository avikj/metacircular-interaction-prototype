...
-- flows ≅ sections of one's own fibers ≅ the product of the fibers'
...
-- subst p (x , r) ≡ (x , r ∙ p) and its associativity) is not needed at set level, where nothing
-- consumes it.
--
-- ────────────────────────────────────────────────────────────────────
-- TERMS.  तन्तु for the fiber follows this corpus's own use
...
module TheGanaOfAnObservableIsTheProductOfItsFibersOwnEndomorphismMonoids where
...
open import TheConservingFlowsOfAnyObservableAreTheSectionsOfItsOwnFibers
...
module तन्तौ {A B : Type ℓ} (setA : isSet A) (setB : isSet B) (f : A → B) where
  ...
  ------------------------------------------------------------------
  -- §४′ · THE COMPOSITE: flows ≅ the product of the fibers' monoids.
