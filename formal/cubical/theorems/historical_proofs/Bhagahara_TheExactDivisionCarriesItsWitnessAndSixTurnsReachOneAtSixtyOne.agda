...
-- anything "the ��������", and both name the SAME missing ingredient:
--
--   `loss/�/Composition_TheKsepaIsDeterminedByTheRootsAnd�`:
--     "It requires choosing m with k � (a + b�m), and then dividing the
--      composed row through by k � exact division in �, which needs a
--      divisibility witness carried alongside and is not done here."
--   `Vargaprakrtifiber_ThePellFibreIsInfinite�`:
...
--
-- That refusal was correct and the gap it names is one object.  §� builds
-- it: `�������� j n` is the exact division of n by (suc j), presented as a
-- CARRIER � base = the pair (dividend, divisor), carried = the �����
-- (quotient), witness = n ≡ suc j � �����.  §� proves the carried datum is
-- DETERMINED (`�������-����` : the type is a proposition), which is what
-- makes carrying it free.
--
-- AND THE DISTINCTION THAT MATTERS, which the ������ case hides.  For the
-- ������ the fibre is `singl` � contractible � because the roots determine
-- it TOTALLY: every pair has a ������.  For the ������� the fibre is a
...
-- vs. merely propositional is the whole difference between the �����
-- (free) and the �������� (not free).
--
-- ��������������������������������������������������������������������
-- WHAT IS PROVED HERE.
--
--   §� `��-��������` � THE TURN.  Given a row a² − D b² = �K with K = suc j,
...
--     to claim.
--   §� `����` � the same as a total function on rows.  Total: once the
--     three ������� are in hand there is no further obligation.
--   §� THE RUN AT D = 61, Bhskara's own example.  Six turns from (8,1,+3)
--     to (29718, 3805, −1), every divisibility witness discharged by
--     `refl` in the kernel � the divisions are PERFORMED, not assumed.
--   §� `�����-��������-���` � Brahmagupta's composition of a k = −1 row with
--     itself, giving k = +1, and hence
--
--        1766319049² − 61 � 226153980² = 1.
--
--     Which is Bhskara's answer, and the case the naive ����� orbit
...
--   * BHSKARA'S CHOICE RULE.  The six m's of §� are supplied as data.
--     That each is the m his rule selects (minimise |m² − D| subject to
--     the congruence) is NOT checked here � and `CakravalaBound.agda` §�
...
-- complete �������� RUN is executed in the kernel.  The �������� as an
...
--
--   BRAHMAGUPTA, �����������������������, ������ �� (�����������������), 628 CE �
--   the ����� (§� here), with �������� for the multiplier D, ������� for the
--   greater root, ������� for the lesser, ������ for the interpolator, and
--   �� / ��� (asset / debt) for the two signs, which is why §� names the
--   sign type with his two words rather than with a Bool.
--
--   JAYADEVA, c. 950 CE, surviving only inside UDAYADIVKARA's �������
--   (1073) � the ��������.
--
--   BHSKARA II, ����������, 1150 CE � the �������� in full, and D = 61 as
--   his worked example, with the answer §� reaches.
--
--   ������� (divisor / the act of dividing) and ����� (the quotient, "what
--   is obtained") are the ���������� vocabulary of the same chapter.
--
...
-- postulates, no holes.  � throughout, never �: cubical's � product does
-- not reduce on numerals of this size, and §� needs numbers up to
-- 3 � 10�� to actually compute in the kernel.  Signs are therefore
-- carried as �� / ��� beside a magnitude, and every equation below is
-- subtraction-free.
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- � � �� and ��� � Brahmagupta's two signs, and the equation split by them.
--
-- �������������� D a b �� K   is   a² = D b² + K      (a² − D b² = +K)
-- �������������� D a b ��� K   is   a² + K = D b²      (a² − D b² = −K)
--
-- Both are subtraction-free, which is what lets � state them and what
-- lets the kernel compute them at §�'s magnitudes.
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- � � �������� � THE MISSING INGREDIENT, as a carrier.
--
-- Base: the dividend n and the divisor suc j.  Carried: the �����.
-- Witness: n ≡ suc j � �����.  The divisor is written `suc j` and not
-- `k` with a side condition, because a zero divisor is not an omission
-- to be excluded later � it is a divisor that was never one.
--
-- `�������-����`: the type is a PROPOSITION.  The quotient is determined
...
--
-- And it is exactly one h-level weaker, which is the content: the ������'s
-- fibre is contractible because every pair HAS a ������, and this one is
...
-- the two shapes the conclusion can take.  `���-�` produces a �� row,
-- `���-�` an ��� row; the last hypothesis of each is the sign-specific
...
------------------------------------------------------------------------
-- ��-�������� � THE TURN.  All four sign combinations.
--
...
-- Bhskara's rule, and the reader should not read more into it.
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- � � ���� � the turn as a TOTAL function on rows.
--
-- Total, and that is the point.  The three ������� are the whole of the
...
------------------------------------------------------------------------
-- � � ���������� at �������� �� � BHSKARA'S OWN EXAMPLE.
--
-- Six turns.  Every `refl` below is a division actually carried out by
-- the kernel: `�������� 2 (8 � 7 + 61 � 1)` is inhabited by `39 , refl`
-- only because 8�7 + 61�1 and 3�39 are the same numeral.
--
-- D = 61 is the case the naive ����� orbit does not reach: there is no
-- small row with ������ 1 to seed it, and squaring a row multiplies its
-- ������ rather than reducing it.  The wheel gets there by going THROUGH
-- ������ 3, 4, 5, 5, 4, 3 and out at 1 � which is what a cycle is for.
------------------------------------------------------------------------
...
-- the wheel's own reading, turn by turn: (������� , ������� , ������).
...
-- and the sign at the exit is ���: 29718² + 1 = 61 � 3805².
चिह्न-६ : चिह्नम् वलय-६ ≡ ऋण
चिह्न-६ = refl

-- the exit row's equation, restated in its own numerals.  This is the
-- theorem of §� applied six times, not a computation: it is `��������`.
वलय-षष्ठ-समीकरणम् : 29718 · 29718 + 1 ≡ 61 · (3805 · 3805)
वलय-षष्ठ-समीकरणम् = प्रमाणम् वलय-६

------------------------------------------------------------------------
-- � � ����� at the exit � Brahmagupta closes what Jayadeva opened.
--
-- The wheel delivers ������ −1, not +1.  Composing that row with ITSELF is
-- Brahmagupta's �����, and the ������ multiplies: (−1)�(−1) = +1.  So the
-- last move of the �������� is 522 years older than the ��������.
--
--   (a² + D b²)² − D�(2ab)² = (a² − D b²)² = K²
--
-- proved here subtraction-free in the ��� case, for any K.
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- �������� � THE ANSWER.  1766319049² − 61 � 226153980² = 1.
--
-- Bhskara II's own stated answer to his own stated example.  Here it is
-- the exit row of §� composed with itself by §�: the numerals
...
-- stated in its own numerals, and it is `�������� ��������` � derived,
