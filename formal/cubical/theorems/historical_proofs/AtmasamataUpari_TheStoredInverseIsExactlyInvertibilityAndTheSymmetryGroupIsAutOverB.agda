...
--
--   � `SamraksakaSamuha_�.agda` §��§� builds the symmetry group over the
--     carrier `�[ � ∈ ������� f ] �����������-��� �` � a flow bundled with
--     STORED two-sided conserving-inverse evidence � and proves that
--     evidence is a proposition (�����������-��������), so the carrier is a
--     subtype.  That is all correct.
--   � `SamanaKaksya_�.agda` §� proves, separately, that when the map is
--     an equivalence its inverse conserves automatically
--     (�����������-���������� � one `sym`, one `cong`).
--
-- The two were never joined.  Joining them (§�) collapses the stored
-- datum: �����������-��� � � isEquiv (� .fst).  Left to right is the
-- iso�equiv the group's own `�����` already performs; right to left is
...
-- biimplication.
--
-- Consequently the group's carrier is `�[ � ∈ ������� f ] isEquiv (� .fst)`
-- � the conserving flows that HAPPEN to be invertible, with no attached
-- data � and, reassociated (§�), it is
--
--     ��������-�����  =  �[ ε ∈ (A � A) ] ���������� f (equivFun ε),
--
-- Aut_B(A): the automorphisms of A lying OVER B, i.e. commuting with f.
-- §� makes that a `Group` in its own right and exhibits a `GroupEquiv`
-- from `���������-�����` onto it whose hom law is `�����-���� refl`.  So the
-- Dhruva/Khahara scale reads, intrinsically:
--
--     near pole (isEquiv f)   Aut_B(A) is trivial                (§�a)
--     far pole  (����-���� f)  Aut_B(A) � Aut(A), the witness free (§�c)
--     in between              Aut_B(A) exactly, nothing else.
--
-- §�b prices `Apratiloma`'s witness intrinsically: ����� is outside the
-- unit subgroup for exactly one reason � it is not an equivalence
-- (����� 0 ≡ ����� 1 while 0 � 1) � and by §� that reason is the ONLY one
-- available.  `Apratiloma`'s fence does not move: `������� f` is still a
-- monoid and not a group, `crush` is still in it and still has no
-- inverse.  What §� adds is that non-invertibility of the underlying map
...
--
-- ��������������������������������������������������������������������
-- TERMS.  ����-���� � "self-sameness"; the compound ����-����-����� is
-- already this corpus's own name for Aut(A) (`SamraksakaSamuha` §�b),
-- and it is MODERN mathematical : no classical text is claimed
-- for it, there and not here either.  ����� � "above, over", ordinary
-- , used here for the slice: "over B".  ���������, ������, ���, ����,
-- ����������� as in `SamraksakaGana`/`SamraksakaSamuha`, with their limits
-- unchanged (��� attested as the gaapha's device, Pini,
-- ������������, ~500 BCE; the application to flows is this corpus's).
-- The compound ��������-����� is BUILT HERE; no source states
-- anything below.
------------------------------------------------------------------------
...
module उपरि {A B : Type ℓ} (setA : isSet A) (setB : isSet B) (f : A → B) where
  ...
  ------------------------------------------------------------------
  -- §� � THE STORED INVERSE IS EXACTLY INVERTIBILITY.
  --
  ...
  -- §�), and the two laws are then secEq / retEq under ������-����.
  ------------------------------------------------------------------
  ...
  ------------------------------------------------------------------
  -- §� � THE CARRIER, WITH THE STORED DATUM REMOVED, AND THEN
  ...
  -- one �≡Prop for the conservation witness (setB), one equivEq for the
  -- isEquiv field.
  ...
  ------------------------------------------------------------------
  -- §� � Aut_B(A) IS THE SYMMETRY GROUP, BY A GroupEquiv WHOSE HOM LAW
  -- IS `�����-���� refl`.
  --
  -- Composition is in application order (u ��� v applies v first), to
  -- match `_��_`'s order, so that the comparison never has to commute
  -- anything.
  ------------------------------------------------------------------
  ...
  ------------------------------------------------------------------
  -- §�a � NEAR POLE, intrinsically.  Zero loss � Aut_B(A) is trivial.
  -- The flow space is already contractible (�����-�������), so no
...
------------------------------------------------------------------------
-- §�b � THE UNIT SUBGROUP, PRICED AT `Apratiloma`'s OWN WITNESS.
--
-- ����� conserves ���� and is not a unit.  §� says the reason can only be
-- one thing, and here it is that thing: ����� is not an equivalence,
...
-- and therefore no stored inverse exists � by §�, not by inspection of
...
------------------------------------------------------------------------
-- §�c � FAR POLE, intrinsically.  At total loss the conservation
-- witness is free, so Aut_B(A) � Aut(A) by the bare projection �
-- which is `SamraksakaSamuha` §�b's GroupEquiv seen without the stored
-- inverse in the way.
------------------------------------------------------------------------
