...
-- ‡‡µ‡‡‡ ‚î ‡‡‡‡‡‡Ø ‡‡®‡‡‡‡ ‡‡‡∞‡‡‡ ‡‡µ ‡
--
-- (the remainder's fiber is nothing but the progression.)
--
...
--
-- And a residue map has one, exactly: **the fiber of `_mod k` over `r`
...
------------------------------------------------------------------------
-- ‡ß ¬ ‡‡‡∞‡‡‡ ‚î the progression, and that it lands in the fiber.
------------------------------------------------------------------------
...
-- cancellation of `K ¬_`, and the fiber's proof component is carried for
...
------------------------------------------------------------------------
  ...
  ‡§Ö‡§µ‡§∂‡•á‡§∑-‡§§‡§®‡•ç‡§§‡•Å‡§É = isoToEquiv (iso ‡§≠‡§æ‡§ó‡§É ‡§∂‡•ç‡§∞‡•á‡§¢‡•Ä-‡§§‡§®‡•ç‡§§‡•å ‡§™‡•Å‡§®‡§∞‡§æ‡§ó‡§Æ‡§®‡§Æ‡•ç ‡§™‡•ç‡§∞‡§§‡•ç‡§Ø‡§æ‡§ó‡§Æ‡§®‡§Æ‡•ç)
    ...
    -- m ‚â° r + K¬(m/K), because the remainder IS r by the fiber's own witness
...
--
-- Above the modulus the fiber is EMPTY: `mod<` bounds every value by K,
-- so nothing maps to an r ‚â K.  Below it the fiber is a full copy of ‚ï.
