...
--
--   Jyehadeva, ����������� / *Gaita-yukti-bh* (Malayalam, c. 1530),
--   mathematics part, the ����������� chapter � the **������-���������**,
--   the summation of like powers, argued for a general power by
--   obtaining each power-sum by means of the one below it.
--
--   Nlakaha Somayji, ���������������� (1501), which the �����������
--   derives; Mdhava of Sagamagrma (c. 1340�1425), to whom the
--   material is attributed there.
--
--   Earlier, and the two results §� and §� recover: ryabhaa,
--   �����������, ��������� ����� (499) � the ������� (the sakalita of
--   sakalitas) and the doubling rule 2�� = n(n+1).  Those verse numbers
--   are carried from `Citighana.agda` and `Sankalita.agda` in this lane.
--
-- Nothing here is claimed about �, and there is no series in this file.
--
-- ��������������������������������������������������������������������
-- WHAT IS EXACT, AND WHAT IS NOT (§�, and it is declared, not filled).
--
-- The �����������'s use of the ������-��������� is asymptotic: for a fine
...
-- this lane and is not approximated here.**  It needs � or �, a notion
-- of "negligible", and a limit; � has none of the three.  This is the
-- same declaration `Madhava.agda` makes about the geometric remainder �
-- ���������, � ������-�������, un-said, not falsely proved � and the same
...
--
--     (n+1) � �_{k�n} f(k)  =  �_{k�n} k�f(k)  +  �_{j�n} �_{k�j} f(k)
--
-- � §�, `���������-�������`.  Read right to left it is the �����������'s own
-- move: the (p+1)-th power-sum is what is left of (n+1) times the p-th
-- once the repeated summation (���������-�����������) has been taken out.
-- The deficit of �k^(p+1) from n��k^p is EXACTLY a sakalita one order
-- down, at every finite n, with nothing thrown away.  §� is that form
...
--
-- The descent has nothing to do with powers.  §� is proved for an
-- ARBITRARY sequence f : � � �; the powers enter only in §�, and only to
...
-- rules falling out of the �����������'s law as its first two instances:
--
--   §�  at the CONSTANT sequence f ≡ 1, the descent reads
--         (n+1)�n = �n + �n,
--       which is ���������' doubling rule.  `��������-�����������-����` is
--       `Sankalita.��������-�����������` re-derived from the general law
--       rather than by its own induction.
--
--   §�  at f = k, the descent reads (n+1)��n = �²n + ���� n, and
--       together with `Vargacitighana.�����������` it gives
--       3����� n = (n+2)��n and hence 6����� n = n(n+1)(n+2) �
--       `��������-����`, which is `Citighana.��������` obtained from the
...
-- establish is that the two ryabhaa results and the �����������'s
...
--
-- §� is the p = 2 instance, which does not close on a corpus theorem:
-- (n+1)��²n = �³n + �_{j�n} �²j, linking `Sankalita.�³` to
...
------------------------------------------------------------------------
-- �.  ����������� � the summation of a sequence from 1 to n.
--
-- The shape is `Sankalita.�`'s, so that �, �², �³ and ���� are all
...
------------------------------------------------------------------------
-- �.  THE DESCENT, for an arbitrary sequence.
--
--     (n+1) � �_{k�n} f(k)  ≡  �_{k�n} k�f(k)  +  �_{j�n} �_{k�j} f(k)
--
-- Finite, exact, in �, for every f.  No division, no limit, no order.
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- �.  THE SAME, IN THE FORM THE TEXT STATES.
--
-- n times the sum, not (n+1) times, with the repeated summation running
-- one short � "the excess of n��f over �k�f is the sakalita of the
-- sakalitas up to n−1".  Obtained from §� by cancelling one copy of the
-- sum on both sides, which is `inj-m+`, addition in � being injective.
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- �.  ������-����������� � the like-power instance.
--
-- The only thing the powers contribute is that k�k^p is k^(p+1), which
-- is how `�����` is defined; so §� is §� with nothing added.
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- �a.  The low powers, identified with the lane's existing summations.
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- �.  FIRST INSTANCE � p = 0, the constant sequence.
--
-- �_{k�n} 1 = n, �_{k�n} k�1 = �n, �_{j�n} j = �n, so the descent is
--
--     (n+1)�n = �n + �n,
--
-- which is ����������� ���������' doubling rule.  `Sankalita` proves it by
...
------------------------------------------------------------------------
-- �.  SECOND INSTANCE � p = 1, and it recovers the �������.
--
-- The descent at f = k reads (n+1)��n = �²n + ���� n.  With
-- `Vargacitighana.�����������` (�²n + �n = 2����� n) that gives
-- (n+2)��n = 3����� n, and doubling gives 6����� n = n(n+1)(n+2).
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- �.  THIRD INSTANCE � p = 2, which closes on no corpus theorem.
--
-- (n+1)��²n = �³n + �_{j�n} �²j.  `Sankalita.�³` and
