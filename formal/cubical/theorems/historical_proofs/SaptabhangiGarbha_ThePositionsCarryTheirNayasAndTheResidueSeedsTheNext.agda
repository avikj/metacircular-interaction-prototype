...
--
-- ����������� � the sevenfold predication, as the machine's return type, with
...
-- types � seven nullary constructors and a presence-profile in {���, �}³.
--
--   1.  Here no position is a label.  ������-����� carries the standpoint that
--       affirmed and the term by which it affirmed; ������-������ carries the
...
--       second half is withdrawn here: ����������� is the failure of ONE
...
--       `���������-������` (§5) recovers the third position from the fourth by
--       `refl` � nothing was destroyed.
--
--   3.  And the price, stated because it is a real loss and it is the
--       honest one: �����-������� � commutativity of succession � is FALSE
--       once the positions carry evidence (`�����-�-�������`, §7, a checked
...
--       its witness (����������), which is what a succession IS.
--
-- THE THIRD AND THE FOURTH.  Not distinguished by a comment and not by a
-- tag passed in: ������������ and ��������� are two different functions.
-- ������������ accumulates and, off its inputs, never manufactures a ���
-- (`�����-�-�����-�����`).  ��������� demands one utterance and, wherever an
-- affirmation and a denial are both in hand, returns the ��� instead
-- (`��-�����-�����`).  `�����-��-����` is then the instance, not the claim.
--
-- THE FOURTH POSITION DOES NOT END THE COMPUTATION.  §6: a ��� over P is a
-- ��� over the born family �������� P, whose standpoints are the old ones
-- under ������ / ������� (TS 5.31).  The born third position needs only ONE
-- old standpoint (`�����-�������������`), while at the root every third
-- position needs two distinct ones (`����-����-���`).  So what no single
...
-- permission absent.  §5 below carries the ��� into that theorem, so the
...
------------------------------------------------------------------------
-- §1  ��� � the residue an ����������� retains.
--
-- Not "the two seeds were consumed".  The two nayas are still here, each
-- with the term by which it spoke: ������ is a sydasti (a standpoint and
-- a proof), ������ a sydnsti (a standpoint and a refutation).  Nothing
...
-- ������� � the two standpoints are DERIVED to be distinct.  Not asserted:
...
------------------------------------------------------------------------
-- §2  ����������� � the seven positions, each carrying its nayas.
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- §3  ������������ � assertion IN SUCCESSION.
--
-- One after the other.  What is already in hand stands (����������: the
-- first speaker keeps its witness); what is new is added beside it.  The
-- three ���- operations are the whole of it, and they are the reason
...
------------------------------------------------------------------------
-- §4  ��������� � assertion AT ONCE, and it is a different function.
--
-- ������� is where the tongue breaks: given an affirmation and a denial
...
-- The structural difference, so that �����-��-���� is an instance of
...
-- by refl.  And the ��� is a certificate against collapse � via
...
------------------------------------------------------------------------
-- §6  ������ � the residue is the seed of the next derivation.
--
-- Tattvrthastra 5.31, arpitnarpita-siddhe, as an operation.  The born
-- standpoint space is the old one under two aspects: ������, the aspect
-- asserted, and �������, the aspect held back.  The born family reads a
-- standpoint's affirmation under the first and its denial under the second.
--
-- ������'s two constructors are not labels: �������� is their definition, and
-- the two aspects of one naya carry evidence that refutes each other
-- (`������`), which is what the aspect distinction IS.
------------------------------------------------------------------------
...
-- ������ � the birth.  A residue over P is a residue over the born family,
...
-- distinct standpoints (this is ������� again, and it is the measure of
...
--
-- Is the forgetful map records � labels a
...
--   as follows:
--
--   * BOTH.  �������� (this file's positions, read with the naya UNASSERTED
--     � Tattvrthastra 5.31's �������) commutes with ������������ and with
--     ��������� alike, exhaustively, for every S and every P.
--   * It has a section ������� which is also a homomorphism for both, with
--     �������� ∘ ������� ≡ id.  The label lane is a RETRACT of this one: a
--     subalgebra and a quotient at once.  It is not a rival account.
--   * It has no inverse (`�-������������`): two standpoints that both affirm
--     give two positions with one label.  So there is no equivalence, §�
--     path one is unavailable AS A THEOREM, and §� of
--     AHIMSA_SUTRA_VISTARA applies literally � the collapse does not
--     exist.
--
-- AND ONE LAW OF THIS FILE IS WEAKER THAN IT LOOKED.  §7's �����-�-�������
-- is confirmed and sharpened (`�����-�������-�-���������`: identities descend
-- along �������� and do not lift).  But distinctness LIFTS, and the label
-- lane's ��-���������� lifts with it: `��-����������-���������` proves ��������� is
-- NOT associative HERE, on the three positions ������-�����-������,
-- ������-�����, ������-������, with every naya and every witness retained and
-- `�����������-�-�������` in force throughout.  Retaining the ��� does not buy
-- associativity back, and the label lane's explanation of its own failure
-- (that ���������� destroys the seeds) is refuted by that � the failure
-- survives the retention.  What breaks the law in both lanes is that ��
-- tests the JOINED position for an asti�nsti pair, and whether that pair
