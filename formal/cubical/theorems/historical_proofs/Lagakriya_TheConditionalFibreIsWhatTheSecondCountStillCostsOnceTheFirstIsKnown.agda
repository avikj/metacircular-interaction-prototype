...
--       RESTRICTED TO THE FIBRE OF f ‚î the conditional receipt: once f
...
--
--   ¬ß‡©  ‡Æ‡‡ï‡‡-‡≤‡ó‡ï‡‡∞‡ø‡Ø‡æ  if f's fibre at b is contractible, the joint
--       fibre collapses to a single path `g a‚ ‚â° c`.  When f is already
...
--       `Chandomudra_‚¶.‡Ø‡‡ó‡‡Æ-‡‡®‡‡‡‡` which prices the joint fibre.
--
...
module _ {A : Type ‚Ñì} {B : Type ‚Ñì'} {C : Type ‚Ñì''} (f : A ‚Üí B) (g : A ‚Üí C) where
  ...
  -- ¬ß‡ß  the joint fibre IS the conditional fibre.
  --
  ...
  -- ¬ß‡®  the sum rule.  The fibre of f is the sum, over the values of g,
...
-- `Chandomudra_‚¶.‡Ø‡‡ó‡‡Æ-‡‡®‡‡‡‡` says each joint fibre is `Chosen n k`,
...
-- sum of the conditional fibres IS the ‡µ‡∞‡‡‡µ‡‡‡‡, and Pigala's own
