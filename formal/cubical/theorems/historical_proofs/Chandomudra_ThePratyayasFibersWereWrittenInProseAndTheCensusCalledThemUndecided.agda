...
--
-- and their fibers are DEFINED FIFTEEN LINES BELOW THEM, in the same
-- file, by name.  `PingalaPrastara.agda:55` says so in prose: *"`Vak n`,
-- `Metre n` and `Chosen n k` are its fibers over …"*.  No term said it,
...
-- `Chosen n k` is the JOINT fiber of two observables at once, and it
...
-- So the fiber of the mātrā-count satisfies VIRAHĀṄKA'S RECURRENCE — the
...
-- fiber, a type, or an equivalence.  What is claimed is that the objects
-- their pratyayas enumerate ARE the fibers of the counting maps, which is
...
module Chandomudra_ThePratyayasFibersWereWrittenInProseAndTheCensusCalledThemUndecided where
...
------------------------------------------------------------------------
-- १ · मात्रावृत्तम् — a metre IS the fiber of the mātrā-count.
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- २ · वर्णवृत्तम् — and a syllable-metre is the fiber of the syllable count.
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- ३ · The joint fiber, which is NOT definitional.
--
-- `Chosen n k = Σ[ p ] ((varna p ≡ n) × (guruOf p ≡ k))` — a PAIR OF
-- EQUATIONS.  The fiber of the paired map is `Σ[ p ] ((varna p , guruOf p)
...
-- there are 46 such written-out fibers in this corpus, and 11 of the
