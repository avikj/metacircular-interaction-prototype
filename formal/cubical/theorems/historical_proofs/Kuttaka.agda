...
-- the vall.  It is the first solution of ax − by = c on record, six
...
--
-- (It is self-contained over ℤ — no matrix/continuant dependency.  PIN-GREEN
...
-- — and `·Rid` → `·IdR`.  No statement was changed, weakened or added.
...
--
-- The iṣṭa section — the reduction of the solution family to the LEAST
-- non-negative representative, which needs a mod/section convention — is
-- `KuttakaIsta_TheSolutionFamilyHasALeastNonNegativeRepresentativeAndItIsUnique`.
...
------------------------------------------------------------------------
-- The pulverizer's descent, as inductive evidence (the vall).
------------------------------------------------------------------------
