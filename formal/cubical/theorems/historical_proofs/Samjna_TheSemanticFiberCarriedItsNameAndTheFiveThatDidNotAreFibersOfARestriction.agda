...
--
-- SCOPE, and what is NOT claimed.  Pāṇini stated no fiber, no type and no
...
-- assigns names to fibers and then, in `machine/Lopa_…hs`, goes back to
-- re-describing the class — which is why one-way edges sit UNDECIDED
-- while their fibers are written out in the very files the census read.
...
--
-- the fiber of `q` RESTRICTED to a domain predicate.  The census has no
-- vocabulary for that at all, so it prices those edges at nothing.  §२
-- gives it one: such a type is the honest fiber of the restricted map,
...
--   fiber rows usable after detection             244   (of Upalabdhi's 475)
...
--
--   (i)   fiber of a RESTRICTED map — five of the nine, and §२ below;
...
--         fiber of `sgn ∘ Ω` and the join saw only `Ω` (Tantusandhi §३
--         names the composite);
...
-- `TypedUnfold.Lang` and `Gurutama.म` are JOINT fibers of
...
module Samjna_TheSemanticFiberCarriedItsNameAndTheFiveThatDidNotAreFibersOfARestriction where
...
open import SieveFiber
  using (Vis ; q ; domain ; _∈_ ; Fiber)

------------------------------------------------------------------------
-- १ · अर्थतन्तुः — the semantic fiber IS the fiber of the semantics map.
--
...
-- named points of `SemanticFiber false` ARE two points of the fiber of
...
------------------------------------------------------------------------
-- २ · सावच्छेदतन्तुः — the fiber of a map cut down to a domain.
--
-- `SieveFiber` writes, at line 465,
--
--     Fiber v = Σ[ n ∈ ℕ ] ((n ∈ domain) × (q n ≡ v))
--
-- and the probe `fiber q v ≡ Fiber v` DIES: the membership conjunct is
...
-- because the written type is a fiber — of a different map.  Cut `q`
-- down to the subtype the domain predicate carves out, and `Fiber` is
-- that map's fiber, up to Σ-associativity.
--
-- The same shape kills `SieveScaleTower.Fiber₁ ₂ ₃` and
...
सावच्छेद-तन्तुः : (v : Vis) → Fiber v ≃ fiber सावच्छिन्न-q v
...
-- fibers skipped above; (b) the four other restricted fibers, which §२'s
