...
-- those at n-2.  Here as an EQUIVALENCE OF FIBERS, not a count -- the
-- fiber splits and the numbers are its shadow.  Weight is Piṅgala's,
-- laghu 1 guru 2.  No count and no closed form is proved here.
--
-- CHECKED: Agda 2.6.3 + cubical v0.5, --safe, exit 0.

module Virahanka_TheMatraFiberSatisfiesTheTwoStepRecurrence where
...
-- A string of n laghus realises every total, so from 2 upward the fiber
-- has two distinct points -- laghu-laghu against guru on the same tail --
-- and by Dvayam that embeds a bit.  Below 2 the fiber is a point.
...
-- so from total 2 upward the fiber is not a point: the map loses the
