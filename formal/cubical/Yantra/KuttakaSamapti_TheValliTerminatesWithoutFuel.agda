...
-- constant (`valli 200 137 60`) and proves nothing about it; every
-- claim in that module is `refl` on closed numerals. This file removes
-- the fuel: `valli` and `kuttaka` are defined by well-founded
-- recursion on the remainder, so termination is the definition being
-- accepted rather than a constant chosen large enough. `kuttaka`
-- returns the last nonzero remainder together with proofs that it
-- divides both inputs, so the divisor and its certificate are one
-- object.
--
------------------------------------------------------------------------

module Yantra.KuttakaSamapti_TheValliTerminatesWithoutFuel where

open import Yantra.Prakriti using (divides)
open import Yantra.Bhaga using (eucl)
