...
--
-- `Machine.Samorderna` identified the pair â• — â• with the triple
...
--     Brahmagupta's saordera would demand â” and on concrete numerals it
...
------------------------------------------------------------------------
-- The identification is a STRICT monoid isomorphism â” on the nose.
--
-- Because `_âŠµ_` is the conjugate DEFINITIONALLY, `anuloma` and `viloma`
-- are monoid homomorphisms with `refl` proofs: they do not merely respect
-- the operation up to a path, they respect it up to nothing.  So â• — â•
-- and ri-traya are not two isomorphic monoids â” they are the SAME
-- commutative monoid in two presentations (ààààà° à¨à®: everything is proven
...
anuloma-hom : (x y : â„• Ã— â„•) â†’ anuloma (x âŠ y) â‰¡ (anuloma x âŠáµ£ anuloma y)
anuloma-hom x y = refl

viloma-hom : (u v : rÄÅ›i-traya) â†’ viloma (u âŠáµ£ v) â‰¡ (viloma u âŠ viloma v)
viloma-hom u v = refl
