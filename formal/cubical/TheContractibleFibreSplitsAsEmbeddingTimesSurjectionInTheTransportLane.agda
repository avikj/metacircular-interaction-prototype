...
-- (loss-free ⟺ every residual contractible).
-- The split sits beside `-`.
------------------------------------------------------------------------

module TheContractibleFibreSplitsAsEmbeddingTimesSurjectionInTheTransportLane where
...
open import NaturalMachine.EveryTransportOwesItsResidual using (शेष ; अलोप-लक्षणम्)
