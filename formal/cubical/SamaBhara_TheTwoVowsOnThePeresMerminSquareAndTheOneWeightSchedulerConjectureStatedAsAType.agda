...
-- ��-��� � equal weight.  Compound built here (sama: equal; bhra:
-- load/weight).  The doctrine terms
-- inside are sourced: arpita/anarpita from Umsvti, Tattvrthastra 5.31
-- (c. 2nd�5th c. CE); durnaya from Siddhasena Divkara, Sanmatitarka 1.21.
...
-- existing Sthana/verdict machinery (YugaParivartana §4): the Born-weight
...
--
--   ahis   � no live standpoint is silently destroyed: every local
--              section satisfying its context's parity carries weight � 1
--              (AvaktavyaPrasava's Vivada: the undischarged claim is kept,
--              ��� � ������ ��� ���������).
--   aneknta � no standpoint is absolutized: what a CELL receives must not
...
--
-- THE CONJECTURE � `SamaBharaNiyama`: every scheduler
-- satisfying both vows is flat � AND ITS REFUTATION.  The
...
-- refutation demands is exact: aneknta must bind at PAIR grain �
