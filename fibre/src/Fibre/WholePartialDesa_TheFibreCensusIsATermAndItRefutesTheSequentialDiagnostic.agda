...
--
-- The note's scale has five levels; this module builds THREE.  Levels
-- ३ and ४ are not separated — the note establishes that "does a
-- retraction exist" does NOT separate them.  See §4.
--
-- CHECKED: Agda 2.6.3, agda/cubical v0.5 — the library's declared pin.
-- --cubical --safe, no postulates, no holes.
------------------------------------------------------------------------

module Fibre.WholePartialDesa_TheFibreCensusIsATermAndItRefutesTheSequentialDiagnostic where
...
-- simultaneous fibre census whose every entry is level १.
------------------------------------------------------------------------
...
-- अप्रतिकार्या) are both crowded fibres, and both land in `विकलादेश` here.
--
...
-- `नास्ति-प्रत्यानयनम्` (level ४).  Its candidate criterion — the fibre is a
