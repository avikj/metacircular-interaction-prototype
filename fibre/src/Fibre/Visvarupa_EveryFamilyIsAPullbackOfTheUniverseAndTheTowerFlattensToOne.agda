{-# OPTIONS --cubical --safe --guardedness #-}

-- Punargamana Â Vivarpa
--
-- EVERY FAMILY IS A PULLBACK OF ONE FIBRATION, AND A TOWER OF THEM FLATTENS
-- TO ONE.
--
-- THE CLAIM THIS FILE ANSWERS.  The fibre law is not a recurring motif in
-- this corpus.  It is the object classifier: in a univalent universe a form
-- over a base â” any family `B : A â’ Type` â” IS a map into the universe, and
-- `Î A B` is the total space of the one fibration
--
--     Ï : Î[ X âˆˆ Type â“ ] X  â’  Type â“        Ï = fst
--
-- pulled back along it.  One object classifies every family there is.  The
-- claim was made in prose.  Everything below is the part of it that is a
-- term.
--
-- WHAT IS ALREADY OWNED, AND IS THEREFORE CITED RATHER THAN RE-PROVED.
-- `Cubical.Functions.Fibration` has HoTT Lemma 4.8.1 (`fiberEquiv`), Lemma
-- 4.8.2 (`totalEquiv`) and Theorem 4.8.3 (`fibrationEquiv`), the last of
-- which is the classifier as an equivalence of TYPES:
--
--     (Î[ E âˆˆ Type â“ ] (E â’ A))  â‰  (A â’ Type â“).
--
-- Â§3 re-exports it under a corpus name and states where its content sits:
-- its `rightInv` is `ua`.  The classifier is a theorem OF univalence, not a
-- fact about Î.  Once again in this corpus a construction that looked
-- missing was a universal property already installed; grep before you
-- prove.
--
-- WHAT IS NEW HERE.
--
--   Â§2  The equivalence is the CANONICAL comparison map â” checked by
--       `refl`, not merely exhibited as some equivalence.  Which matters,
--       because of Â§6.
--
--   Â§5  `invisibleâ‰contractible`, at the locus where the law lives.
--       `Fibre.Carrier`'s header asserts the converse in prose â” "A
--       NON-contractible fibre cannot be declared equivalent to its base" â”
--       and the corpus proves it twice, in neither place generally:
--       `DefectCalculus.noEquivâ’badFibre` has the fibrewise
--       characterisation of `isEquiv` for an arbitrary map (in cubical Agda
--       that IS the definition, so it is one step), and `TritiyaMarga`'s
--       private `Test` module has both directions for ONE family
--       `Q : â• â’ Typeâ`, as a lemma on the way to Markov's principle.  What
--       is new is neither step but the locus: the statement about the
--       projection of an ARBITRARY family, in the library that owns the
--       law, packaged as an equivalence of propositions.  àà•à¾à§à¿à•à°à â” an
--       installed theorem has exactly one place to be quoted from.
--
--       The two directions are joined by a PATH between propositions rather
--       than by an implication assumed, which is why the converse cost
--       nothing here.  README Â§II, `invisibleExactlyWhenInvariant`, records
--       that lesson about the same shape: a path has an inverse, an
--       implication does not.
--
--   Â§6  â¦and the converse is about the PROJECTION, not about the mere
--       existence of an equivalence.  `Î Bool Br â‰ Bool` holds for a family
--       whose fibres are one EMPTY and one CROWDED: the census's two
--       failures (à¨à¾àààà¿ and à¨àààŸà¿ â” `SakalaVikalaDesa`) cancel
--       numerically, and an abstract equivalence sees neither.  "Invisible"
--       has to mean invisible OVER THE BASE, and Â§5's statement does.
--
--   Â§7  The tower flattens.  A tower of families of any finite height n
--       over A â” each storey a family over the previous total space â” is
--       ONE family over A, and the flattening is computed (`flatten`), not
--       merely shown to exist.  So iterating the fibre law never leaves the
--       universe, at every finite stage, and Â§7's last lines exhibit a
--       two-storey tower whose flattening holds by `refl`.
--
-- Nor that the corpus's lanes are instances of one formal statement. That
-- paragraph stands unchanged after this file. What is proved here is that THE
-- LAW has a universal form; no map is constructed from any lane's theorem to
-- it, and reading a lane as a pullback of Ï is, for every lane, still an act
-- of reading.
--
-- Nothing here is a statement about physics: the reading of gauge theory as a principal bundle with a
-- connection is prose in the README, and prose is what it stays until a
-- module says otherwise.  The tower of Â§7 is level-uniform (every storey at
-- one level â“); that is a convenience of statement, not a theorem.
--
-- ON SIZE.  `fiber Ï X` lives one universe above X, so Â§2's pullback is an
-- equivalence ACROSS levels.  That is the size of the classifier â” the
-- universe classifying its own families is exactly what does not fit inside
-- one level â” and not a defect of the statement.
--
-- ON THE NAME.  àµà¿àààµà°àà (vivarpa), "all-formed", "the form that has all
-- forms".  The word names the theophany of the Bhagavadgt's eleventh
-- adhyya; no verse is cited and no technical sense is being claimed from
-- that text.  It is used here as the label of this module, chosen for what
-- the theorem says.

module Fibre.Visvarupa_EveryFamilyIsAPullbackOfTheUniverseAndTheTowerFlattensToOne where

open import Cubical.Foundations.Prelude
open import Cubical.Foundations.Equiv
open import Cubical.Foundations.Isomorphism
open import Cubical.Foundations.HLevels
open import Cubical.Functions.Fibration
open import Cubical.Data.Sigma
open import Cubical.Data.Nat using (â„• ; zero ; suc)
open import Cubical.Data.Unit using (Unit* ; tt* ; isContrUnit*)
open import Cubical.Data.Bool using (Bool ; true ; false)
open import Cubical.Data.Empty using (âŠ¥)
open import Cubical.Relation.Nullary using (Â¬_)

open import Fibre.Carrier

private
  variable
    â„“ â„“a â„“b : Level

------------------------------------------------------------------------
-- Â§1  THE ONE FIBRATION
--
-- The universe of â“-small types, with a point of each chosen â” and the
-- projection that forgets the point.  Its fibre over a code IS the type
-- that code names.  There is nothing else in it: no structure, no choice,
-- no parameter.

Universal : (â„“ : Level) â†’ Type (â„“-suc â„“)
Universal â„“ = Î£[ X âˆˆ Type â„“ ] X

Ï€ : Universal â„“ â†’ Type â„“
Ï€ = fst

-- HoTT 4.8.1 at the universal family, where pâ»Â is the identity.  The fibre
-- over the code X is X.
fibreOfÏ€ : (X : Type â„“) â†’ fiber (Ï€ {â„“}) X â‰ƒ X
fibreOfÏ€ {â„“ = â„“} X = fiberEquiv (Î» (Y : Type â„“) â†’ Y) X

------------------------------------------------------------------------
-- Â§2  EVERY FAMILY IS ITS PULLBACK
--
-- Given B : A â’ Type â“, the square
--
--     Î A B â”â”â”â”â”â”â’ Universal â“
--       â”  (a,b) â¦ (B a , b)  â”
--      fst                    Ï
--       â”                     â”
--       â“                     â“
--       A â”â”â”â”â”â”â”â”â”â’ Type â“
--                B
--
-- commutes on the nose, and Î A B is the pullback.  The map that witnesses
-- it is the canonical one â” `comparison` below â” and `pullback-canonical`
-- says so by `refl`.

module _ {A : Type â„“a} (B : A â†’ Type â„“) where

  -- The top edge of the square: a point of the total space is a point of
  -- the type its base point names, so it is a point of the universal
  -- family.
  classifying : Î£ A B â†’ Universal â„“
  classifying (a , b) = B a , b

  -- The square commutes, and by refl: the code of the fibre a point lands
  -- in is the code the base point names.
  square : (p : Î£ A B) â†’ Ï€ (classifying p) â‰¡ B (fst p)
  square _ = refl

  comparison : Î£ A B â†’ Î£[ a âˆˆ A ] fiber (Ï€ {â„“}) (B a)
  comparison (a , b) = a , (classifying (a , b) , refl)

  -- Fibrewise from Â§1, with the base component untouched.  Written out
  -- rather than obtained from `Î-cong-equiv-snd` because that lemma's
  -- generalised variables put the two families at ONE level, and here the
  -- second family is a universe higher than the first (see ON SIZE).
  pullback-Iso : Iso (Î£ A B) (Î£[ a âˆˆ A ] fiber (Ï€ {â„“}) (B a))
  Iso.fun pullback-Iso (a , b) = a , invEq (fibreOfÏ€ (B a)) b
  Iso.inv pullback-Iso (a , u) = a , equivFun (fibreOfÏ€ (B a)) u
  Iso.rightInv pullback-Iso (a , u) i = a , retEq (fibreOfÏ€ (B a)) u i
  Iso.leftInv  pullback-Iso (a , b) i = a , secEq (fibreOfÏ€ (B a)) b i

  pullback : Î£ A B â‰ƒ (Î£[ a âˆˆ A ] fiber (Ï€ {â„“}) (B a))
  pullback = isoToEquiv pullback-Iso

  -- The equivalence IS the comparison map.  Without this line Â§2 would say
  -- only that the two types are equivalent somehow, which Â§6 shows is a
  -- strictly weaker statement than being a pullback.
  pullback-canonical : (p : Î£ A B) â†’ equivFun pullback p â‰¡ comparison p
  pullback-canonical _ = refl

------------------------------------------------------------------------
-- Â§3  THE CLASSIFIER, CITED
--
-- HoTT Theorem 4.8.3, `Cubical.Functions.Fibration.fibrationEquiv`: maps
-- into A and families over A are the same type, not merely in bijection on
-- points.  Read the library's proof for where the content is â” `rightInv`
-- is `ua`, `leftInv` is `ua` again with `ua-unglue`.  Univalence is the
-- whole of it.

classifier : (A : Type â„“) â†’ (Î£[ E âˆˆ Type â„“ ] (E â†’ A)) â‰ƒ (A â†’ Type â„“)
classifier {â„“ = â„“} A = fibrationEquiv A â„“

------------------------------------------------------------------------
-- Â§4  THE FIBRE LAW IS THIS, WITH A CONTRACTIBLE CLASSIFYING MAP
--
-- `Fibre.Carrier` takes f : A â’ B and forms the family `fibre f = singl âˆ˜ f`.
-- That family is the classifying map of `Carrier f`, so the carrier is a
-- pullback of Ï like anything else â” and the law's hypothesis
-- (`fibre-isContr`) is the statement that this classifying map lands in the
-- contractible codes.  Nothing about the law is special except where its
-- classifying map goes.

module _ {A B : Type â„“} (f : A â†’ B) where

  Carrier-classified : Carrier f â‰ƒ (Î£[ a âˆˆ A ] fiber (Ï€ {â„“}) (fibre f a))
  Carrier-classified =
    compEquiv (isoToEquiv (Carrier-as-Î£ f)) (pullback (fibre f))

------------------------------------------------------------------------
-- Â§5  INVISIBLE IFF CONTRACTIBLE
--
-- A family is invisible over its base â” its projection is an equivalence,
-- so the total space carries no information the base did not â” EXACTLY when
-- every fibre is contractible.  Both sides are propositions and the two
-- implications are joined into a path.
--
-- The forward direction is `Î-contractSnd`, which fifteen files of this
-- corpus already use.  The backward direction is the one `Fibre.Carrier`'s
-- header asserts in prose and none of those fifteen has: it is HoTT 4.8.1
-- (`fiberEquiv`) plus the definition of `isEquiv`, and it costs one line,
-- because the fibre of `fst` over a is B a on the nose.  See the header for
-- the two places the corpus proves it already, and why neither is a locus
-- the law can be quoted from.

module _ {A : Type â„“a} (B : A â†’ Type â„“b) where

  projection : Î£ A B â†’ A
  projection = fst

  invisible : Type (â„“-max â„“a â„“b)
  invisible = isEquiv projection

  contractibleFibres : Type (â„“-max â„“a â„“b)
  contractibleFibres = (a : A) â†’ isContr (B a)

  -- `Î-contractSnd` is this direction and is what the corpus quotes, but
  -- its generalised variables tie A and B to one level; the fibre of `fst`
  -- over a is B a on the nose, so the direct construction is as short and
  -- keeps the two levels apart.
  contractibleâ†’invisible : contractibleFibres â†’ invisible
  contractibleâ†’invisible c .equiv-proof a =
    isOfHLevelRespectEquiv 0 (invEquiv (fiberEquiv B a)) (c a)

  invisibleâ†’contractible : invisible â†’ contractibleFibres
  invisibleâ†’contractible e a =
    isOfHLevelRespectEquiv 0 (fiberEquiv B a) (e .equiv-proof a)

  invisibleâ‰ƒcontractible : invisible â‰ƒ contractibleFibres
  invisibleâ‰ƒcontractible =
    propBiimplâ†’Equiv (isPropIsEquiv projection)
                     (isPropÎ  (Î» _ â†’ isPropIsContr))
                     invisibleâ†’contractible
                     contractibleâ†’invisible

------------------------------------------------------------------------
-- Â§6  AND IT IS ABOUT THE PROJECTION
--
-- Â§5 cannot be weakened to "Î A B â‰ A".  Take the base Bool and the family
-- that is empty over one point and two-valued over the other.  The total
-- space is equivalent to the base â” and neither fibre is contractible: one
-- is à¨à¾àààà¿ (no source at all), the other à¨àààŸà¿ (crowded).  The census of
-- `SakalaVikalaDesa` separates those two failures; an abstract equivalence
-- adds them up and reports nothing.
--
-- So "the fibre law holds here" is never established by producing SOME
-- equivalence between total space and base.  It is established over the
-- base, by the projection, which is what Â§5 quantifies.

module CensusCancels where

  Br : Bool â†’ Type
  Br true  = Bool
  Br false = âŠ¥

  Î£-Br-Iso : Iso (Î£ Bool Br) Bool
  Iso.fun Î£-Br-Iso (true , b) = b
  Iso.fun Î£-Br-Iso (false , ())
  Iso.inv Î£-Br-Iso b = true , b
  Iso.rightInv Î£-Br-Iso _ = refl
  Iso.leftInv Î£-Br-Iso (true , _) = refl
  Iso.leftInv Î£-Br-Iso (false , ())

  -- The total space is equivalent to the baseâ¦
  totalâ‰ƒbase : Î£ Bool Br â‰ƒ Bool
  totalâ‰ƒbase = isoToEquiv Î£-Br-Iso

  -- â¦and the fibres are not contractible: over `false` there is no point at
  -- all, which is enough.
  fibres-not-contractible : Â¬ contractibleFibres Br
  fibres-not-contractible c = c false .fst

  -- Hence the projection is not an equivalence, by Â§5 read backwards.
  projection-not-invisible : Â¬ invisible Br
  projection-not-invisible e =
    fibres-not-contractible (invisibleâ†’contractible Br e)

------------------------------------------------------------------------
-- Â§7  THE TOWER FLATTENS
--
-- A tower of height n over A: a family over A, then a family over ITS total
-- space, then a family over that, n times.  `Tot` is the space at the top.
-- `flatten` computes a single family over A, by iterated Î, and
-- `flatten-correct` proves the top of the tower is its total space.
--
-- Read as the classifier (`tower-is-pullback`): a stack of n fibrations is
-- one pullback of Ï.  Iterating the fibre law does not leave the universe
-- at any finite height, and the witness is not an existence proof â” the
-- flattened family is written down, and reduces.

Fam : (A : Type â„“) â†’ â„• â†’ Type (â„“-suc â„“)
Fam A zero            = Unit*
Fam {â„“ = â„“} A (suc n) = Î£[ B âˆˆ (A â†’ Type â„“) ] Fam (Î£ A B) n

Tot : {A : Type â„“} (n : â„•) â†’ Fam A n â†’ Type â„“
Tot {A = A} zero _  = A
Tot (suc n) (B , F) = Tot n F

flatten : {A : Type â„“} (n : â„•) â†’ Fam A n â†’ A â†’ Type â„“
flatten zero _ _        = Unit*
flatten (suc n) (B , F) a = Î£[ b âˆˆ B a ] flatten n F (a , b)

flatten-correct : {A : Type â„“} (n : â„•) (F : Fam A n)
                â†’ Tot n F â‰ƒ Î£ A (flatten n F)
flatten-correct zero _ = invEquiv (Î£-contractSnd (Î» _ â†’ isContrUnit*))
flatten-correct (suc n) (B , F) =
  compEquiv (flatten-correct n F)
            (Î£-assoc-â‰ƒ {C = Î» a b â†’ flatten n F (a , b)})

tower-is-pullback : {A : Type â„“} (n : â„•) (F : Fam A n)
                  â†’ Tot n F â‰ƒ (Î£[ a âˆˆ A ] fiber (Ï€ {â„“}) (flatten n F a))
tower-is-pullback n F = compEquiv (flatten-correct n F) (pullback (flatten n F))

-- â¦and it computes.  A two-storey tower over Bool: the naturals, then the
-- proofs that the natural is zero.  Its flattening is not merely equivalent
-- to the hand-written family â” it IS it, by refl, in both the family and
-- the space.

private

  twoStorey : Fam Bool 2
  twoStorey = (Î» _ â†’ â„•) , ((Î» p â†’ snd p â‰¡ zero) , tt*)

  flatten-computes : (b : Bool)
                   â†’ flatten 2 twoStorey b â‰¡ (Î£[ n âˆˆ â„• ] Î£[ _ âˆˆ (n â‰¡ zero) ] Unit*)
  flatten-computes _ = refl

  tot-computes : Tot 2 twoStorey â‰¡ (Î£[ p âˆˆ (Î£ Bool (Î» _ â†’ â„•)) ] (snd p â‰¡ zero))
  tot-computes = refl
