...
--
--   A  �  �[ b ∈ B ] Trace b                     (the whole is the sum
...
-- trace family T provided A � �[ b ] T b" is NOT sufficient as stated, and
-- the counterexample is small: for f : Bool � Bool constantly `true`, the
-- family T b = Unit satisfies Bool � �[ b ] T b, and it is smaller than
...
-- tion of `Carrier f` with its �-form is the one already proved in
-- Carrier.agda, reused rather than restated.
--
-- The decomposition A � �[ b ] fiber f b is standard, and this repository
...
-- �[ x ] (a ≡ x); the contraction below is �[ x ] (x ≡ a), which is what
...
module _ {A B : Type ℓ} (f : A → B) where
  ...
  -- and this is literally Carrier.agda's �-form, not a lookalike:
...
------------------------------------------------------------------------
-- The proviso, refuted.  An equivalence A � �[ b ] T b that is not over B
...
-- Bool � �[ b ∈ Bool ] Unit.  The trace is a point everywhere, and it is
