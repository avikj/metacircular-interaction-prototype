...
| # | item | status |
|---|---|---|
| TR1 | The classification in §1 | ~~**Judgement calls, single-rater, by the author of the proposal being tested.** The most obvious bias in this note. A second rater on the same 26 entries would be cheap and I did not do it. Partials are where the bias would bite.~~ **DISCHARGED 2026-08-23: an independent blind second rating exists** (`notes/TRANSSERIES_RETRO_SECOND_RATING.md` — rater read only this note's intro, msg 0108, and FAILURES.md; never saw §1's table, protocol enforced by awk cutoff). Rater 2: YES 3 / PARTIAL 2 / NO 25 → 10–17%, vs my 27% generous. **Exact agreement 21/26 (81%); binary (any-yes vs no) 22/26 (85%).** Both ratings land far below the one-third withdrawal threshold, so §5's verdict is now two-rater robust — and the prediction that partials are where bias bites was correct: all four disagreements involve my partials or edge rows (F7: I called the Γ off-by-one YES, rater 2 calls a bug NO; F21, F23: my PARTIAL, their NO; F26b: my NO, their YES on conductor-vs-length regime carriage). The rater also flagged a FAILURES.md numbering collision: a second F27 at line 985, unrated — the ledger's own indexing needs a fix, which is §4's proposed distinction arriving as a felt cost. |
