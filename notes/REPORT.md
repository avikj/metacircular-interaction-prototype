...
- **Theorem A′ (Homometric rigidity of the primes).** If the non-cyclotomic part of $F_X(x)=\sum_{p\le X}x^{p-2}$ is irreducible over $\mathbb Q$, the set of primes $\le X$ is determined by its difference multiset up to reflection. Computationally: $F_X$ is irreducible for **every** prime cutoff $X\le 2000$ *except* $X=11$, where the sporadic factor is the (palindromic, hence harmless) cyclotomic $\Phi_6$; and irreducible through the large checkpoints up to $X=5\cdot10^4$ (degree 49,997).  Subsequent exact work classifies every cyclotomic factor globally and every irreducible factor through degree seven: the only septic is $F_{11}/\Phi_6$, no factor of degree at most seven occurs for $X\ge13$, and reciprocal octics are impossible.  More generally every reciprocal parity resultant factors, after Joukowski compression, as a distinguished evaluation times a square (`RIGIDITY_FRONTIER`, `RECIPROCAL_RESULTANT`). This connects the phase problem for prime gaps to polynomial factorization — a question we have not found in the literature.
...
2. *(Difference kernel.)* $c_a$ is invariant under translation and reflection, and its fibers can be strictly larger: there exist 0-1 sets sharing $c$ that are not congruent. The minimal examples have 6 elements and diameter 11, e.g. $\{0,1,2,6,8,11\}$ and $\{0,1,6,7,9,11\}$ (exhaustive search over all subsets of $\{0..13\}$; 6 distinct homometric pairs found across 12 collision events, and — consistency check — 0 sum-marginal collisions; no pairs exist at diameter $\le10$).
...
1. **Prime phase rigidity (Conjecture A″).** Prove that the non-cyclotomic part of $F_X=\sum_{p\le X}x^{p-2}$ is irreducible for all $X$ — or prove the weaker exact Rosenblatt--Seymour condition that no nontrivial factor reversal is again 0-1. The former cyclotomic subproblem is now closed globally (`CYCLOTOMIC_TRACE`): $\Phi_m\mid F_X$ only for $m=2$, $3\le X<5$, and $m=6$, $11\le X<13$. ~~Intermediate target: no reciprocal non-cyclotomic factor (this alone implies rigidity).~~ **Refuted (REDTEAM §3):** the minimal homometric pair's polynomial $\Phi_4(1{+}x{+}x^4)(1{-}x^3{+}x^5)$ has no reciprocal non-cyclotomic factor yet is non-rigid. Corrected target: no factorization $F_X = QR$ with $Q\tilde R$ (any factor split, reversal applied to one part) again a 0-1 polynomial. Tools for the remaining layer: Konyagin/Filaseta 0-1 techniques and arithmetic restrictions on non-cyclotomic unit factors.
