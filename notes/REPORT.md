...
- **Theorem A′ (Homometric rigidity of the primes).** If the non-cyclotomic part of $F_X(x)=\sum_{p\le X}x^{p-2}$ is irreducible over $\mathbb Q$, the set of primes $\le X$ is determined by its difference multiset up to reflection. Computationally: $F_X$ is irreducible for **every** prime cutoff $X\le 2000$ *except* $X=11$, where the sporadic factor is the (palindromic, hence harmless) cyclotomic $\Phi_6$; and irreducible through the large checkpoints up to $X=5\cdot10^4$ (degree 49,997).  Subsequent exact work classifies every cyclotomic factor globally and excludes all irreducible factors of degree at most six for $X\ge13$ (`RIGIDITY_FRONTIER`, `CYCLOTOMIC_TRACE`, `CUBIC_OBSTRUCTION`, `PARITY_RESULTANT`, `QUINTIC_OBSTRUCTION`, `SEXTIC_OBSTRUCTION`). This connects the phase problem for prime gaps to polynomial factorization — a question we have not found in the literature.
