...
| T25.D — guarded Indra equation | `IndraNet.Coinductive.Net` checks a coinductive unfolding `Net x ≃ L x × ((y : J) → Net y)`, a productive constructor, and `Bisim → path`; `NaturalMachine.ProductiveIndraNet` checks productive propagation and equality of every finite observation of bisimilar streams. | **Coinductive/productivity analogue checked; explicit-later guarded equation open.** | There is no explicit later modality `▷`, clock quantification, or `Image_xy`; `--guardedness` is constructor-based coinduction, not Agda's distinct `--guarded` later/tick interface. `Bisim → path` is not an exhibited equivalence and no final-coalgebra universal property is proved. The source's stage-relative/clocked equation remains a program. |
