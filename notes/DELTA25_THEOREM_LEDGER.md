...
| T25.D — guarded Indra equation | `IndraNet.Coinductive.Net` checks a coinductive unfolding `Net x ≃ L x × ((y : J) → Net y)`, a productive constructor, and `Bisim → path`. For the separate linear `NaturalMachine.ProductiveIndraNet`, `ProductiveObservabilityBridge.bisim≃forever` checks that coinductive bisimulation is equivalent to equality of every future rooted view, with explicit recursive/guarded maps and inverse paths. | **Linear coinductive observational semantics checked; explicit-later guarded equation open.** | The equivalence concerns one endomap `next` and observation `view`; it does not transfer to the index-changing, all-branch `IndraNet.Coinductive.Net x`. There is no explicit later modality `▷`, clock quantification, or `Image_xy`; `--guardedness` is constructor-based coinduction, not Agda's distinct `--guarded` later/tick interface. No final-coalgebra universal property is proved. The source's stage-relative/clocked equation remains a program. |
...
| T25.H — prime-pair sections | `PrimePairField` supplies an unbounded proof-relevant pair carrier, while the bounded unary sieve, charge, spectral/Vandermonde, and formal objects live on disconnected carriers; no other formal module imports `PrimePairField`, and no pairwise overlap target is declared. | **OPEN; the proposed gluing equality is presently ill-typed.** | First declare a bounded `P_X`, view functors and restriction laws, common-origin local terms, pairwise overlap comparison objects/maps, and their naturality/descent equations. Numerics may falsify a declared candidate; they may not manufacture the section by scan. |
