...
| T25.H — prime-pair sections | `PrimePairField` supplies the earlier unbounded parameterized carrier. `Pairfield.BoundedPrimePair` now supplies a finite ordered leg-box carrier `p,q ≤ X` with actual-prime certificates, exchange, sum/signed-gap views, bound enlargement with identity/composition, and covariant centre/gap-fibre maps; it is imported by the Lean aggregate. `Pairfield.PrimePairDecomposition` installs `(7,11)` in both fibres and checks a real endpoint lost by the mandatory `+2` waypoint. | **Finite common ambient and one decomposition control checked; the source-coordinate section problem remains open and the four-view gluing equality is still ill-typed.** | The landed bound is on both legs, while Delta 24 defines `P_X` by a centre cutoff and UP-D0025 describes integral `w,r` jewels with `p+q=2w`, `q-p=2r` without restating the bound. A source-faithful next base is the even-sum centre-cutoff subtype of `BoundedPrimePair (2*X)`, retaining ordered exchange; then prove centre-fibre stability under `X≤Y`. Current `weakenCenterFiber`/`weakenGapFiber` are forward inclusions, despite “restriction” in their comments. The Agda carrier, unary sieve, charge, spectral/Vandermonde, and Haskell optimizer still do not consume this same base; common-origin local terms, overlap maps/naturality, and descent equations remain absent. Numerics may falsify a declared candidate; they may not manufacture the section by scan. |
