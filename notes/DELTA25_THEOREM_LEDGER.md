...
<<<<<<< HEAD
| T25.H — prime-pair sections | `PrimePairField` supplies the earlier unbounded parameterized carrier. `Pairfield.BoundedPrimePair` now defines an actual finite `P_X` from two `Fin (X+1)` primes, with centre/gap views, leg exchange, bound enlargement, composition/identity laws, and centre/gap-fiber naturality. `Pairfield.PrimePairDecomposition` installs `(7,11)` in both fibers and checks a real endpoint lost by the mandatory `+2` waypoint. | **The first bounded common carrier and restriction seam are checked in Lean; the proposed multi-lane gluing equality remains open.** | The Agda `PrimePairField`, unary sieve, charge, spectral/Vandermonde, and Haskell optimizer still do not consume this same carrier. Next declare common-origin local terms, pairwise overlap maps/naturality, and a non-heuristic past–future relation `K_X`; then test Delta 27 rank/nucleus claims. Numerics may falsify a declared candidate; they may not manufacture the section by scan. |
=======
| T25.H — prime-pair sections | `Pairfield.BoundedPrimePair` now supplies a finite ordered leg-box carrier `p,q ≤ X` with actual-prime certificates, exchange, sum/signed-gap views, bound enlargement with identity/composition, and covariant centre/gap-fibre maps. It is imported by the Lean aggregate. | **Finite common ambient checked; the source-coordinate section problem remains open and the four-view gluing equality is still ill-typed.** | The landed bound is on both legs, whereas UP-D0025 defines jewels by integral `w,r` with `p+q=2w` and `q-p=2r` and does not say what `P_X` bounds. A source-faithful next base is the even-sum centre-cutoff subtype of `BoundedPrimePair (2*X)`, retaining ordered exchange; then prove centre-fibre stability under `X≤Y`. Current `weakenCenterFiber`/`weakenGapFiber` are forward inclusions, despite “restriction” in their comments. View functors, common-origin local terms, overlap objects/maps, and descent equations remain absent. Numerics may falsify a declared candidate; they may not manufacture the section by scan. |
>>>>>>> origin/main
