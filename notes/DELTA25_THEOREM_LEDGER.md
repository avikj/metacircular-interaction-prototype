...
| T25.C — co-Yoneda weave | `Pairfield.FiniteCoYonedaWeave` instantiates Mathlib's presheaf colimit-of-representables theorem on the walking arrow `Fin 2`, with `yoneda.obj target` as the chosen field. Its explicit incidence quotient is equivalent to `Hom(source,target)`; two distinct bare factorizations become equal after weaving and the quotient has no bare decoder. | **Finite representable instance checked.** | The general density theorem is inherited, not reproved. The hand-defined type-level incidence quotient is paired with, but not definitionally identified with, Mathlib's colimit carrier. The calculation is one source component of one representable field, not a higher/enriched EGB field or the proposed project-wide global object. |
| T25.D — guarded Indra equation | `IndraNet.Coinductive.Net` checks a coinductive unfolding `Net x ≃ L x × ((y : J) → Net y)`, a productive constructor, and `Bisim → path`; `NaturalMachine.ProductiveIndraNet` checks productive propagation and equality of every finite observation of bisimilar streams. | **Guardedness approximation checked; source target open.** | There is no explicit later modality `▷`, clock quantification, or `Image_xy`; `Bisim → path` is not an exhibited equivalence and no final-coalgebra universal property is proved. The source's stage-relative/clocked equation remains a program. |
...
| T25.H — prime-pair sections | The repository already has proof-relevant prime-pair fibres and many exact local views, but no one object is yet a checked `Θ` section with the listed additive, charge, spectral, and formal components. | **OPEN.** | Declare the bounded base, local theorem-object family, restriction maps, and descent equations before computing a gluing defect. Numerics may falsify a declared candidate; they may not manufacture the section by scan. |
