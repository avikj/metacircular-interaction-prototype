...
- **Recorded priors — REPLICATED, large effects.** Good Judgment
  ...
  this entire survey.
- **Backlink magic — FOLKLORE.** No controlled study shows
  Roam/Obsidian-style automatic bidirectional links improve retrieval
  or creativity; the Zettelkasten community's own flagship analysis
  argues context-free backlinks dilute the graph. What IS replicated:
  "lost in hyperspace" overload in link-dense structures, mitigated by
  hierarchy + maps. Sharpens L3/§3.1: **sparse, annotated links (each
  stating its relation) plus a stable hierarchy** — not auto-linking.
...
1. Linking norm — now: *annotated* links, sparse, stable hierarchy.
