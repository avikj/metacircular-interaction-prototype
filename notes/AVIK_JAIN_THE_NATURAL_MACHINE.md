...
`Ratri` makes the same refusal at the level of generated formal results. Its loop lands both roads the kernel accepts: `Nirdharita_...` when a field is determined and an equivalence is exhibited, and `Anirdharita_...` when it is not and two inhabitants agreeing on every other field are exhibited as the separating pair. The negative files are not prose annotations on failed proof search. They are small checked terms showing the distinction that survives. Existing paths are skipped rather than overwritten. A later pass runs on the corpus changed by the previous pass, so the generated equivalences and separations alter what the next pass can reach. The first pass landed thirty-four probes; later passes continued until a pass landed none. That is a fixpoint of what the current generator can emit, not a declaration that the mathematics is finished.

`Nama_TheNameIsCarriedAndTheHashIsTheBase.hs` puts a related boundary around identity itself. It content-addresses presentations and explicitly says what that address does **not** prove: sameness of mathematical structure. The repository now has several positive relations standing beside that refusal. `Alopa_TheFirstRoadIsStatedThriceAndTheThreeAreOneTerm` has three statements that are literally the same term by `refl` while refusing to identify the routes or reasons that produced them. `Bhedanirnaya` has two same-clause testers that are not definitionally one function; it proves the path and transports a theorem. `MadhyaVinimaya` identifies one formal law across six sites and still retains six source or standpoint routes. **The task is not to call everything connected equivalent. The task is to type the relation that was actually earned.**
