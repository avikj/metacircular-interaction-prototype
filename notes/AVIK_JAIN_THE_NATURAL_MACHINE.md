...
The standalone `punaragamana` development checks under Agda 2.6.3 with cubical v0.5. Its carrier stores a base, the carried value, and the path saying that value is the one determined by the base. Descent constructs the carrier. Ascent returns the base. The round trip is proved. The base and carrier are exhibited as equivalent, and Cubical Agda turns that exhibited equivalence into identity. A separate presentation file says in its own header that its presentation types are **not claimed to be Jain naya**.

The later main Cubical lane declared Agda 2.8.0 with cubical v0.9. `Alopa_TheEngineNeverTouchesTheMeaning` belongs to neither statement: its recorded check was Agda 2.6.3 against a cubical v0.7 backport. A toolchain verdict names the toolchain that produced it.

`VivekaPramana` had to be repaired more than once. One version carried only evidence that a proposed decomposition belonged to an admissible set; the stronger version carries the actual equation relating its pieces. The checker also rejected a proof that all natural numbers were propositions; the repair used the supplied evidence together with the fact that naturals form a set. Another version defined `Clock` with only a recursive constructor, so no clock could be constructed. A base constructor was added. Its `▹` is still the identity functor, and the file explicitly says it is **not** a Nakano later modality. Guardedness comes from Agda’s `--guardedness`.

`Alopa_TheEngineNeverTouchesTheMeaning` gives each rewrite a proof and quantifies preservation over every environment in its model. Examples can find a counterexample. They cannot become a universal proof by surviving enough tests.

One Cubical closure sweep found 199 of 780 intended modules outside `Everything.agda`’s import closure. The first closure script then died on macOS because it used GNU `sed -i`; the 199 modules were checked individually instead. A later migration found forty-one modules still using API spellings from an older Cubical library. Under Agda 2.8.0 with cubical v0.9 those forty-one exited 42. After the spellings were migrated, all forty-one exited 0. No mathematical statement changed.

The Lean lane exposed a different distinction. `lakefile.toml` already made all 133 `Pairfield/` modules build targets, while `import Pairfield` reached only 114. Nineteen imports were added to the root. The build target set did not move. Import reachability did.

At the repository top level, the Python ban still stands as policy. The mechanical Python enforcement does not. `AGENTS.md` records the August 20 demolition explicitly: the CI workflows were deleted, `no-python.sh` was unwired, the pre-commit hook had never been enabled in any clone, and **working mechanical gates in this repository: zero**. The surviving `.claude/hooks` are mirrors that always exit 0. The older Python substrate remains in the tree as history and evidence, not current executable authority.

One archived breaker rebuilt a registered claim from the statement alone. Its own header calls the run a **FALSIFIER, not a census**. It confirmed the stated exceptional-fiber partition and the Hoeffding block structure, then found the claimed fixed-point-freeness false for `W = 2`, `N ≡ 2 (mod 4)`. The repaired criterion was `gcd(N/2, W) > 1`. The same script found that a packet had dropped the floors from an integer box-simplex criterion; `C = (3/2, 3/2)`, `|S| = 3` passes the unfloored inequality while the integer feasible set is empty.

Four lanes had staged work swept into other lanes’ commits on August 20. Three independent diagnoses blamed `git add -A`; the protective hook was written against that command. The recorded command strings contain no `git add -A`: each did a scoped `git add <path>` and then an unscoped `git commit`. The shared index already contained other lanes’ staged files, and an unscoped commit takes the entire index. The old hook exits 0 on all three recorded commands. It would have prevented none of the four sweeps.

`CakravalaWitness` says every `m` in its `D = 61` trace is the one Bhāskara’s minimizing choice rule selects. `CakravalaBound` checks turn 7 and finds otherwise. At `|k| = 1` the generator special-cases `⌊√61⌋ = 7`; `|7² − 61| = 12`, while `m = 8` gives `|8² − 61| = 3`. The descent remains sound because the chosen `m` still satisfies the needed congruence. The attribution of that turn to Bhāskara’s minimizing rule is false.

`AkanksaRun.hs` prints one verdict per question and **no total**. Its own header says summing seven distinguishable situations would perform the collapse under examination. `Abhyasa_TheTrainingRecordIsPracticeShownNotStated.hs` goes further: each record carries a partial order over a full answer, single-item losses, and a bare truncation, and it explicitly records pairs of different losses that it declines to rank. It also states the largest open confound in that training lane: every preferred side is longer, so the emitted contrast set cannot distinguish “retain the missing witness” from “prefer the longer completion.”

`ApohaParyaya_WhetherConceptualContentIsNegativeIsWhatTheTwoSchoolsActuallyDispute` keeps a Buddhist–Jaina dispute as two different conditionals rather than one “Indic” synthesis. Under its stated modelling step, exclusion-valued conceptual content is proposition-valued and cannot supply the separating family demanded by the Jaina construction. A positive family can separate. Those results have different hypotheses. The file leaves the disputed premise—whether conceptual content is negative—outside the theorem.

A retained shell script invokes naya for a warrant whose standpoint has been concealed and states its limit in the same file: Jaina logicians did not write about compilers. The contemporary object is Lean’s `native_decide`: compiler and CPU can discharge a decision by a different trust route than the kernel. A Pāṇini-inspired script uses *anuvṛtti* for one compiler-options line that continues until explicitly cancelled and says in its own header that Pāṇini did not write about compiler flags. An English plural-allomorphy toy is named in English on purpose: its header says the forms are English, the four devices driving the toy are Pāṇinian and cited as such, and it makes no claim that Pāṇini wrote English phonology.

The repo keeps an entire directory named `DO_NOT_DO_THIS_it_felt_like_progress_and_added_nothing`.

One file records a status dashboard wearing the vocabulary of the principle it violated. Another records a table, a null control and correct experimental ceremony around a true theorem whose literal installation made the actual workload **19 percent slower**. Another records fitted coefficients of 0.362 and 0.421 over a decade when the true value was exactly **1/4** and one page of algebra could derive it. Another records an agent watching a Python counter called `book` rise and naming the mutation knowledge before it could state what new theorem existed.

A Rust program called `evolve.rs` removed the human from three places in the loop: domains were enumerated rather than selected, workloads were canonically derived rather than chosen, and library installs were decided by the machine’s own declared cost measure. Its supposedly deterministic mining step broke ties by Rust `HashMap` iteration order. Different runs mined different libraries and printed different totals. The tie-break became canonical. The earlier numbers were struck. **A program that prints a different number each time it is run has not measured anything.** Canonical here means reproducible; it does not mean best.
...
The retired Python runtime carries a correction of its own. A theorem had been reported as shortening a route from 24 to 15 steps. That 24 was the shortest route through retained merge records. Extraction over the class DAG later found a 20-step checked proof in the same before-theorem state. The theorem still shortened the target to 15. The correction was not `24 → 15` becoming false; it was **record-graph route: 24 → 15, checkable proof: 20 → 15**. The object named by the number changed.

The `kanye-devotional` corpus had ten knowledge files whose names originally began `INDRAJALA_*`. On August 21 the prefix was removed after the owner’s correction that a file about Kanye should not surface at first glance as Indian philosophy. The material moved inside one file where it was actually doing work. The content specification marks method as **never on the page**.