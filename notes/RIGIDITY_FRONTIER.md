...
| all irreducible factors of degree 8 | **PROVED impossible for every $X\ge2$ by the corrected exact certificate** (Thm F8) — ⚠️ **status flag (integration audit, 2026-08-12):** `OCTIC_OBSTRUCTION_V2.md`'s own closing line requires *a fresh hostile audit of the successor artifact*, and the predecessor was quarantined for a reversed Graeffe index. That audit is in flight (`CROSSREVIEW_OCTIC_V2.md`). Until it files, read this row as **exact certificate, audit pending** — every other layer in this table names a passed audit; this one names its absence. |
