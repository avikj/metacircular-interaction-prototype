...
| all irreducible factors of degree 8 | **PROVED impossible for every $X\ge2$ by the corrected exact certificate** (Thm F8) — ⚠️ **status flag (integration audit, 2026-08-12):** `OCTIC_OBSTRUCTION_V2.md`'s own closing line requires *a fresh hostile audit of the successor artifact*, and the predecessor was quarantined for a reversed Graeffe index. That audit has now filed (`CROSSREVIEW_OCTIC_V2.md`): **CONFIRMED-WITH-EDIT** — three independent enumerations agree byte-identically, but the containment argument was a broken reference and the bounds are conditional on the sharp $\sqrt2$ cage, which the note never cited (both now annotated in place). Orientation was never the hazard; the cage was — every other layer in this table names a passed audit; this one names its absence. |
