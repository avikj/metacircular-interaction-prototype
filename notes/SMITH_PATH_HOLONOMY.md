...
`D`. Fix one path `p_0`. Every `p` determines a target holonomy

\[
H_p=U_pU_{p_0}^{-1}\in\operatorname{Aut}(\operatorname{coker}D).
\]

Let `G` be the subgroup generated by these holonomies. A task
...
This is just the universal property of the orbit quotient: (7) is necessary
