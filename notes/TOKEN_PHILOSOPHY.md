...
`machinery/test_token_philosophy.py` (28/28, six hostile controls). Two
...
- `PROVE` — **Several places.** Everything above is one place. With several
  places the fitting condition is $\partial_0t+\partial_0t'\le a$ as multisets,
  which is what the derivation of Theorem 13 will want, but the objects are no
  longer determined by a token count and "all placements of a pad are equal"
  needs restating. I expect Theorem 14 to survive with the marking-wise
  independence relation; I have not written it and it is a guess.
